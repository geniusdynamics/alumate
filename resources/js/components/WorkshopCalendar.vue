    ...
    <div class="workshop-calendar">
        <div class="calendar-header">
            ...
            <div class="header-actions">
                <button @click="showCreateModal = true" class="btn-primary">Host Workshop</button>
                <div class="view-toggle">
                    <button @click="currentView = 'calendar'" :class="['view-btn', currentView === 'calendar' ? 'active' : '']">
                        <Icon name="calendar" class="h-4 w-4" />
                        Calendar
                    </button>
                    <button @click="currentView = 'list'" :class="['view-btn', currentView === 'list' ? 'active' : '']">
        ...
        <div class="calendar-filters">
            <div class="filter-row">
                ...
                <div class="filter-group">
                    <input v-model="filters.search" @input="applyFilters" type="text" placeholder="Search workshops..." class="form-input" />
                </div>
            </div>
        </div>
                ...
                <div v-for="week in calendarWeeks" :key="week.weekNumber" class="calendar-week">
                    ...
                    >
                        ...
                        <div class="day-events">
                            ...
                            ></div>
                            <div v-if="day.workshops.length > 2" class="more-events" @click="showDayEvents(day)">+{{ day.workshops.length - 2 }}</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div v-else class="list-view">
            <div class="workshops-list">
                <div v-for="workshop in filteredWorkshops" :key="workshop.id" class="workshop-card" @click="selectWorkshop(workshop)">
            ...
            <div class="modal-content workshop-modal" @click.stop>
                ...
                <div class="workshop-details">
                    <div class="detail-section">
                        <div class="host-info">
                            <img :src="selectedWorkshop.host.avatar_url" :alt="selectedWorkshop.host.name" class="h-12 w-12 rounded-full" />
                            <div>
            ...
            <div class="modal-content create-modal" @click.stop>
                ...
                <form @submit.prevent="createWorkshop">
                    <div class="form-group">
                        ...
                        <input v-model="newWorkshop.title" type="text" class="form-input" placeholder="e.g., Advanced React Patterns" required />
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label>Date *</label>
                            <input v-model="newWorkshop.date" type="date" class="form-input" :min="today" required />
                        </div>
                        <div class="form-group">
                            <label>Start Time *</label>
                            <input v-model="newWorkshop.start_time" type="time" class="form-input" required />
                        </div>
                        <div class="form-group">
                            <label>End Time *</label>
                            <input v-model="newWorkshop.end_time" type="time" class="form-input" required />
                        </div>
                    </div>

                    <div class="form-group">
                        <label>Max Attendees</label>
                        <input v-model.number="newWorkshop.max_attendees" type="number" min="1" max="500" class="form-input" placeholder="50" />
                    </div>
...
export default {
    ...
    computed: {
        ...
        calendarWeeks() {
            ...
            for (let i = 0; i < 42; i++) {
                const dayWorkshops = this.workshops.filter((w) => new Date(w.date).toDateString() === currentDate.toDateString());
...
.calendar-header-cell {
    @apply border-r border-gray-200 p-3 text-center text-sm font-medium text-gray-700;
}
...
.calendar-day {
    @apply min-h-24 cursor-pointer border-r border-gray-200 p-2 hover:bg-gray-50;
}
...
.event-dot {
    @apply h-2 w-2 cursor-pointer rounded-full;
}
...
.more-events {
    @apply cursor-pointer text-xs text-blue-600;
}
...
.workshop-card {
    @apply cursor-pointer rounded-lg border bg-white p-6 transition-shadow hover:shadow-md;
}

.workshop-header {
    @apply mb-3 flex items-start space-x-4;
}

.workshop-date {
    @apply min-w-16 rounded-lg bg-blue-50 p-3 text-center;
}
...
.attendee-count {
    @apply mb-2 flex items-center space-x-1 text-sm text-gray-600;
}
...
.register-btn:disabled {
    @apply cursor-not-allowed bg-gray-300 text-gray-500;
}
...
.close-btn {
    @apply rounded-lg p-1 hover:bg-gray-100;
}
...
.agenda-item {
    @apply flex items-center space-x-4 rounded-lg bg-gray-50 p-2;
}

.agenda-time {
    @apply min-w-16 text-sm font-medium text-blue-600;
}
