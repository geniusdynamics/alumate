    ...
    <div class="recent-activity-container">
        <div class="activity-header">
            ...
            <div class="activity-controls">
                <select v-model="selectedType" @change="filterActivities" class="type-filter">
        ...
        <div class="activity-timeline">
            ...
            <div v-else class="timeline-items">
                <div
                    ...
                    :key="activity.id"
                    :class="['timeline-item', `activity-${activity.type}`, { 'timeline-item-last': index === filteredActivities.length - 1 }]"
                >
...
const filteredActivities = computed(() => {
    ...
    return filtered.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).slice(0, displayedCount.value);
});

const hasMoreActivities = computed(() => {
    const totalFiltered =
        selectedType.value === 'all' ? activitiesData.value.length : activitiesData.value.filter((a) => a.type === selectedType.value).length;
...
.stat-card {
    @apply flex items-center gap-3 rounded-lg bg-gray-50 p-4 dark:bg-gray-700;
}
...
.timeline-line {
    @apply mt-2 h-full w-px bg-gray-200 dark:bg-gray-600;
    min-height: 2rem;
}
...
.avatar-placeholder {
    @apply flex h-full w-full items-center justify-center bg-gray-300 dark:bg-gray-600;
...
.load-more {
    @apply border-t border-gray-200 pt-4 text-center dark:border-gray-700;
}

.load-more-button {
    @apply mx-auto flex items-center gap-2 px-4 py-2;
