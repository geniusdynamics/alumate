...
import type { FormConfig, FormField, FieldType, CRMIntegration, FormSubmission } from '@/types/forms'
...
describe('FormBuilder.vue', () => {
  ...
  describe('Unit Tests - Field Operations', () => {
    it('adds new field on drag and drop', async () => {
      ...
      expect(mockSocket.emit).toHaveBeenCalledWith('form:change', expect.objectContaining({
        type: 'field_added',
        tenantId: 'tenant-1'
      }))
    })
  ...
  describe('Unit Tests - Validation and Submission', () => {
    ...
    it('handles form submission successfully with CRM', async () => {
      ...
      wrapper.vm.crmIntegration = { crmType: 'hubspot', connectionId: 'conn1', fieldMapping: [{ formFieldId: wrapper.vm.formFields[0].id, crmFieldId: 'email' }] } as CRMIntegration
      ...
      expect(mockedAxios.post).toHaveBeenCalledWith('/api/crm/submit', expect.objectContaining({
        tenantId: 'tenant-1'
      }))
      expect(mockedAxios.post).toHaveBeenNthCalledWith(2, '/api/forms/test-form-1/submissions', expect.objectContaining({
        tenantId: 'tenant-1'
      }))
      expect(mockSocket.emit).toHaveBeenCalledWith('form:submit', expect.objectContaining({
        tenantId: 'tenant-1'
      }))
    })
    ...
    it('submits to CRM with field mapping and tenant context', async () => {
      ...
      wrapper.vm.crmIntegration = { crmType: 'salesforce', connectionId: 'conn1', fieldMapping: [{ formFieldId: 'field1', crmFieldId: 'email' }] } as CRMIntegration
  ...
  describe('Tenant Isolation Tests', () => {
    ...
    it('includes tenantId in all submissions and broadcasts', async () => {
      ...
      await wrapper.vm.handleFormSubmission(mockFormData)
      expect(mockedAxios.post).toHaveBeenNthCalledWith(1, '/api/forms/test-form-1/submissions', expect.objectContaining({
        tenantId: 'tenant-1'
      }))
      expect(mockSocket.emit).toHaveBeenCalledWith('form:submit', expect.objectContaining({
        tenantId: 'tenant-1'
