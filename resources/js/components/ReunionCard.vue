<template>
    <div class="overflow-hidden rounded-lg bg-white shadow-md transition-shadow duration-200 hover:shadow-lg">
            ...
            <div class="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-blue-100">{{ reunion.class_identifier || `Class of ${reunion.graduation_year}` }}</p>
                    </div>
                    <div v-if="reunion.reunion_year_milestone" class="rounded-full bg-white bg-opacity-20 px-3 py-1">
                        <span class="text-sm font-medium text-white">{{ reunion.reunion_year_milestone }} Year</span>
                    </div>
                </div>
            </div>

            <!-- Theme banner -->
            <div v-if="reunion.reunion_theme" class="border-b bg-blue-50 px-6 py-2">
                <p class="text-sm italic text-blue-800">"{{ reunion.reunion_theme }}"</p>
            </div>
        </div>
        ...
        <div class="p-6">
            ...
            <!-- Description -->
            <p class="mb-4 line-clamp-3 text-sm text-gray-700">{{ reunion.description }}</p>
            ...
            <div class="mb-4 flex items-center justify-between">
                ...
                <div v-if="reunion.ticket_price" class="text-sm font-medium text-gray-900">${{ reunion.ticket_price }}</div>
            </div>

            <!-- Organizer -->
            <div class="mb-4 flex items-center">
                <img :src="reunion.organizer.avatar_url || '/default-avatar.png'" :alt="reunion.organizer.name" class="mr-2 h-6 w-6 rounded-full" />
                <span class="text-sm text-gray-600">Organized by {{ reunion.organizer.name }}</span>
            </div>

            <!-- Status indicator -->
            <div class="mb-4">
                <span :class="['inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium', getStatusClasses(reunion.status)]">
        ...
        <div class="flex items-center justify-between border-t bg-gray-50 px-6 py-4">
            <button @click="$emit('view', reunion)" class="text-sm font-medium text-blue-600 hover:text-blue-800">View Details</button>

            <div class="flex space-x-2">
                <button
                    v-if="canRegister"
                    @click="$emit('register', reunion)"
                    class="rounded-md bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                    Register
                </button>

                <button v-else-if="isUpcoming" disabled class="cursor-not-allowed rounded-md bg-gray-300 px-4 py-2 text-sm text-gray-500">
