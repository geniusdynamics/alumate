<template>
    <button
        ...
        :disabled="loading"
        :class="[
            'bookmark-button flex items-center space-x-1 rounded-full px-3 py-1 text-sm transition-all duration-200',
            isBookmarked ? 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200',
        ]"
        :title="isBookmarked ? 'Remove bookmark' : 'Bookmark this post'"
    >
        <i :class="['transition-all duration-200', isBookmarked ? 'fas fa-bookmark text-yellow-600' : 'far fa-bookmark', loading && 'fa-spin']"></i>
        <span v-if="showText">
