...
import { Briefcase, Menu, Search, X } from 'lucide-vue-next';
...
<style scoped>
.homepage-navigation {
    @apply sticky top-0 z-50 w-full border-b border-border bg-white/95 shadow-sm backdrop-blur transition-all duration-300 supports-[backdrop-filter]:bg-white/90;
}
.nav-container {
...
.brand-link {
    @apply flex items-center gap-3 text-2xl font-bold text-gray-900 transition-colors duration-300 hover:text-primary;
}
.brand-icon {
...
.nav-dropdown-item {
    @apply flex items-start gap-4 rounded-lg p-3 transition-colors duration-200 hover:bg-accent;
}
.nav-dropdown-title {
...
.signup-button {
    @apply rounded-full bg-primary px-6 text-primary-foreground hover:bg-primary/90;
}
.employer-button {
    @apply gap-2 rounded-full border-primary/50 px-4 text-primary hover:border-primary hover:bg-primary/10;
}
.employer-icon {
    @apply h-4 w-4;
}
...
.mobile-nav-link {
    @apply flex items-center gap-4 rounded-lg p-4 text-lg font-medium text-foreground/80 transition-colors duration-200 hover:bg-accent hover:text-foreground;
}
.mobile-auth {
    @apply space-y-3 border-t border-border pt-6;
}
.mobile-auth-button {
    @apply w-full justify-center rounded-full py-3 text-lg;
}
