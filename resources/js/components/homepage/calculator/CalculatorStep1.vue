<template>
    <div class="calculator-step">
        <div class="mb-8">
            <h4 class="mb-2 text-2xl font-bold text-gray-900">Tell us about your current situation</h4>
            <p class="text-gray-600">We'll use this information to provide personalized career insights.</p>
        </div>

        <div class="space-y-6">
            <!-- Current Role -->
            <div>
                <label for="currentRole" class="mb-2 block text-sm font-medium text-gray-700"> What is your current role? * </label>
                <select
                    id="currentRole"
            ...
            <!-- Industry -->
            <div>
                <label for="industry" class="mb-2 block text-sm font-medium text-gray-700"> What industry do you work in? * </label>
                <select
                    id="industry"
            ...
            <!-- Experience Years -->
            <div>
                <label for="experienceYears" class="mb-2 block text-sm font-medium text-gray-700"> Years of professional experience * </label>
                <div class="relative">
                    <input
                        type="range"
                        id="experienceYears"
            ...
            <div>
                <label for="currentSalary" class="mb-2 block text-sm font-medium text-gray-700">
                    Current annual salary (optional)
                    <span class="ml-1 text-xs text-gray-500">- helps provide more accurate projections</span>
                </label>
                <div class="relative">
                    ...
                    <input
                        ...
                        @input="
                            updateField(
                                'currentSalary',
                                ($event.target as HTMLInputElement).value ? parseInt(($event.target as HTMLInputElement).value) : undefined,
                            )
                        "
                        placeholder="75000"
                        min="0"
                        step="1000"
                        class="w-full rounded-lg border border-gray-300 py-3 pl-8 pr-4 focus:border-transparent focus:ring-2 focus:ring-blue-500"
                    />
                </div>
                <p class="mt-1 text-xs text-gray-500">This information is kept confidential and used only for calculations</p>
            </div>
        </div>
...
const canProceed = computed(() => {
    return props.formData.currentRole && props.formData.industry && props.formData.experienceYears >= 0;
});
