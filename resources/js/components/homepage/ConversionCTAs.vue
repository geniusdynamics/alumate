<template>
    <div class="conversion-ctas">
        ...
        <div v-for="cta in strategicCTAsFiltered" :key="cta.id" :class="getCTAClasses(cta)">
            <component :is="getCTAComponent(cta.type)" :cta="cta" :audience="audience" @click="handleCTAClick" />
        </div>

        <!-- Simple Mobile CTA (no floating popup) -->
        <div v-if="isMobile" class="simple-mobile-cta">
            <button class="mobile-cta-button" @click="handleMobileCTA">
...
const scrollToNextSection = (currentSection: string) => {
    ...
    const currentIndex = Array.from(sections).findIndex((section) => section.getAttribute('data-section') === currentSection);
...
@media (max-width: 768px) {
    .mobile-cta {
        @apply bottom-0 left-0 right-0 rounded-none bg-blue-600 p-4 text-center text-white;
    }
}
</style>
