    ...
    <div class="career-progression-timeline">
                ...
                <div class="card-content">
                    ...
                    <div class="responsibilities">
                        ...
                        <ul class="responsibilities-list">
                            <li v-for="responsibility in careerProgression.before.responsibilities" :key="responsibility" class="responsibility-item">
                                {{ responsibility }}
                            </li>
                        </ul>
                    </div>
            ...
            <div class="comparison-card after-card">
                ...
                <div class="card-content">
                    ...
                    <div class="responsibilities">
                        <h5 class="responsibilities-title">Key Responsibilities:</h5>
                        <ul class="responsibilities-list">
                            <li v-for="responsibility in careerProgression.after.responsibilities" :key="responsibility" class="responsibility-item">
                                {{ responsibility }}
                            </li>
                        </ul>
                    </div>
        ...
        <div v-if="successMetrics.length > 0" class="success-metrics">
            ...
            <div class="metrics-grid">
                <div v-for="metric in successMetrics" :key="metric.type" class="metric-card" :class="{ 'metric-animate': animateMetrics }">
        ...
        <div v-if="linkedinUrl" class="linkedin-integration">
            ...
            <div class="linkedin-card">
                <img :src="profileImage" :alt="`${alumniName} profile photo`" class="linkedin-avatar" />
                <div class="linkedin-info">
                    <h5 class="linkedin-name">{{ alumniName }}</h5>
                    <p class="linkedin-role">{{ currentRole }}</p>
                    <p class="linkedin-company">{{ currentCompany }}</p>
                </div>
                <a :href="linkedinUrl" target="_blank" rel="noopener noreferrer" class="linkedin-button" @click="trackLinkedInClick">
        ...
        <div class="animation-controls">
            ...
            <button @click="replayAnimations" class="replay-button" :disabled="!animationsEnabled" aria-label="Replay animations">
...
const startDate = computed(() => {
    ...
    if (props.careerProgression.keyMilestones.length > 0) {
        const earliestMilestone = [...props.careerProgression.keyMilestones].sort(
...
const endDate = computed(() => {
    if (props.careerProgression.keyMilestones.length > 0) {
        const latestMilestone = [...props.careerProgression.keyMilestones].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
...
const sortedMilestones = computed(() => {
    return [...props.careerProgression.keyMilestones].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
});
...
const getMilestoneIcon = (type: string): string => {
    const icons = {
        ...
        skill_acquisition:
            'M12 3L1 9l4 2.18v6L12 21l7-3.82v-6L23 9l-11-6zM18.82 9L12 12.72 5.18 9 12 5.28 18.82 9zM17 15.99l-5 2.73-5-2.73v-3.72L12 15l5-2.73v3.72z',
...
const getMetricIcon = (type: string): string => {
    const icons = {
        ...
        network_expansion:
            'M16 4c0-1.11.89-2 2-2s2 .89 2 2-.89 2-2 2-2-.89-2-2zM4 18c0 1.11.89 2 2 2s2-.89 2-2-.89-2-2-2-2 .89-2 2zm0-12c0-1.11.89-2 2-2s2 .89 2 2-.89-2-2-2-2 .89-2 2zm12 8c0 1.11.89 2 2 2s2-.89 2-2-.89-2-2-2-2 .89-2 2zM12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2z',
    };
...
.comparison-card {
    @apply flex-1 rounded-lg border-2 bg-gray-50 p-6 transition-all duration-300;
}
...
.arrow-icon {
    @apply mb-2 h-8 w-8 text-blue-600;
...
.timeline-line {
    @apply absolute bottom-0 left-6 top-0 w-0.5 bg-gray-300;
}
...
.marker-dot {
    @apply relative z-10 flex h-12 w-12 items-center justify-center rounded-full border-4 border-white shadow-lg;
}
...
.milestone-content {
    @apply flex-1 rounded-lg border border-gray-200 bg-white p-4 shadow-sm;
}
...
.milestone-type-badge {
    @apply inline-block rounded-full px-2 py-1 text-xs font-medium text-white;
}
...
.metric-card {
    @apply rounded-lg border border-gray-200 bg-white p-6 text-center shadow-sm;
}
...
.metric-label {
    @apply mb-2 font-medium text-gray-600;
}
...
.metric-verified {
    @apply flex items-center justify-center gap-1 text-sm text-green-600;
}
...
.linkedin-card {
    @apply flex items-center gap-4 rounded-lg border border-gray-200 bg-white p-6 shadow-sm;
}
...
.linkedin-button {
    @apply flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-white transition-colors hover:bg-blue-700;
}
...
.animation-controls {
    @apply flex justify-center gap-4 border-t border-gray-200 pt-6;
}

.animation-toggle-button,
.replay-button {
    @apply flex items-center gap-2 rounded-lg border border-gray-300 px-4 py-2 text-gray-700 transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50;
}
