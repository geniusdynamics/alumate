<template>
    <div class="enterprise-metrics-visualization rounded-lg bg-white p-6 shadow-lg">
        <!-- Header -->
        <div class="mb-6">
        ...
        <div class="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            <div
                ...
                :key="metric.id"
                class="metric-card rounded-lg border border-gray-200 bg-gray-50 p-4 transition-colors duration-200 hover:border-blue-300"
            >
                <!-- Metric Header -->
                <div class="mb-3 flex items-center justify-between">
                    <div class="flex items-center">
                        <div class="mr-3 flex h-10 w-10 items-center justify-center rounded-full" :class="getMetricIconBg(metric.category)">
                            <component :is="getMetricIcon(metric.category)" class="h-5 w-5" :class="getMetricIconColor(metric.category)" />
                        </div>
                        <div>
                            <h4 class="text-sm font-semibold text-gray-900">{{ metric.name }}</h4>
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative">
                        <div class="h-2 w-full rounded-full bg-gray-200">
                            <div
                                class="h-2 rounded-full bg-gradient-to-r from-blue-500 to-green-500 transition-all duration-1000 ease-out"
                                :style="{ width: `${Math.min(100, (metric.afterValue / Math.max(metric.beforeValue, metric.afterValue)) * 100)}%` }"
                            ></div>
                        </div>
                        <div class="absolute -right-1 -top-1 h-4 w-4 rounded-full border-2 border-white bg-green-500"></div>
                    </div>
        ...
        <div v-if="showSummary" class="border-t border-gray-200 pt-6">
            <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                <div class="text-center">
                    <div class="mb-1 text-3xl font-bold text-blue-600">{{ averageImprovement }}%</div>
        ...
        <!-- ROI Highlight -->
        <div v-if="roiData" class="mt-6 rounded-lg border border-blue-200 bg-gradient-to-r from-blue-50 to-green-50 p-4">
            <div class="flex items-center justify-between">
                <div>
                    <h4 class="mb-1 font-semibold text-gray-900">Return on Investment</h4>
            ...
            <div class="mt-3 text-sm text-gray-700">
                <strong>Investment:</strong> {{ formatCurrency(roiData.investment) }} | <strong>Return:</strong> {{ formatCurrency(roiData.return) }}
            </div>
        </div>
    </div>
</template>
...
const verifiedMetrics = computed(() => props.metrics.filter((metric) => metric.verified).length);
