                ...
                <div class="min-w-0 flex-1">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-sm text-gray-600">{{ recipient.scholarship?.name }} Recipient</p>
                        </div>
                        <div class="text-right">
                            <p class="text-sm text-gray-500">
                                {{ formatDate(recipient.award_date) }}
                            </p>
                            <p class="text-sm font-medium text-green-600">${{ formatCurrency(recipient.awarded_amount) }}</p>
                        </div>
                    </div>
                    ...
                    <div v-if="recipient.thank_you_message" class="mt-4 rounded-lg bg-blue-50 p-3">
                        <p class="text-sm italic text-blue-800">"{{ recipient.thank_you_message }}"</p>
                    </div>

                    <div v-if="recipient.academic_progress || recipient.impact_metrics" class="mt-4 border-t border-gray-200 pt-4">
                    ...
                    <div v-if="recipient.updates && recipient.updates.length > 0" class="mt-4">
                        <h4 class="mb-2 text-sm font-medium text-gray-900">Recent Updates</h4>
                        <div class="space-y-2">
                            <div
                            ...
                            >
                                ...
                                <p class="mt-1 text-xs text-gray-500">{{ formatDate(update.date) }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 flex items-center justify-between">
                        ...
                        <div class="flex space-x-2">
                            <button v-if="canEdit" @click="$emit('edit', recipient)" class="text-sm font-medium text-blue-600 hover:text-blue-800">
                                Edit Story
                            </button>
                            <button @click="$emit('share', recipient)" class="text-sm font-medium text-gray-600 hover:text-gray-800">Share</button>
                        </div>
                    </div>
                </div>
            </div>
