        ...
        <div v-if="isOpen" class="color-picker-panel" @click.stop>
                ...
                <div class="contrast-checker">
                    <div v-for="(result, index) in contrastResults" :key="index" class="contrast-result">
                        ...
                        <div class="contrast-ratio">
                            <span :class="getContrastClass(result.level)"> {{ result.ratio?.toFixed(2) }}:1 </span>
            ...
            <div class="action-buttons">
                <button
                    ...
                    :disabled="!hasChanged"
                    :aria-label="`Reset ${label} color to ${originalColor}`"
                >
                    Reset
                </button>
                <button @click="applyColor" class="action-btn action-btn--primary" :aria-label="`Apply ${currentColor} color`">Apply</button>
                <button @click="cancel" class="action-btn action-btn--ghost" aria-label="Cancel color selection">Cancel</button>
            </div>
        </div>
...
const rgbToHsv = (r: number, g: number, b: number): { h: number; s: number; v: number } => {
    ...
    const v = max;
    let h = max === min ? 0 : max === r ? ((g - b) / diff) % 6 : max === g ? (b - r) / diff + 2 : (r - g) / diff + 4;
    h = (h * 60 + 360) % 360;
...
.color-preview {
    @apply flex items-center gap-2 rounded-lg border border-gray-300 px-3 py-2 dark:border-gray-600;
    @apply cursor-pointer bg-white transition-colors dark:bg-gray-700;
    @apply focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 dark:focus:ring-blue-400;
}
...
.color-swatch {
    @apply h-6 w-6 rounded border border-gray-200 dark:border-gray-600;
    @apply shadow-sm;
}

.color-value {
    @apply min-w-[80px] font-mono text-sm text-gray-900 dark:text-gray-100;
}
...
/* Color Picker Panel */
.color-picker-panel {
    @apply absolute left-0 top-full mt-2 w-80 rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800;
    @apply z-50 p-4 shadow-xl;
}

/* Color Mode Tabs */
.color-mode-tabs {
    @apply mb-4 flex border-b border-gray-200 dark:border-gray-700;
}

.mode-tab {
    @apply flex-1 rounded-t-lg px-3 py-2 text-sm font-medium transition-colors;
    @apply text-gray-600 focus:ring-2 focus:ring-inset focus:ring-blue-500 dark:text-gray-400;
    @apply focus:outline-none;
}
...
.color-map {
    @apply relative h-32 w-full rounded-lg border border-gray-200 dark:border-gray-600;
    @apply cursor-crosshair overflow-hidden;
}

.color-map-canvas {
    @apply h-full w-full cursor-crosshair;
    @apply focus:outline-none;
}

.color-map-indicator {
    @apply absolute h-3 w-3 rounded-full border-2 border-white shadow-lg;
    @apply pointer-events-none -translate-x-1/2 -translate-y-1/2 transform;
}

.hue-slider {
    @apply relative h-32 w-5 overflow-hidden rounded-lg border border-gray-200 dark:border-gray-600;
    @apply cursor-ns-resize;
}

.hue-slider-canvas {
    @apply h-full w-full cursor-ns-resize;
    @apply focus:outline-none;
}

.hue-slider-indicator {
    @apply pointer-events-none absolute left-0 h-1 w-full bg-white shadow-lg;
...
.input-label {
    @apply w-12 text-sm font-medium text-gray-700 dark:text-gray-300;
}
...
.color-number-input {
    @apply w-16 border border-gray-300 px-2 py-1 text-sm dark:border-gray-600;
    @apply rounded bg-white focus:ring-2 focus:ring-inset focus:ring-blue-500 dark:bg-gray-700;
    @apply focus:outline-none;
}
...
.hex-input {
    @apply flex-1 border border-gray-300 px-3 py-2 font-mono text-sm dark:border-gray-600;
    @apply rounded-lg bg-white focus:ring-2 focus:ring-inset focus:ring-blue-500 dark:bg-gray-700;
    @apply focus:outline-none;
    @apply uppercase;
}
...
.format-btn {
    @apply flex-1 rounded-md px-2 py-1 text-xs font-medium transition-colors;
...
.recent-color {
    @apply h-6 w-6 rounded border border-gray-200 dark:border-gray-600;
    @apply transition-all duration-200 focus:ring-2 focus:ring-blue-500 focus:ring-offset-1;
    @apply hover:scale-110 hover:transform focus:outline-none;
}
...
/* Accessibility Panel */
.accessibility-panel {
    @apply rounded-lg bg-blue-50 p-3 dark:bg-blue-900/20;
}
...
.accessibility-toggle {
    @apply rounded p-1 text-blue-700 hover:bg-blue-100 dark:text-blue-300 dark:hover:bg-blue-800/50;
}
...
.contrast-result {
    @apply flex items-center justify-between rounded bg-white p-2 dark:bg-gray-800;
...
.contrast-swatch {
    @apply h-4 w-4 rounded border border-gray-300 dark:border-gray-600;
}
...
.accessibility-suggestions {
    @apply border-t border-blue-200 pt-3 dark:border-blue-800;
}

.accessibility-suggestions h5 {
    @apply mb-2 text-sm font-medium text-blue-900 dark:text-blue-100;
}
...
/* Action Buttons */
.action-buttons {
    @apply flex items-center gap-2 border-t border-gray-200 pt-4 dark:border-gray-700;
}

.action-btn {
    @apply flex-1 rounded-lg px-4 py-2 text-sm font-medium transition-colors;
...
.action-btn--ghost {
    @apply bg-transparent text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700;
}

/* Accessibility Icon */
.accessibility-icon {
    @apply absolute right-2 top-2 p-1 text-gray-600 dark:text-gray-400;
    @apply rounded transition-colors hover:bg-gray-100 dark:hover:bg-gray-700;
...
@media (prefers-reduced-motion: reduce) {
    ...
    .recent-color:hover,
    .color-preview:hover {
        @apply scale-100 transform-none;
    }
}
</style>
