<template>
  <div class="custom-code-panel h-full flex flex-col bg-white border-l border-gray-200">
    <!-- Header -->
    <div class="flex-shrink-0 p-4 border-b border-gray-200">
      ...
      <div class="mt-4">
        <nav class="flex space-x-1" aria-label="Code Types">
          ...
          >
            <component :is="type.icon" class="w-4 h-4 mr-2 inline" />
            {{ type.label }}
          </button>
        </nav>
      </div>
    ...
    <div class="flex-1 overflow-y-auto">
      <div class="p-4 space-y-3">
          ...
          <div class="flex items-start justify-between">
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center mt-2 space-x-2">
                <span :class="[
                  code.type === 'html' ? 'bg-orange-100 text-orange-800' :
                  code.type === 'css' ? 'bg-blue-100 text-blue-800' :
                ...
                <span v-if="code.isDraft" class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-yellow-100 text-yellow-800">
                  Draft
                </span>
              </div>
            </div>
        ...
        <div v-if="filteredCodes.length === 0" class="text-center py-8">
          ...
          <p class="mt-1 text-sm text-gray-500">
            Get started by creating your first {{ activeCodeType }} code snippet.
          </p>
          <div class="mt-6">
            <button
              @click="createNewCode"
    ...
    <!-- Validation Results -->
    <ValidationPanel
      v-if="selectedCode && validationResults"
...
const handleFixIssue = async (issue: any) => {
  ...
  try {
    ...
    if (lintResult.fixedCode && lintResult.fixedCode !== selectedCode.value.code) {
      ...
      // Re-validate
      const validationResult = await customCodeValidationService.validateCode(
        updatedCode.code,
        updatedCode.type
      )
      validationResults.value = validationResult
...
defineExpose({
  ...
  getCodesByType: (type: 'html' | 'css' | 'javascript') => 
    customCodes.value.filter(c => c.type === type && c.isActive)
})
</script>