  ...
  <div class="responsive-design-tools">
    ...
    <div class="device-switcher">
      ...
      <div class="custom-device">
        <button
          @click="showCustomDevice = !showCustomDevice"
          :class="['custom-device-btn', { 'active': showCustomDevice }]"
        >
        ...
        <div v-if="showCustomDevice" class="custom-device-controls">
          ...
          <button @click="applyCustomDevice" class="apply-custom-btn">
            Apply
          </button>
        </div>
      </div>
    ...
    <div class="responsive-utilities">
      ...
      <div class="utility-buttons">
        <button
          ...
          :class="['utility-btn', { 'active': responsiveMode }]"
          title="Toggle responsive editing mode"
        >
        ...
        <button
          @click="copyResponsiveStyles"
          class="utility-btn"
          title="Copy styles to other breakpoints"
        >
        ...
        <button
          @click="resetResponsiveStyles"
          class="utility-btn danger"
          title="Reset responsive styles"
        >
    ...
    <div v-if="responsiveMode && selectedComponent" class="responsive-style-editor">
      <h4 class="editor-title">
        Responsive Styles for {{ selectedComponent.getName() }}
      </h4>
      ...
      <div class="responsive-actions">
        <button @click="applyResponsiveStyles" class="apply-styles-btn">
          Apply Styles
        </button>
        <button @click="previewResponsiveStyles" class="preview-styles-btn">
          Preview
        </button>
      </div>
    </div>
        ...
        <div class="breakpoint-guide">
          ...
          <ul>
            <li v-for="bp in tailwindBreakpoints" :key="bp.name">
              <code>{{ bp.name }}</code>: {{ bp.minWidth }}px and up
            </li>
          </ul>
        </div>
      </div>
...
const currentDevice = computed(() => 
  devices.find(d => d.id === selectedDevice.value) || devices[0]
)
...
.responsive-design-tools {
  @apply flex flex-col space-y-6 p-4 bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700;
}
...
.device-btn {
  @apply w-full flex items-center justify-between p-3 text-left text-sm border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors;
}
...
.custom-device-btn {
  @apply w-full flex items-center justify-center p-2 text-sm text-gray-600 dark:text-gray-300 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700;
}
...
.size-input {
  @apply w-full px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white;
}
...
.apply-custom-btn {
  @apply w-full px-3 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg;
}
...
.breakpoint-item {
  @apply flex items-center justify-between p-2 text-sm cursor-pointer rounded hover:bg-gray-50 dark:hover:bg-gray-700;
}
...
.utility-btn {
  @apply flex items-center justify-center p-2 text-xs font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded transition-colors;
}
...
.responsive-style-editor {
  @apply space-y-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg;
}
...
.responsive-tab {
  @apply flex items-center px-3 py-2 text-xs font-medium text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-t-lg hover:bg-gray-50 dark:hover:bg-gray-700;
}

.responsive-tab.active {
  @apply bg-blue-50 dark:bg-blue-900 text-blue-700 dark:text-blue-300 border-blue-300 dark:border-blue-600;
}
...
.property-input {
  @apply flex-1 px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded-l focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white;
}

.property-unit {
  @apply px-2 py-1 text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-600 border border-l-0 border-gray-300 dark:border-gray-600 rounded-r;
}
...
.apply-styles-btn {
  @apply flex-1 px-3 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded;
}

.preview-styles-btn {
  @apply flex-1 px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 rounded;
}
