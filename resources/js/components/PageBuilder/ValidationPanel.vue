<template>
  <div class="validation-panel bg-white border-t border-gray-200 p-4">
    ...
    <div class="mb-4">
      <div class="flex items-center space-x-4 text-sm">
        <div :class="[
          ...
          <component :is="results.isValid ? CheckCircleIcon : XCircleIcon" class="w-4 h-4 mr-1" />
    ...
    <div class="border-b border-gray-200 mb-4">
      <nav class="-mb-px flex space-x-8">
        ...
        >
          <component :is="tab.icon" class="w-4 h-4 mr-1 inline" />
          {{ tab.label }}
    ...
    <div class="max-h-64 overflow-y-auto">
      ...
      <div v-if="activeTab === 'errors'" class="space-y-2">
        ...
        >
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <div class="flex items-center text-red-800 text-sm font-medium">
            ...
            <button
              @click="goToLine(error.line, error.column)"
        ...
        <div v-if="results.errors.length === 0" class="text-center py-4 text-gray-500 text-sm">
          No errors found
        </div>
      </div>
      ...
      <div v-if="activeTab === 'warnings'" class="space-y-2">
        ...
        >
          <div class="flex items-start justify-between">
            <div class="flex-1">
              ...
              <p class="text-yellow-700 text-sm mt-1">{{ warning.message }}</p>
            ...
            <div class="flex items-center space-x-2">
              <button
                @click="fixIssue(warning)"
                class="text-yellow-600 hover:text-yellow-800 text-xs"
              >
                Fix
              </button>
              <button
                @click="goToLine(warning.line, warning.column)"
                class="text-yellow-600 hover:text-yellow-800 text-xs"
              >
                Go to line
              </button>
        ...
        <div v-if="results.warnings.length === 0" class="text-center py-4 text-gray-500 text-sm">
          No warnings found
        </div>
      </div>
      ...
      <div v-if="activeTab === 'security'" class="space-y-2">
        <div
          v-for="issue in results.securityIssues"
          :key="issue.message"
          class="p-3 bg-red-50 border border-red-200 rounded-lg"
        >
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <div class="flex items-center text-red-800 text-sm font-medium">
                ...
                <span :class="[
                  ...
                  'text-gray-700',
                  'uppercase text-xs font-bold mr-2'
                ]">
                  {{ issue.severity }}
                </span>
                <span v-if="issue.line">Line {{ issue.line }}</span>
              </div>
              <p class="text-red-700 text-sm mt-1">{{ issue.message }}</p>
              <div v-if="issue.remediation" class="text-red-600 text-xs mt-2 p-2 bg-red-100 rounded">
                <strong>Remediation:</strong> {{ issue.remediation }}
              </div>
              <div v-if="issue.cwe" class="text-red-600 text-xs mt-1">
                CWE: {{ issue.cwe }}
              </div>
            </div>
            <button
        ...
        <div v-if="results.securityIssues.length === 0" class="text-center py-4 text-gray-500 text-sm">
      ...
      <div v-if="activeTab === 'performance'" class="space-y-2">
        <div
          v-for="issue in results.performanceIssues"
          :key="issue.message"
          class="p-3 bg-orange-50 border border-orange-200 rounded-lg"
        >
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <div class="flex items-center text-orange-800 text-sm font-medium">
              ...
              <p class="text-orange-700 text-sm mt-1">{{ issue.message }}</p>
              <div v-if="issue.recommendation" class="text-orange-600 text-xs mt-2 p-2 bg-orange-100 rounded">
        ...
        <div v-if="results.performanceIssues.length === 0" class="text-center py-4 text-gray-500 text-sm">
      ...
      <div v-if="activeTab === 'info'" class="space-y-2">
        <div
          v-for="info in results.info"
          :key="`info-${info.line}-${info.column}`"
        ...
        >
          <div class="flex items-start justify-between">
            <div class="flex-1">
              ...
              <p class="text-blue-700 text-sm mt-1">{{ info.message }}</p>
            ...
            <button
              @click="goToLine(info.line, info.column)"
        ...
        <div v-if="results.info.length === 0" class="text-center py-4 text-gray-500 text-sm">
          No info messages
        </div>
      </div>
    </div>
    ...
    <div v-if="codeAnalysis" class="mt-4 pt-4 border-t border-gray-200">
      <h5 class="text-sm font-medium text-gray-900 mb-2">Code Analysis</h5>
      <div class="grid grid-cols-2 gap-4 text-sm">
        <div>
          <span class="text-gray-600">Complexity:</span>
          <span :class="[
            codeAnalysis.complexity > 10 ? 'text-red-600' :
            codeAnalysis.complexity > 5 ? 'text-yellow-600' :
            'text-green-600',
            'ml-1 font-medium'
          ]">
        ...
        <div>
          ...
          <span :class="[
            codeAnalysis.maintainability < 50 ? 'text-red-600' :
            codeAnalysis.maintainability < 75 ? 'text-yellow-600' :
            'text-green-600',
            'ml-1 font-medium'
          ]">
      ...
      <div v-if="codeAnalysis.suggestions.length > 0" class="mt-2">
        <h6 class="text-xs font-medium text-gray-700 mb-1">Suggestions:</h6>
...
const canAutoFix = computed(() => {
  return props.results.warnings.some(w => 
    w.code === 'MISSING_SEMICOLON' || 
    w.code === 'UNQUOTED_ATTRIBUTE'
  )
})
...
const autoFix = async () => {
  ...
  try {
    const lintResult = await customCodeValidationService.lintCode(
      props.code.code,
      props.code.type
    )
...
const loadCodeAnalysis = async () => {
  try {
    const analysis = await customCodeValidationService.analyzeCode(
      props.code.code,
      props.code.type
    )
