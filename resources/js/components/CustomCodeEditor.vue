  ...
  <div class="custom-code-editor" :class="{ 'loading': isLoading, 'error': hasErrors }">
    ...
    <div class="editor-header">
      ...
      <div class="editor-actions">
        ...
        >
          <svg v-if="isValidating" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="editor-container">
      ...
      <div class="line-numbers">
        <div
          ...
          :key="line"
          class="line-number"
          :class="{ 'error-line': hasErrorOnLine(line) }"
        >
          {{ line }}
        </div>
      </div>
    ...
    <div v-if="validationErrors.length > 0" class="validation-errors">
      ...
      <div class="errors-list">
        <div
          ...
          class="error-item"
          @click="focusErrorLine(error.line)"
        >
...
const PaletteIcon = () => h('svg', { class: 'w-4 h-4', fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' }, [
...
const ZapIcon = () => h('svg', { class: 'w-4 h-4', fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' }, [
...
<style scoped>
.custom-code-editor {
  @apply bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 flex flex-col h-full;
}
...
.code-editor {
  @apply w-full h-full p-4 text-sm font-mono resize-none border-0;
...
.syntax-highlight-overlay {
  @apply absolute inset-0 p-4 text-sm font-mono pointer-events-none;
...
.line-numbers {
  @apply bg-gray-100 dark:bg-gray-700 border-r border-gray-200 dark:border-gray-600 px-2 py-4;
  @apply text-gray-500 dark:text-gray-400 text-sm font-mono select-none;
}

.line-number {
  @apply text-right pr-2 leading-5;
}

.line-number.error-line {
  @apply text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900;
}

/* Validation Errors */
.validation-errors {
  @apply border-t border-gray-200 dark:border-gray-700 bg-red-50 dark:bg-red-900;
}

.errors-header {
  @apply px-6 py-3 border-b border-red-200 dark:border-red-800 flex items-center justify-between;
}
...
.clear-errors-btn {
  @apply p-1 text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-200;
}
...
.error-item {
  @apply flex items-start gap-3 px-6 py-2 hover:bg-red-100 dark:hover:bg-red-800 cursor-pointer;
}
...
.error-message {
  @apply text-sm text-red-800 dark:text-red-200 font-medium;
}

.error-location {
  @apply text-xs text-red-600 dark:text-red-300 mt-1;
}
...
.preview-modal {
  @apply bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-4xl w-full mx-4 max-h-[80vh] flex flex-col;
}
...
.preview-iframe {
  @apply w-full h-full border border-gray-200 dark:border-gray-700 rounded-lg;
}
