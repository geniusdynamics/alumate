<template>
    <div class="reunion-photo-gallery">
        ...
        <!-- Photo grid -->
        <div v-else-if="photos.length > 0" class="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
            <div v-for="photo in photos" :key="photo.id" class="group relative cursor-pointer" @click="openLightbox(photo)">
                ...
                <!-- Overlay with info -->
                <div
                    class="absolute inset-0 flex items-end rounded-lg bg-black bg-opacity-0 transition-opacity duration-200 group-hover:bg-opacity-40"
                >
                    <div class="p-3 text-white opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                        <p v-if="photo.title" class="truncate text-sm font-medium">{{ photo.title }}</p>
                ...
                <!-- Featured badge -->
                <div v-if="photo.is_featured" class="absolute right-2 top-2">
                    <div class="rounded-full bg-yellow-400 px-2 py-1 text-xs font-medium text-yellow-900">Featured</div>
                </div>
            </div>
        </div>
        ...
        <div v-else class="py-12 text-center">
            ...
            <p class="mb-4 text-gray-600">Be the first to share memories from this reunion!</p>
            <button v-if="canUpload" @click="showUploadModal = true" class="rounded-md bg-blue-600 px-4 py-2 text-white hover:bg-blue-700">
                Upload First Photo
            </button>
        </div>

        <!-- Upload Modal -->
        <PhotoUploadModal v-if="showUploadModal" :event-id="eventId" @close="showUploadModal = false" @uploaded="handlePhotoUploaded" />
