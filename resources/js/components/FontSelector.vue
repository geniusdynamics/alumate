<template>
    <div class="font-selector" :class="{ 'font-selector--disabled': disabled }">
        ...
        <div class="font-input-group">
            ...
            >
                <div class="font-sample" :style="{ fontFamily: currentFont.family }">Aa</div>
                <div class="font-info">
        ...
        <div v-if="isOpen" class="font-selector-panel" @click.stop>
            ...
            <div class="action-buttons">
                <button
                    ...
                    :disabled="!hasChanged"
                    :aria-label="`Reset ${label} to original font`"
                >
                    Reset
                </button>
                <button @click="applyFont" class="action-btn action-btn--primary" :aria-label="`Apply ${currentFont.family} font`">Apply</button>
                <button @click="cancel" class="action-btn action-btn--ghost" aria-label="Cancel font selection">Cancel</button>
            </div>
        </div>
...
.font-sample {
    @apply flex h-8 w-8 items-center justify-center rounded bg-gray-100 text-lg dark:bg-gray-600;
    @apply font-bold leading-none text-gray-900 dark:text-gray-100;
}
...
.font-name {
    @apply truncate text-sm font-medium text-gray-900 dark:text-gray-100;
}
...
/* Font Selector Panel */
.font-selector-panel {
    @apply absolute left-0 top-full mt-2 w-96 rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800;
...
.font-search-input {
    @apply w-full border border-gray-300 px-3 py-2 text-sm dark:border-gray-600;
    @apply rounded-lg bg-white focus:ring-2 focus:ring-inset focus:ring-blue-500 dark:bg-gray-700;
    @apply pr-8 focus:outline-none;
}

.clear-search-btn {
    @apply absolute right-2 top-1/2 -translate-y-1/2 transform p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300;
...
.filter-select {
    @apply border border-gray-300 px-3 py-1 text-sm dark:border-gray-600;
...
/* Font List */
.font-list {
    @apply mb-4 max-h-64 overflow-y-auto;
}
...
.loading-spinner {
    @apply mb-3 h-6 w-6 animate-spin rounded-full border-2 border-blue-600 border-t-transparent;
}

.no-fonts-icon {
    @apply mb-3 h-8 w-8 text-gray-400;
}
...
.font-item {
    @apply w-full rounded-lg border border-gray-200 px-3 py-3 transition-colors dark:border-gray-600;
...
.font-sample {
    @apply mb-1 text-lg leading-tight text-gray-900 dark:text-gray-100;
}
...
/* Font Controls */
.font-controls {
    @apply mb-4 space-y-3 border-t border-gray-200 pt-4 dark:border-gray-700;
}
...
.control-label {
    @apply w-16 text-sm font-medium text-gray-700 dark:text-gray-300;
}

.font-weight-select,
.font-size-input {
    @apply border border-gray-300 px-2 py-1 text-sm dark:border-gray-600;
    @apply rounded bg-white focus:ring-2 focus:ring-inset focus:ring-blue-500 dark:bg-gray-700;
    @apply focus:outline-none;
}
...
.loading-item span {
    @apply flex-1 text-xs text-gray-600 dark:text-gray-400;
}

.loading-bar {
    @apply h-1 flex-1 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700;
}
...
.loading-dot {
    @apply h-2 w-2 animate-pulse rounded-full bg-blue-600;
}
...
@media (prefers-color-scheme: dark) {
    ...
    .font-item {
        @apply border-gray-600 bg-gray-800 hover:bg-gray-700;
    }
