    ...
    <div class="touch-optimized-controls">
        ...
        >
            <div class="touch-button-content">
                <component v-if="loading" :is="LoadingIcon" class="touch-button-icon animate-spin" aria-hidden="true" />
                <component v-else-if="icon" :is="icon" class="touch-button-icon" aria-hidden="true" />
        ...
        <div v-else-if="type === 'input'" class="touch-input-container" :class="{ 'touch-input-focused': isFocused, 'touch-input-error': error }">
            ...
            <div class="touch-input-wrapper">
                <component v-if="inputIcon" :is="inputIcon" class="touch-input-icon" aria-hidden="true" />
        ...
        <div v-else-if="type === 'select'" class="touch-select-container" :class="{ 'touch-select-error': error }">
            ...
            <div class="touch-select-wrapper" @click="toggleSelect">
                ...
                >
                    ...
                    <ChevronDownIcon class="touch-select-icon" :class="{ 'rotate-180': isSelectOpen }" />
                </div>

                <div v-if="isSelectOpen" class="touch-select-dropdown" role="listbox">
                    <div
                    ...
                    >
                        ...
                        <CheckIcon v-if="option.value === modelValue" class="h-5 w-5 text-blue-600 dark:text-blue-400" />
                    </div>
                </div>
            </div>
        ...
        <div v-else-if="type === 'toggle'" class="touch-toggle-container">
        ...
        <div v-else-if="type === 'slider'" class="touch-slider-container">
            ...
            >
                <div class="touch-slider-fill" :style="{ width: `${sliderPercentage}%` }"></div>
                <div
...
/* Touch Button Styles */
.touch-button {
    @apply relative inline-flex items-center justify-center overflow-hidden rounded-lg font-medium transition-all duration-200;
...
.touch-button-badge {
    @apply ml-2 rounded-full bg-red-500 px-2 py-0.5 text-xs text-white;
}

.touch-ripple {
    @apply pointer-events-none absolute rounded-full bg-white bg-opacity-30;
    transform: scale(0);
...
.touch-input {
    @apply w-full rounded-lg border border-gray-300 px-4 py-3 text-base dark:border-gray-600;
...
.touch-input-clear {
    @apply absolute right-3 top-1/2 -translate-y-1/2 transform p-1 text-gray-400 hover:text-gray-600;
    @apply rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 dark:hover:text-gray-300;
}
...
.touch-select {
    @apply w-full rounded-lg border border-gray-300 px-4 py-3 text-base dark:border-gray-600;
    @apply cursor-pointer bg-white text-gray-900 dark:bg-gray-800 dark:text-white;
    @apply focus:border-transparent focus:ring-2 focus:ring-blue-500;
...
.touch-select-dropdown {
    @apply absolute left-0 right-0 top-full mt-1 border border-gray-300 bg-white dark:border-gray-600 dark:bg-gray-800;
    @apply z-50 max-h-60 overflow-y-auto rounded-lg shadow-lg;
}

.touch-select-option {
    @apply cursor-pointer px-4 py-3 text-base hover:bg-gray-50 dark:hover:bg-gray-700;
    @apply flex items-center justify-between;
    min-height: 44px;
}
...
.touch-toggle {
    ...
    @apply bg-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:bg-gray-700;
    min-height: 44px;
    min-width: 44px;
    padding: 9px;
}
...
.touch-slider-label {
    @apply mb-2 flex items-center justify-between text-sm font-medium text-gray-700 dark:text-gray-300;
}
...
.touch-slider-track {
    @apply relative h-2 cursor-pointer rounded-full bg-gray-200 dark:bg-gray-700;
    min-height: 44px;
    padding: 21px 0;
}

.touch-slider-fill {
    @apply absolute left-0 top-1/2 h-2 -translate-y-1/2 transform rounded-full bg-blue-600 dark:bg-blue-500;
}

.touch-slider-thumb {
    @apply absolute top-1/2 h-6 w-6 rounded-full border-2 border-blue-600 bg-white dark:border-blue-500;
    @apply -translate-x-1/2 -translate-y-1/2 transform cursor-grab shadow-md;
