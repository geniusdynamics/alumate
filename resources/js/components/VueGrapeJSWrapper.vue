  ...
  <div class="grapejs-wrapper" :class="{ 'loading': isLoading, 'error': isError }">
    ...
    <div class="grapejs-toolbar" v-if="!isLoading">
      <div class="toolbar-left">
        ...
        <div class="action-buttons">
          ...
          >
            <svg v-if="isPublishing" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" class="opacity-25"></circle>
              <path fill="currentColor" class="opacity-75" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <svg v-else class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="toolbar-right">
        ...
        <div class="export-backup-actions">
          <button
            ...
            :title="'Export page to various formats'"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            class="backup-btn"
            :aria-label="'Backup page'"
            :title="'Create or restore page backup'"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grapejs-editor-container" :class="{ 'panels-hidden': !showComponentPanel && !showStylePanel && !showLayerPanel }">
      ...
      <div v-if="showStylePanel" class="style-panel">
        ...
        <div class="style-panel-tabs">
          <button
            @click="activeStyleTab = 'visual'"
            :class="['style-tab', { 'active': activeStyleTab === 'visual' }]"
          >
            Visual
          </button>
          <button
            @click="activeStyleTab = 'css'"
            :class="['style-tab', { 'active': activeStyleTab === 'css' }]"
          >
            CSS Editor
          </button>
        </div>
        ...
        <div v-if="activeStyleTab === 'css'" class="css-editor-container">
          ...
          <div class="css-actions">
            <button @click="applyCustomCSS" class="apply-css-btn" :disabled="cssValidationErrors.length > 0">
              Apply CSS
            </button>
            <button @click="formatCSS" class="format-css-btn">
              Format
            </button>
          </div>
        </div>
      ...
      <div class="settings-panel">
        ...
        <div class="settings-content">
          ...
          <div class="setting-group">
            ...
            <label class="setting-label">
              <input
                v-model="settings.showDevicePreview"
                type="checkbox"
                class="setting-checkbox"
              />
      ...
      <div class="preview-content">
        ...
        <div class="preview-controls">
          ...
          <div class="interaction-controls">
            <label class="control-label">
              <input
                v-model="mockInteractions"
                type="checkbox"
                @change="toggleMockInteractions"
                class="interaction-checkbox"
              />
              Enable Interactions
            </label>
            <button @click="clearInteractionLogs" class="clear-logs-btn">
              Clear Logs
            </button>
          </div>
        </div>
        ...
        <div class="interaction-logs">
          ...
          <div class="logs-container">
            <div v-if="interactionLogs.length === 0" class="no-logs">
      ...
      <div class="analytics-content">
        ...
        <div v-else-if="analyticsData" class="analytics-metrics">
          ...
          <div class="analytics-actions">
            <button @click="loadAnalyticsData" class="refresh-analytics-btn">
              Refresh Data
            </button>
          </div>
        </div>
        <div v-else class="no-analytics">
          ...
          <button @click="loadAnalyticsData" class="load-analytics-btn">
            Load Analytics
          </button>
        </div>
      </div>
      ...
      <div class="ab-testing-content">
        ...
        <div class="experiment-list">
          <div class="experiment-header">
            <h4>Experiments</h4>
            <button @click="isCreatingExperiment = true" class="create-experiment-btn">
              Create Experiment
            </button>
          </div>
        ...
        <div v-if="selectedExperiment" class="experiment-details">
          <div class="experiment-header">
            ...
            <div class="experiment-actions">
              <button @click="startExperiment" v-if="selectedExperiment.status === 'draft'" class="action-btn start-btn">
                Start
              </button>
              <button @click="pauseExperiment" v-if="selectedExperiment.status === 'running'" class="action-btn pause-btn">
                Pause
              </button>
              <button @click="endExperiment" v-if="selectedExperiment.status === 'running'" class="action-btn end-btn">
                End
              </button>
            </div>
          </div>
            ...
            <form @submit.prevent="createExperiment">
              ...
              <div class="form-group">
                ...
                <textarea
                  ...
                  v-model="newExperimentHypothesis"
                  required
                  class="form-textarea"
                ></textarea>
              </div>
              ...
              <div class="form-actions">
                <button type="button" @click="isCreatingExperiment = false" class="cancel-btn">
                  Cancel
                </button>
      ...
      <div class="dialog-panel">
        ...
        <div class="dialog-content">
          ...
          <div class="dialog-actions">
            <button @click="showPublishDialog = false" class="dialog-btn cancel-btn">
              Cancel
            </button>
        ...
        <div class="dialog-content">
          ...
          <div class="form-group">
            <label class="form-checkbox">
              <input
                v-model="exportOptions.includeAssets"
                type="checkbox"
          ...
          <div class="dialog-actions">
            <button @click="showExportDialog = false" class="dialog-btn cancel-btn">
              Cancel
            </button>
      ...
      <div class="dialog-panel">
        ...
        <div class="dialog-content">
          ...
          <div class="form-group">
            ...
            <input
              id="retention-days"
              v-model.number="backupOptions.retentionDays"
              type="number"
              min="1"
              max="365"
              class="form-input"
          ...
          <div class="form-group">
            <label class="form-checkbox">
              <input
                v-model="backupOptions.includeAssets"
                type="checkbox"
                class="form-checkbox-input"
              />
              Include Assets
          ...
          <div class="dialog-actions">
            <button @click="showBackupDialog = false" class="dialog-btn cancel-btn">
              Cancel
            </button>
      ...
      <div class="dialog-panel">
        ...
        <div class="dialog-content">
          ...
          <div class="form-group">
            <label class="form-checkbox">
              <input
                v-model="migrationConfig.includeAssets"
                type="checkbox"
                class="form-checkbox-input"
              />
              Include Assets
          ...
          <div class="form-group">
            <label class="form-checkbox">
              <input
                v-model="migrationConfig.validateBeforeMigration"
                type="checkbox"
                class="form-checkbox-input"
              />
          ...
          <div class="dialog-actions">
            <button @click="showMigrationDialog = false" class="dialog-btn cancel-btn">
              Cancel
            </button>
    ...
    <div v-if="isError" class="error-overlay">
      <div class="error-content">
        ...
        <button @click="resetEditor" class="error-btn">
          Try Again
        </button>
      </div>
    </div>
...
import { analyticsService, type AnalyticsIntegrationService } from '@/services/AnalyticsIntegrationService'
...
import { pageMigrationService, type MigrationConfig, type Migration } from '@/services/PageMigrationService'
import type {
  Component,
  Page,
  GrapeJSConfig,
...
// Watchers
watch(() => props.pageId, async (newPageId: string | undefined, oldPageId: string | undefined) => {
...
const setupEditorEvents = (): void => {
  ...
  editor.value.on('component:selected', (component: any) => {
    ...
    if (componentId) {
      selectedComponent.value = componentLibraryBridge.getRegisteredComponents()
        .find(c => c.blockDefinition.attributes?.['data-component-id'] === componentId) as any

      // Track component selection
      analyticsService.trackElementInteraction(componentId, 'select', {
  ...
  editor.value.on('component:add', (component: any) => {
    ...
    analyticsService.trackEvent('component_added', {
      componentId: component.getId(),
      componentType: component.get('type'),
      componentTag: component.get('tagName'),
      pageId: props.pageId
...
const handleExport = async (): Promise<void> => {
  ...
  try {
    ...
    exportResult.value = await pageExportService.exportPage(
      props.pageId,
      exportOptions.value,
      editorData,
      props.tenantId
...
const handleBackup = async (): Promise<void> => {
  ...
  try {
    ...
    const editorData = getCurrentEditorData()
    backupResult.value = await pageBackupService.createBackup(
      props.pageId,
      backupOptions.value,
      editorData,
      props.tenantId
...
const handleMigration = async (): Promise<void> => {
  ...
  try {
    ...
    const editorData = getCurrentEditorData()
    migrationResult.value = await pageMigrationService.createMigration(
      props.pageId,
...
const restoreBackup = async (backupId: string): Promise<void> => {
  ...
  try {
    const result = await pageBackupService.restoreBackup(
      backupId,
      props.pageId,
      props.tenantId
    )
...
.settings-panel {
  @apply bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full mx-4;
}
...
/* Error Overlay */
.error-overlay {
  @apply absolute inset-0 bg-red-50 dark:bg-red-900 bg-opacity-75 flex items-center justify-center z-40;
}

.error-content {
  @apply bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4 text-center;
}
...
@media (max-width: 768px) {
  ...
  .style-panel,
  .layer-panel {
    @apply w-full absolute inset-y-0 right-0 z-30;
  }
...
.block-search-input {
  @apply w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-md;
...
.style-tab {
  @apply flex-1 px-4 py-2 text-sm font-medium text-center transition-colors;
...
.css-editor {
  @apply flex-1 p-3 text-sm font-mono border-0 resize-none;
...
.css-errors {
  @apply p-2 bg-red-50 dark:bg-red-900 border-t border-red-200 dark:border-red-800;
}

.css-error {
  @apply text-sm text-red-700 dark:text-red-300 mb-1;
}

.css-actions {
  @apply flex gap-2 p-3 border-t border-gray-200 dark:border-gray-700;
}
...
/* Preview Panel */
.preview-panel {
  @apply fixed top-0 right-0 h-full w-96 bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 z-40;
...
.preview-controls {
  @apply p-4 border-b border-gray-200 dark:border-gray-700 space-y-4;
}
...
.device-selector .device-btn {
  @apply px-3 py-2 text-sm font-medium rounded-lg border border-gray-300 dark:border-gray-600;
...
.clear-logs-btn {
  @apply px-3 py-1 text-sm bg-gray-200 text-gray-900 rounded hover:bg-gray-300;
  @apply dark:bg-gray-700 dark:text-white dark:hover:bg-gray-600 transition-colors;
}

.preview-container {
  @apply flex-1 p-4 bg-gray-50 dark:bg-gray-900;
}
...
.preview-iframe-container {
  @apply w-full h-full bg-white rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700;
}
...
.log-entry {
  ...
  @apply bg-gray-50 dark:bg-gray-700 px-2 py-1 rounded;
}
...
.retry-btn {
  @apply mt-2 px-3 py-1 text-sm bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors;
}
...
.load-analytics-btn {
  @apply w-full px-4 py-2 text-sm font-medium rounded-lg transition-colors;
  @apply focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 dark:focus:ring-offset-gray-800;
  @apply focus:outline-none;
}
...
.error-message {
  @apply mb-4 p-3 bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-800 rounded-lg;
}
...
.create-experiment-btn {
  ...
  @apply focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 dark:focus:ring-offset-gray-800;
  @apply focus:outline-none text-sm font-medium;
}
...
.experiment-metrics {
  @apply flex gap-4 text-sm text-gray-600 dark:text-gray-400 mt-2;
}
...
.hypothesis {
  @apply text-gray-700 dark:text-gray-300 mb-3 italic;
}
...
.modal-content {
  @apply bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full mx-4 p-6;
}
...
.cancel-btn {
  ...
  @apply bg-gray-200 dark:bg-gray-700 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600;
  @apply transition-colors;
}

.create-btn {
  @apply px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700;
  @apply transition-colors;
}
