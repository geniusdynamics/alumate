<template>
    <Teleport to="body">
        <div class="share-preview-modal fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-labelledby="share-modal-title" aria-modal="true">
            <!-- Backdrop -->
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative w-full max-w-2xl rounded-lg bg-white shadow-xl dark:bg-gray-800" @click.stop>
                    <!-- Modal Header -->
                    <header class="flex items-center justify-between border-b border-gray-200 p-6 dark:border-gray-700">
                        <div>
                            <h2 id="share-modal-title" class="text-xl font-semibold text-gray-900 dark:text-white">Share Component Preview</h2>
                    ...
                    <div class="space-y-6 p-6">
                        ...
                        <div class="rounded-lg bg-gray-50 p-4 dark:bg-gray-700">
                            <div class="flex items-center space-x-3">
                                <Icon :name="getCategoryIcon(component.category)" class="h-8 w-8 text-gray-500 dark:text-gray-400" />
                                <div>
                        ...
                        <div class="space-y-4">
                            <!-- Direct Link -->
                            <div>
                                <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"> Direct Link </label>
                                <div class="flex items-center space-x-2">
                                    <input
                                        ref="directLinkInput"
                                        :value="previewUrl"
                                        readonly
                                        class="block w-full flex-1 rounded-md border border-gray-300 bg-gray-50 px-3 py-2 text-sm text-gray-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                    />
                                    <button
                                ...
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Share this link to let others view the component preview</p>
                            </div>

                            <!-- Embed Code -->
                            <div>
                                <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"> Embed Code </label>
                                <div class="relative">
                                    <textarea
                                        ...
                                        readonly
                                        rows="4"
                                        class="block w-full rounded-md border border-gray-300 bg-gray-50 px-3 py-2 font-mono text-sm text-gray-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                    ></textarea>
                                    <button
                                        @click="copyEmbedCode"
                                        class="absolute right-2 top-2 inline-flex items-center rounded border border-gray-300 bg-white px-2 py-1 text-xs text-gray-500 hover:text-gray-700 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:text-gray-300"
                                    >
                                        <Icon name="clipboard" class="mr-1 h-3 w-3" />
                                        Copy
                                    </button>
                                </div>
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Embed this iframe in your documentation or website</p>
                            </div>

                            <!-- QR Code -->
                            <div>
                                <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"> QR Code </label>
                                <div class="flex items-center space-x-4">
                                    <div class="flex-shrink-0">
                                        <div
                                            ref="qrCodeContainer"
                                            class="flex h-32 w-32 items-center justify-center rounded-lg border border-gray-300 bg-white"
                                        >
                                            <canvas ref="qrCodeCanvas" width="128" height="128" class="max-h-full max-w-full"></canvas>
                                        </div>
                                    </div>
                                    <div class="flex-1">
                        ...
                        <div class="border-t border-gray-200 pt-6 dark:border-gray-700">
                            <h3 class="mb-4 text-lg font-medium text-gray-900 dark:text-white">Share Settings</h3>

                            <div class="space-y-4">
                                <!-- Expiration -->
                                <div>
                                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"> Link Expiration </label>
                                    <select
                                ...
                                <!-- Analytics -->
                                <div class="flex items-center justify-between">
                                    <label class="text-sm font-medium text-gray-700 dark:text-gray-300"> Track Views </label>
                                    <button
                        ...
                        <!-- Social Sharing -->
                        <div class="border-t border-gray-200 pt-6 dark:border-gray-700">
                            <h3 class="mb-4 text-lg font-medium text-gray-900 dark:text-white">Social Sharing</h3>
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <button
                                    ...
                                    class="inline-flex items-center rounded-md border border-transparent bg-blue-700 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                                >
                                ...
                                >
                                    <Icon name="envelope" class="mr-2 h-4 w-4" />
                                    Email
                                </button>
                            </div>
                        </div>
                    ...
                    <!-- Modal Footer -->
                    <footer class="flex items-center justify-end space-x-3 border-t border-gray-200 p-6 dark:border-gray-700">
                        <button
                            @click="handleClose"
        ...
        <div
            ...
            class="z-60 fixed bottom-4 right-4 rounded-md bg-green-500 px-4 py-2 text-white shadow-lg transition-all duration-300"
        >
            <div class="flex items-center space-x-2">
...
const getPasswordToggleThumbClasses = () => [shareSettings.value.passwordProtected ? 'translate-x-5' : 'translate-x-0'];
...
const getCommentsToggleThumbClasses = () => [shareSettings.value.allowComments ? 'translate-x-5' : 'translate-x-0'];
...
const getAnalyticsToggleThumbClasses = () => [shareSettings.value.trackViews ? 'translate-x-5' : 'translate-x-0'];
...
const generateQRCode = async () => {
    ...
    try {
        ...
        const positions = [
            [10, 10],
            [canvas.width - cornerSize - 10, 10],
            [10, canvas.height - cornerSize - 10],
        ];
