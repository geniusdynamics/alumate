    ...
    <div class="content-editor" role="region" aria-labelledby="content-editor-label">
        ...
        <div class="content-blocks-list">
            <div class="blocks-header">
                ...
                <button @click="addNewBlock('text')" class="btn-add-block" aria-label="Add new text block">
                    ...
                    <div class="block-header">
                        <div class="block-info">
                            <div class="block-type-icon">
                                <component :is="getBlockIcon(block.type)" class="h-4 w-4" />
                            </div>
                        ...
                        <div class="block-actions">
                            <button @click.stop="duplicateBlock(block.id)" class="block-action-btn" :aria-label="`Duplicate ${block.type} block`">
                            ...
                            <button @click.stop="deleteBlock(block.id)" class="block-action-btn delete" :aria-label="`Delete ${block.type} block`">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
                                    <path
                    ...
                    <div class="block-preview">
                        ...
                        <template v-else-if="block.type === 'image'">
                            <div class="image-preview">
                                <img v-if="block.data.url" :src="block.data.url" :alt="block.data.alt || 'Image preview'" class="image-thumb" />
                ...
                <template v-else-if="selectedBlock.type === 'image'">
                    <div class="image-editor">
                        <div
                            ...
                            @keydown.space.prevent="openImageSelector"
                            tabindex="0"
                            role="button"
                        >
        ...
        <div v-if="showBlockTypesModal" class="modal-overlay" @click="showBlockTypesModal = false">
            <div class="block-types-modal" @click.stop>
                ...
                <div class="block-types-grid">
                    ...
                    >
                        <component :is="type.icon" class="mb-2 h-8 w-8" />
        ...
        <!-- Hidden file input -->
        <input ref="fileInput" type="file" class="hidden-input" @change="handleFileSelection" accept="image/*" aria-label="Image file selection" />
...
const updateActiveFormats = () => {
    activeFormats.value = textFormats.filter((format) => document.queryCommandState(format.command)).map((format) => format.id);
};
...
<style scoped>
.content-editor {
    @apply min-h-screen rounded-lg bg-white shadow-sm dark:bg-gray-800;
}
...
.btn-add-block {
    @apply flex items-center gap-2 rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700;
}
...
.content-block {
    @apply relative cursor-pointer rounded-lg border-2 border-transparent bg-gray-50 transition-all duration-200 dark:bg-gray-700;
}
...
.content-block.dragging {
    @apply scale-105 transform opacity-50;
}

.block-header {
    @apply flex items-center justify-between rounded-t-lg bg-gray-100 p-3 dark:bg-gray-600;
}
...
.block-action-btn {
    @apply rounded p-1 text-gray-600 transition-colors hover:bg-gray-200 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-500 dark:hover:text-gray-100;
}
...
.text-preview {
    @apply line-clamp-3 text-sm text-gray-700 dark:text-gray-300;
    display: -webkit-box;
    -webkit-line-clamp: 3;
...
.image-thumb {
    @apply h-16 w-16 rounded border object-cover;
}

.image-placeholder {
    @apply flex h-16 w-16 items-center justify-center rounded border border-dashed border-gray-400 bg-gray-200 text-gray-400 dark:border-gray-500 dark:bg-gray-600;
}
...
.btn-preview {
    @apply rounded-md bg-blue-600 px-4 py-2 text-sm text-white;
}
...
.add-block-placeholder {
    @apply cursor-pointer rounded-lg border-2 border-dashed border-gray-300 p-8 transition-colors hover:border-gray-400 dark:border-gray-600 dark:hover:border-gray-500;
}
...
.block-drag-handle {
    @apply absolute right-2 top-2 cursor-move rounded bg-gray-200 p-1 opacity-0 transition-opacity hover:opacity-100 dark:bg-gray-700;
}
...
/* Content Editor Panel */
.content-editor-panel {
    @apply border-t border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800;
}
...
.panel-close {
    @apply text-xl text-gray-400 transition-colors hover:text-gray-600;
}
...
.editor-toolbar {
    @apply flex gap-1 rounded border bg-gray-50 p-2 dark:bg-gray-700;
}

.format-btn {
    @apply rounded p-2 text-gray-600 transition-colors hover:bg-gray-200 dark:text-gray-400 dark:hover:bg-gray-600;
}
...
.text-input {
    @apply min-h-32 w-full resize-y rounded border border-gray-300 bg-white p-3 text-gray-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white;
}
...
.align-btn {
    @apply rounded p-2 transition-colors hover:bg-gray-200 dark:hover:bg-gray-600;
}
...
.image-upload-area {
    @apply cursor-pointer rounded-lg border-2 border-dashed border-gray-300 p-8 text-center transition-colors hover:border-gray-400 dark:border-gray-600 dark:hover:border-gray-500;
}
...
.selected-image {
    @apply h-48 w-full rounded border object-cover;
}

.image-overlay {
    @apply absolute inset-0 flex items-center justify-center rounded bg-black bg-opacity-50 opacity-0 transition-opacity hover:opacity-100;
}

.replace-btn {
    @apply rounded bg-white px-4 py-2 font-medium text-black transition-colors hover:bg-gray-100;
}
...
.button-styles-preview {
    @apply flex justify-center rounded bg-gray-100 p-4 dark:bg-gray-700;
}
...
.transcript-preview {
    @apply rounded border bg-gray-50 p-3 dark:bg-gray-700;
}

.edit-transcript {
    @apply mt-2 rounded bg-blue-600 px-3 py-1 text-sm text-white transition-colors hover:bg-blue-700;
}
...
.setting-input,
.setting-select {
    @apply w-full rounded border border-gray-300 bg-white px-3 py-2 text-gray-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white;
}
...
.block-type-btn {
    @apply rounded-lg border border-gray-300 p-4 text-center transition-colors hover:border-blue-500 hover:bg-blue-50 dark:border-gray-600 dark:hover:bg-blue-900/20;
}
...
@media (prefers-color-scheme: dark) {
    ...
    .text-input {
        @apply border-gray-600 bg-gray-700 text-gray-300;
    }
