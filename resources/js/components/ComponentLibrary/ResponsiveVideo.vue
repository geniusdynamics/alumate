<template>
    <div ref="containerRef" :class="containerClasses" :style="containerStyles">
        <!-- Video Element -->
        <video
        ...
        >
            ...
            <p class="sr-only">
                ...
                <a :href="fallbackImageSrc" target="_blank" rel="noopener"> View static image instead </a>
            </p>
        </video>
        ...
        >
            ...
            <template #overlay>
                <button
                    v-if="showPlayButton"
                    @click="playVideo"
                    class="rounded-full bg-black/50 p-4 text-white transition-colors duration-200 hover:bg-black/70 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-black/50"
        ...
        <!-- Loading State -->
        <div v-if="isLoading" class="absolute inset-0 flex items-center justify-center bg-gray-900/50" role="status" :aria-label="loadingMessage">
            <div class="rounded-lg bg-black/50 p-4 text-center text-white">
                <div class="mx-auto mb-2 h-8 w-8 animate-spin rounded-full border-b-2 border-white"></div>
                <p class="text-sm">{{ loadingMessage }}</p>
                <div v-if="loadingProgress > 0" class="mt-2 h-1 rounded-full bg-gray-700">
                    <div class="h-1 rounded-full bg-white transition-all duration-300" :style="{ width: `${loadingProgress}%` }" />
                </div>
            </div>
        </div>
        ...
        >
            <div class="p-4 text-center text-gray-500 dark:text-gray-400">
                ...
                <button v-if="fallbackImageSrc" @click="showFallbackImage" class="text-sm text-blue-500 underline hover:text-blue-600">
        ...
        <div
            ...
            class="absolute bottom-4 left-4 right-4 flex items-center justify-between rounded-lg bg-black/50 p-2"
        >
            <button @click="togglePlayPause" class="p-2 text-white hover:text-gray-300" :aria-label="isPlaying ? 'Pause video' : 'Play video'">
            ...
            <button @click="toggleMute" class="p-2 text-white hover:text-gray-300" :aria-label="isMuted ? 'Unmute video' : 'Mute video'">
                <svg v-if="isMuted" class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                    <path
                        fill-rule="evenodd"
        ...
        <!-- Bandwidth Warning -->
        <div v-if="showBandwidthWarning" class="absolute left-4 right-4 top-4 rounded-lg bg-yellow-500/90 p-3 text-sm text-yellow-900" role="alert">
            <p class="font-medium">Slow connection detected</p>
            <p>Video quality has been reduced to improve loading time.</p>
            <button @click="dismissBandwidthWarning" class="mt-1 text-yellow-800 underline hover:text-yellow-900">Dismiss</button>
        </div>
    </div>
</template>
