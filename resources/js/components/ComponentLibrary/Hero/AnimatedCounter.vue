<template>
    ...
    >
        ...
        <div v-else-if="hasError" class="error-state" role="alert">
            ...
            <button v-if="canRetry" @click="retryDataFetch" class="retry-button" :aria-label="retryMessage" type="button">
...
const canRetry = computed(() => {
    return props.statistic.source === 'api' && props.statistic.apiEndpoint && retryCount.value < props.retryAttempts;
});
...
const formatNumber = (value: number): string => {
    try {
        ...
        if (value >= 1000000000) {
            return (
                (value / 1000000000).toLocaleString(props.locale, {
            ...
            return (
                (value / 1000000).toLocaleString(props.locale, {
                    minimumFractionDigits: 0,
                    maximumFractionDigits: 1,
                }) + 'M'
            );
            ...
            return (
                (value / 1000).toLocaleString(props.locale, {
                    minimumFractionDigits: 0,
                    maximumFractionDigits: 1,
                }) + 'K'
            );
...
.retry-button {
    @apply rounded p-1 text-red-600 transition-colors duration-200 hover:text-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:text-red-400 dark:hover:text-red-300;
}
