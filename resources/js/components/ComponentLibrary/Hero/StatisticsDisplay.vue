<template>
    ...
    >
            ...
            >
                ...
                <div class="statistic-content">
                    ...
                    <div v-if="showLastUpdated && getLastUpdated(statistic.id)" class="statistic-meta">
                        <time :datetime="getLastUpdated(statistic.id)?.toISOString()" class="last-updated">
                ...
                <div v-if="getStatisticState(statistic.id)?.error" class="statistic-error" role="alert">
        ...
        <div v-if="showActions && (hasAnyError || showRefresh)" class="statistics-actions">
            <button v-if="hasAnyError" @click="retryFailed" :disabled="isAnyLoading" class="action-button retry-button" type="button">
            ...
            <button v-if="showRefresh" @click="refresh" :disabled="isAnyLoading" class="action-button refresh-button" type="button">
                <svg class="button-icon" viewBox="0 0 20 20" fill="currentColor">
                    <path
                        fill-rule="evenodd"
                        d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z"
...
const { statisticStates, isAnyLoading, hasAnyError, loadingProgress, getStatisticState, refresh, retryFailed, formatError } = useStatistics(
    props.statistics,
    {
...
// Computed properties
const accessibilityLabel = computed(() => `Statistics display with ${props.statistics.length} metrics`);

const loadingMessage = computed(() => `Loading ${props.statistics.length} statistics...`);

const hasAnyData = computed(() => Array.from(statisticStates.value.values()).some((state) => state.data !== null));
...
/* Loading Overlay */
.loading-overlay {
    @apply absolute inset-0 flex items-center justify-center rounded-lg bg-white/80 backdrop-blur-sm dark:bg-gray-900/80;
}
...
.loading-spinner svg {
    @apply mx-auto mb-4 h-8 w-8 text-blue-600 dark:text-blue-400;
}
...
.loading-progress {
    @apply h-2 w-32 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700;
}

.progress-bar {
    @apply h-full bg-blue-600 transition-all duration-300 ease-out dark:bg-blue-400;
}
...
.layout-compact .statistics-grid {
    @apply grid-cols-2 gap-4 sm:grid-cols-4;
}
...
/* Error States */
.statistic-error {
    @apply mt-2 flex items-center justify-center gap-1 text-xs text-red-600 dark:text-red-400;
}
...
/* Actions */
.statistics-actions {
    @apply mt-6 flex justify-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700;
}

.action-button {
    @apply inline-flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2;
}

.retry-button {
    @apply bg-red-50 text-red-700 hover:bg-red-100 focus:ring-red-500 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/30 dark:focus:ring-red-400;
}

.refresh-button {
    @apply bg-blue-50 text-blue-700 hover:bg-blue-100 focus:ring-blue-500 dark:bg-blue-900/20 dark:text-blue-400 dark:hover:bg-blue-900/30 dark:focus:ring-blue-400;
}
...
@media (prefers-contrast: high) {
    .statistic-item {
        @apply rounded-lg border border-gray-300 p-4 dark:border-gray-600;
    }

    .statistic-error {
        @apply rounded border border-red-600 p-2;
    }
}
