    ...
    >
        <template #default="{ statistics, isVisible, animate }">
            ...
            <div
                ...
                :class="['statistics-counters', `grid-cols-${getGridColumns()}`]"
            >
                <AnimatedCounter
            ...
            <!-- Progress Bars -->
            <div v-if="config.displayType === 'progress' || config.displayType === 'mixed'" class="statistics-progress">
                <ProgressBar
            ...
            <!-- Comparison Charts -->
            <div v-if="config.displayType === 'charts' || config.displayType === 'mixed'" class="statistics-charts">
                <ComparisonChart
...
import type { ChartDataItem, ChartLegendItem } from './ComparisonChart.vue';
import ComparisonChart from './ComparisonChart.vue';
import type { ProgressMilestone, ProgressSegment } from './ProgressBar.vue';
import ProgressBar from './ProgressBar.vue';
import type { StatisticItem, StatisticsConfig } from './StatisticsBase.vue';
...
const isLoading = computed(() => {
    return dataLoading.value || props.statistics.some((stat) => stat.source === 'api' && isSourceLoading(stat.id));
});

const hasErrors = computed(() => {
    return dataHasErrors.value || props.statistics.some((stat) => getErrorBySource(stat.id));
});
