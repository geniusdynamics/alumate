<template>
    <div
        ref="progressRef"
        :class="['progress-bar-container', `size-${size}`, `theme-${theme}`, { 'has-label': showLabel, 'has-value': showValue }]"
    ...
    >
        ...
        <!-- Progress bar track -->
        <div :class="['progress-track', { 'has-segments': segments && segments.length > 0 }]">
            <!-- Segments (if defined) -->
            <div v-if="segments && segments.length > 0" class="progress-segments">
                <div
                    v-for="(segment, index) in segments"
                    :key="index"
                    :class="['progress-segment', { active: animatedProgress >= segment.threshold }]"
                    :style="{
                        width: `${segment.width}%`,
            ...
            <!-- Main progress fill -->
            <div
                :class="['progress-fill', `color-${color}`, { animated: animate && !prefersReducedMotion }]"
                :style="{
            ...
            >
                ...
                <div v-if="theme === 'modern'" class="progress-gradient" />

                <!-- Animated stripes for loading state -->
                <div v-if="loading || animated" class="progress-stripes" />
            </div>

            <!-- Milestone markers -->
            <div v-if="milestones && milestones.length > 0" class="progress-milestones">
                <div
        ...
        <!-- Loading overlay -->
        <div v-if="loading" class="progress-loading" aria-label="Loading progress data">
            <div class="loading-spinner" />
        </div>

        <!-- Error state -->
        <div v-if="error" class="progress-error" role="alert" :aria-label="`Error loading ${label}: ${error}`">
...
.progress-value {
    @apply text-sm font-semibold tabular-nums text-gray-900 dark:text-white;
...
.progress-track {
    @apply relative w-full overflow-hidden bg-gray-200 dark:bg-gray-700;
}
...
.theme-modern .progress-track {
    @apply bg-gradient-to-r from-gray-100 to-gray-200 shadow-inner dark:from-gray-800 dark:to-gray-700;
}
...
.milestone-marker {
    @apply pointer-events-auto absolute top-0 -translate-x-1/2 transform;
    height: 100%;
}

.milestone-dot {
    @apply absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform;
    @apply h-3 w-3 rounded-full border-2 border-gray-400 bg-white transition-colors duration-200;
}
...
.milestone-marker.current .milestone-dot {
    @apply scale-125 border-blue-500 bg-blue-500;
}

.milestone-label {
    @apply absolute left-1/2 top-full mt-1 -translate-x-1/2 transform;
    @apply whitespace-nowrap text-xs text-gray-600 dark:text-gray-400;
}
...
.progress-loading {
    @apply absolute inset-0 flex items-center justify-center rounded-lg bg-white/80 dark:bg-gray-800/80;
}

.loading-spinner {
    @apply h-5 w-5 animate-spin rounded-full border-2 border-blue-600 border-t-transparent;
}

.progress-error {
    @apply absolute inset-0 flex items-center justify-center rounded-lg bg-red-50/90 p-2 dark:bg-red-900/20;
}

.error-icon {
    @apply mr-2 h-4 w-4 text-red-600 dark:text-red-400;
}
