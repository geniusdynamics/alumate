<template>
    <div
        ref="counterRef"
        :class="['animated-counter', `size-${size}`, `theme-${theme}`, { 'has-icon': !!icon, 'has-trend': !!trend }]"
    ...
    >
        ...
        <div class="counter-content">
            ...
            <div class="counter-value-container">
                <span ref="valueRef" class="counter-value" :aria-label="`Current value: ${displayValue}`">
        ...
        <!-- Loading overlay -->
        <div v-if="loading" class="counter-loading" aria-label="Loading counter data">
            <div class="loading-spinner" />
        </div>

        <!-- Error state -->
        <div v-if="error" class="counter-error" role="alert" :aria-label="`Error loading ${label}: ${error}`">
...
<style scoped>
.animated-counter {
    @apply relative flex flex-col items-center rounded-lg p-4 text-center transition-all duration-300;
}
...
.counter-icon .icon {
    @apply h-8 w-8 text-blue-600 dark:text-blue-400;
}
...
.counter-value {
    @apply font-bold tabular-nums text-gray-900 dark:text-white;
...
.trend-indicator {
    @apply flex items-center gap-1 rounded-full px-2 py-1 text-sm font-medium;
}
...
.counter-label {
    @apply mb-1 text-sm font-medium text-gray-600 dark:text-gray-300;
}
...
.counter-description {
    @apply max-w-xs text-xs text-gray-500 dark:text-gray-400;
}
...
.loading-spinner {
    @apply h-6 w-6 animate-spin rounded-full border-2 border-blue-600 border-t-transparent;
}

.counter-error {
    @apply absolute inset-0 flex flex-col items-center justify-center rounded-lg bg-red-50/90 p-2 dark:bg-red-900/20;
}

.error-icon {
    @apply mb-1 h-5 w-5 text-red-600 dark:text-red-400;
}

.error-text {
    @apply text-center text-xs text-red-600 dark:text-red-400;
}
...
.theme-modern {
    @apply border border-blue-100 bg-gradient-to-br from-blue-50 to-indigo-50 dark:border-blue-800 dark:from-blue-900/20 dark:to-indigo-900/20;
}

.theme-card {
    @apply border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800;
}
...
.theme-minimal:hover {
    @apply scale-100 transform bg-gray-50 dark:bg-gray-800/50;
}
