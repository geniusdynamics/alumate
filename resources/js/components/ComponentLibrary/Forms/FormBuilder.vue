<template>
    <div class="form-builder overflow-hidden rounded-lg bg-white shadow-lg dark:bg-gray-800">
        <!-- Header -->
        <div class="border-b border-gray-200 bg-gray-50 px-6 py-4 dark:border-gray-600 dark:bg-gray-700">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                    <button
                        type="button"
                        class="rounded-md bg-blue-100 px-3 py-1 text-sm text-blue-700 hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-blue-900/30 dark:text-blue-300 dark:hover:bg-blue-900/50"
                        @click="previewForm"
                    >
                        Preview
                    </button>
                    <button
                        type="button"
                        class="rounded-md bg-green-100 px-3 py-1 text-sm text-green-700 hover:bg-green-200 focus:outline-none focus:ring-2 focus:ring-green-500 dark:bg-green-900/30 dark:text-green-300 dark:hover:bg-green-900/50"
                        @click="saveForm"
                    >
                        Save
                    </button>
        ...
        <div class="flex h-[600px]">
            ...
            <div class="w-64 overflow-y-auto border-r border-gray-200 bg-gray-50 dark:border-gray-600 dark:bg-gray-700">
                <div class="p-4">
                    <h4 class="mb-3 text-sm font-medium text-gray-900 dark:text-white">Field Types</h4>

                    <div class="space-y-2">
                        <div
                            v-for="fieldType in fieldTypes"
                            :key="fieldType.type"
                            class="field-type-item cursor-grab rounded-lg border border-gray-200 bg-white p-3 transition-shadow hover:shadow-md dark:border-gray-600 dark:bg-gray-800"
                            :draggable="true"
                            @dragstart="handleFieldTypeDragStart($event, fieldType)"
                            @dragend="handleFieldTypeDragEnd"
                        >
                            <div class="flex items-center">
                                <component :is="fieldType.icon" class="mr-3 h-5 w-5 text-gray-500 dark:text-gray-400" aria-hidden="true" />
                                <div>
            ...
            <div class="flex flex-1 flex-col">
                <!-- Form Settings Bar -->
                <div class="border-b border-gray-200 bg-white px-6 py-3 dark:border-gray-600 dark:bg-gray-800">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            <div>
                                ...
                                <select
                                    ...
                                    class="ml-2 rounded-md border-gray-300 bg-white text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                >
                            ...
                            <div>
                                <label class="text-sm font-medium text-gray-700 dark:text-gray-300"> Spacing: </label>
                                <select
                ...
                >
                    ...
                    <div
                        ...
                        class="flex h-full items-center justify-center rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600"
                    >
                        <div class="text-center">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No fields added</h3>
                    ...
                    <div v-else class="space-y-4">
                        <div
                            ...
                            :key="field.id"
                            class="form-field-item group relative"
                            :class="{ 'ring-2 ring-blue-500': selectedFieldId === field.id }"
                            @click="selectField(field.id)"
                        >
                            <!-- Drop Indicator -->
                            <div v-if="dragOverIndex === index" class="absolute -top-2 left-0 right-0 z-10 h-1 rounded-full bg-blue-500" />

                            <!-- Field Preview -->
                            <div
                                class="cursor-pointer rounded-lg border border-gray-200 bg-gray-50 p-4 transition-colors hover:border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:hover:border-gray-500"
                                :draggable="true"
                            ...
                            >
                                ...
                                <div class="absolute right-2 top-2 opacity-0 transition-opacity group-hover:opacity-100">
                                    <div class="flex items-center space-x-1">
                                        ...
                                        <button
                                            type="button"
                                            class="rounded p-1 text-gray-400 hover:text-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 dark:text-gray-500 dark:hover:text-red-400"
                            ...
                            <div v-if="dragOverIndex === index + 1" class="absolute -bottom-2 left-0 right-0 z-10 h-1 rounded-full bg-blue-500" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-if="selectedField" class="w-80 overflow-y-auto border-l border-gray-200 bg-gray-50 dark:border-gray-600 dark:bg-gray-700">
                <div class="p-4">
                    <h4 class="mb-4 text-sm font-medium text-gray-900 dark:text-white">Field Properties</h4>

                    <FieldPropertiesEditor :field="selectedField" @update:field="updateField" />
                </div>
            </div>
        </div>

        <!-- Field Editor Modal -->
        <FieldEditorModal v-if="editingField" :field="editingField" @save="saveFieldEdit" @cancel="cancelFieldEdit" />
    </div>
</template>
...
const TextIcon = {
    template:
        '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path d="M2 5a2 2 0 012-2h12a2 2 0 012 2v10a2 2 0 01-2 2H4a2 2 0 01-2-2V5zm3.293 1.293a1 1 0 011.414 0l3 3a1 1 0 010 1.414l-3 3a1 1 0 01-1.414-1.414L7.586 10 5.293 7.707a1 1 0 010-1.414zM11 12a1 1 0 100 2h3a1 1 0 100-2h-3z"/></svg>',
};
const EmailIcon = {
    template:
...
const TextareaIcon = {
    template:
        '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M3 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 4a1 1 0 011-1h6a1 1 0 110 2H4a1 1 0 01-1-1z" clip-rule="evenodd"/></svg>',
};
const SelectIcon = {
    template:
...
const NumberIcon = {
    template:
        '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"/></svg>',
};
const UrlIcon = {
    template:
