<template>
    ...
    >
        ...
        <div v-if="config.title || config.description" class="mb-6 sm:mb-8">
            <h2 v-if="config.title" :id="`${formId}-title`" class="mb-2 text-2xl font-bold text-gray-900 dark:text-white">
                {{ config.title }}
            </h2>
            <p v-if="config.description" :id="`${formId}-description`" class="text-gray-600 dark:text-gray-300">
                {{ config.description }}
            </p>
        </div>

        <!-- Screen Reader Instructions -->
        <div v-if="config.screenReaderInstructions" class="sr-only" :id="`${formId}-instructions`" aria-live="polite">
        ...
        >
            <div class="mb-2 flex items-center justify-between">
                ...
                <span class="text-sm text-gray-500 dark:text-gray-400"> {{ Math.round((currentStep / totalSteps) * 100) }}% Complete </span>
            </div>
        ...
        <div
            ...
            aria-live="polite"
        >
            <h3 class="mb-2 text-sm font-medium text-red-800 dark:text-red-200">Please correct the following errors:</h3>
            <ul class="space-y-1 text-sm text-red-700 dark:text-red-300">
                <li v-for="error in validationErrors" :key="error.field">
                    <a :href="`#${error.field}`" class="hover:underline focus:underline focus:outline-none" @click.prevent="focusField(error.field)">
                        {{ error.message }}
                    </a>
                </li>
            </ul>
        ...
        <input
            ...
            name="website"
            :id="`${formId}-honeypot`"
            class="-left-9999px pointer-events-none absolute opacity-0"
            tabindex="-1"
            autocomplete="off"
        ...
        <!-- reCAPTCHA -->
        <div v-if="config.recaptcha?.enabled" class="mb-6">
            <div
                :id="`${formId}-recaptcha`"
                class="g-recaptcha"
        ...
        <div class="flex flex-col justify-end gap-4 sm:flex-row">
            <button
                v-if="currentStep > 1"
                type="button"
                class="rounded-lg bg-gray-100 px-6 py-3 font-medium text-gray-700 transition-colors hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600 dark:focus:ring-offset-gray-900"
                @click="previousStep"
            ...
            <button
                ...
                type="button"
                class="rounded-lg bg-blue-50 px-6 py-3 font-medium text-blue-700 transition-colors hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:bg-blue-900/20 dark:text-blue-300 dark:hover:bg-blue-900/30 dark:focus:ring-offset-gray-900"
                @click="saveProgress"
                :disabled="isSubmitting"
            >
                Save Progress
        ...
        <div v-if="isSubmitting" :id="`${formId}-submitting`" class="sr-only" aria-live="assertive">Form is being submitted. Please wait.</div>
    </form>
</template>
...
// Composables
const { fieldErrors, validationErrors, hasErrors, isFormValid, validateField, validateForm, clearFieldError } = useFormValidation(
    props.config,
    formData,
);

const { submitForm, isSubmitting: submissionLoading } = useFormSubmission(props.config);

const { autoSaveStatus, autoSaveStatusText, startAutoSave, stopAutoSave, saveProgress } = useFormAutoSave(props.config, formData);
    ...
    try {
        ...
        if (props.config.trackingEnabled && typeof window !== 'undefined' && (window as any).gtag) {
            (window as any).gtag('event', 'form_submit', {
                ...
                ...(props.config.trackingEvents || []).reduce(
                    (acc, event) => {
                        acc[event] = true;
                        return acc;
                    },
