<template>
    <div :class="fieldContainerClasses">
        <!-- Field Label -->
        <label v-if="field.type !== 'checkbox'" :for="fieldId" :class="labelClasses">
            {{ field.label }}
            <span v-if="field.required" class="ml-1 text-red-500" aria-label="required"> * </span>
        </label>
        ...
        <div class="relative">
            ...
            <div v-else-if="field.type === 'checkbox'" class="flex items-start">
                <input
                    ...
                    @focus="handleFocus"
                />
                <label :for="fieldId" :class="checkboxLabelClasses">
                    {{ field.label }}
                    <span v-if="field.required" class="ml-1 text-red-500" aria-label="required"> * </span>
                </label>
            </div>
            ...
            <!-- Radio Group -->
            <fieldset v-else-if="field.type === 'radio'" :aria-label="field.label" :aria-describedby="getAriaDescribedBy()" class="space-y-2">
                ...
                <div v-for="option in field.options" :key="option.value" class="flex items-center">
                    <input
                        ...
                        @focus="handleFocus"
                    />
                    <label :for="`${fieldId}-${option.value}`" :class="radioLabelClasses">
                        {{ option.label }}
                    </label>
                </div>
            </fieldset>
            ...
            <button
                v-if="draggable"
                type="button"
                class="absolute -left-8 top-1/2 -translate-y-1/2 transform cursor-grab rounded p-1 text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 active:cursor-grabbing dark:text-gray-500 dark:hover:text-gray-300"
        ...
        <!-- Help Text -->
        <p v-if="field.helpText" :id="`${fieldId}-help`" class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ field.helpText }}
        </p>
        ...
        <div
            ...
            aria-live="polite"
        >
            <svg class="mr-1 h-4 w-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                <path
                    fill-rule="evenodd"
