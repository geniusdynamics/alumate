<template>
    ...
    >
        ...
        <div v-if="config.loading" class="cta-button__loading">
            <div v-if="config.animation?.loading === 'spinner'" class="cta-button__spinner" aria-hidden="true" />
            <div v-else-if="config.animation?.loading === 'dots'" class="cta-button__dots" aria-hidden="true">
                <span></span>
                <span></span>
                <span></span>
            </div>
            <div v-else class="cta-button__pulse" aria-hidden="true" />
        </div>
        ...
        <div v-else class="cta-button__content">
            ...
            <!-- Button text -->
            <span v-if="config.icon?.position !== 'only'" class="cta-button__text">
                {{ config.text }}
            </span>
        ...
        <div v-if="showRipple && config.animation?.click === 'ripple'" class="cta-button__ripple" :style="rippleStyles" />
    </component>
</template>
...
/* Styles */
.cta-button--primary {
    @apply border border-transparent bg-blue-600 text-white;
}
...
.cta-button--secondary {
    @apply border border-transparent bg-gray-600 text-white;
}
...
.cta-button--ghost {
    @apply border border-transparent bg-transparent text-blue-600;
}
...
.cta-button--link {
    @apply h-auto border border-transparent bg-transparent p-0 text-blue-600;
}
...
.cta-button__dots span {
    @apply h-1 w-1 animate-pulse rounded-full bg-current;
}
...
.cta-button__pulse {
    @apply h-4 w-4 animate-pulse rounded-full bg-current;
}
