<template>
    <div :class="cardClasses" role="gridcell" :aria-label="cardAriaLabel" @dragstart="handleDragStart" :draggable="!comparisonMode">
        <!-- Component Preview Image -->
        <div class="component-card__preview">
            <div class="group relative aspect-video overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-800">
                <img
                    v-if="previewImage"
                    :src="previewImage"
                ...
                <!-- Overlay Actions -->
                <div
                    class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-0 opacity-0 transition-all duration-300 group-hover:bg-opacity-40 group-hover:opacity-100"
                >
                    <div class="flex space-x-2">
                        <button
                            @click="$emit('preview', component)"
                            class="rounded-md bg-white px-4 py-2 text-sm font-medium text-gray-900 transition-colors hover:bg-gray-100"
                        ...
                        >
                            <Icon name="information-circle" class="mr-2 h-4 w-4" />
                            Details
                        </button>
                    </div>
                </div>
                ...
                <!-- Comparison Checkbox -->
                <div v-if="comparisonMode" class="absolute right-3 top-3">
                    <label class="flex items-center">
                        ...
                        >
                            <Icon v-if="selectedForComparison" name="check" class="h-4 w-4 text-white" />
                        </div>
                        <span class="sr-only"> {{ selectedForComparison ? 'Remove from' : 'Add to' }} comparison </span>
                    </label>
                </div>
            </div>
        </div>
        ...
        <div class="component-card__content p-4">
            ...
            <div class="mb-3 flex items-start justify-between">
                <div class="min-w-0 flex-1">
                    ...
                    <div class="mt-1 flex items-center space-x-2">
                        ...
                        >
                            <Icon :name="getCategoryIcon(component.category)" class="mr-1 h-3 w-3" />
            ...
            <!-- Description -->
            <p v-if="component.description" class="mb-4 line-clamp-2 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <div class="flex items-center space-x-1">
                        <div class="flex items-center">
                            <Icon
                                ...
                                :class="['h-4 w-4', star <= Math.floor(rating.average) ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600']"
                            />
                        </div>
            ...
            <div class="flex space-x-2">
                <button
                    ...
                    class="flex-1 rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                >
            ...
            <div v-if="viewMode === 'list'" class="mt-4 border-t border-gray-200 pt-4 dark:border-gray-700">
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        <span class="font-medium text-gray-900 dark:text-white">Version:</span>
                        <span class="ml-2 text-gray-600 dark:text-gray-400">{{ component.version }}</span>
                    </div>
                    <div>
                        <span class="font-medium text-gray-900 dark:text-white">Type:</span>
                        <span class="ml-2 text-gray-600 dark:text-gray-400">{{ component.type }}</span>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <span class="ml-2 text-gray-600 dark:text-gray-400">{{ usageStats.recentUsage }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- Drag Indicator -->
        <div v-if="!comparisonMode" class="absolute right-2 top-2 opacity-0 transition-opacity group-hover:opacity-100">
...
.component-card--comparison:hover {
    @apply translate-y-0 transform-none;
}
