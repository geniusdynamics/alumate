<template>
    ...
    >
        ...
        >
            ...
            <div
                v-if="showDemoHeader"
                class="demo-header flex items-center justify-between border-b border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800"
            >
                <div class="flex items-center space-x-3">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                        {{ demoTitle }}
                    </h3>
                    <span v-if="demoStatus" :class="statusClasses">
                        {{ demoStatus }}
                    </span>
                </div>
                ...
                <div class="flex items-center space-x-2">
                    ...
                    >
                        <svg v-if="!isRunning" class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <button v-if="config.demoSettings?.showControls" @click="resetDemo" :class="controlButtonClasses" :aria-label="'Reset demo'">
            ...
            >
                ...
                <div v-else-if="demoType === 'sequence' && demoImages.length > 0" class="relative h-full w-full">
                    <ResponsiveImage
                ...
                <div v-if="config.demoSettings?.enableInteraction && hotspots.length > 0" class="pointer-events-none absolute inset-0">
                    <button
                    ...
                    >
                        <div class="h-4 w-4 animate-ping rounded-full bg-indigo-600"></div>
                        <div class="absolute inset-0 h-4 w-4 rounded-full bg-indigo-600"></div>
                    </button>
                </div>
                ...
                >
                    <div class="text-white">
                        <h4 class="mb-2 text-lg font-semibold">{{ overlayInfo.title }}</h4>
                        <p class="mb-3 text-sm text-gray-200">{{ overlayInfo.description }}</p>
                ...
                <!-- Error State -->
                <div v-if="isDemoError" class="absolute inset-0 flex items-center justify-center bg-gray-50 p-4 dark:bg-gray-800">
                    <div class="text-center">
                        ...
                        <p class="mb-4 text-sm text-gray-600 dark:text-gray-400">Unable to load interactive demo</p>
                        <button @click="retryDemo" class="rounded-lg bg-indigo-600 px-4 py-2 text-white transition-colors hover:bg-indigo-700">
                            Try Again
                        </button>
                    </div>
                </div>
            ...
            <div
                ...
                class="demo-instructions border-t border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800"
            >
                <h4 class="mb-2 text-sm font-semibold text-gray-900 dark:text-white">How to interact:</h4>
...
const demoAriaLabel = computed(() => {
    return props.config.accessibility?.ariaLabel || `Interactive demo: ${demoTitle.value}`;
});
...
watch(
    () => props.config.demoSettings?.autoStart,
    (autoStart) => {
