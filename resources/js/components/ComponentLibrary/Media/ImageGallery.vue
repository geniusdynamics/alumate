<template>
    ...
    >
        <!-- Gallery Grid -->
        <div :class="galleryClasses" role="region" :aria-label="galleryAriaLabel">
            <div
            ...
            >
                ...
                >
                    ...
                    <template #overlay v-if="config.lightbox?.enabled">
                        <div
                            class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-0 opacity-0 transition-all duration-200 hover:bg-opacity-30 hover:opacity-100"
                        >
        ...
        <Teleport to="body">
            ...
            >
                <!-- Lightbox Content -->
                <div class="relative mx-4 max-h-full max-w-7xl" @click.stop>
                    <!-- Main Image -->
                    <div class="relative">
                        <ResponsiveImage
                            v-if="currentImage"
                        ...
                        <!-- Loading State -->
                        <div v-if="lightboxLoading" class="absolute inset-0 flex items-center justify-center">
                    ...
                    <button
                        ...
                        @click="previousImage"
                        class="absolute left-4 top-1/2 -translate-y-1/2 transform rounded-full bg-black bg-opacity-50 p-3 text-white transition-all hover:bg-opacity-70 focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50"
                    ...
                    <button
                        v-if="optimizedImages.length > 1"
                        @click="nextImage"
                        class="absolute right-4 top-1/2 -translate-y-1/2 transform rounded-full bg-black bg-opacity-50 p-3 text-white transition-all hover:bg-opacity-70 focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50"
                    ...
                    <button
                        @click="closeLightbox"
                        class="absolute right-4 top-4 rounded-full bg-black bg-opacity-50 p-2 text-white transition-all hover:bg-opacity-70 focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50"
                    ...
                    <div
                        ...
                        class="absolute bottom-4 left-1/2 -translate-x-1/2 transform rounded-full bg-black bg-opacity-50 px-4 py-2 text-sm text-white"
                    >
                    ...
                    >
                        <div class="rounded-lg bg-black bg-opacity-50 p-4 text-white">
                    ...
                    <div
                        ...
                        class="absolute bottom-16 left-1/2 flex max-w-full -translate-x-1/2 transform space-x-2 overflow-x-auto rounded-lg bg-black bg-opacity-50 p-2"
                    >
                        <button
                            v-for="(image, index) in optimizedImages"
                            :key="`thumb-${image.id}`"
...
const galleryAriaLabel = computed(() => {
    return props.config.accessibility?.ariaLabel || `Image gallery with ${optimizedImages.value.length} images`;
});
...
const generateMobileVariant = (asset: MediaAsset) => {
    ...
    const quality = props.config.optimization.compressionLevel === 'high' ? 80 : props.config.optimization.compressionLevel === 'medium' ? 70 : 60;
