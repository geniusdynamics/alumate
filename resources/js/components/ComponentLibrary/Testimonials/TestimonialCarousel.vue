<template>
    <div :id="carouselId" class="testimonial-carousel" role="region" :aria-label="`Testimonial carousel showing ${testimonials.length} testimonials`">
        <!-- Carousel Header (optional) -->
        <div v-if="config.title || config.description" class="mb-8 text-center">
            <h3 v-if="config.title" class="mb-4 text-2xl font-bold text-gray-900 dark:text-white">
                {{ config.title }}
            </h3>
        ...
        <div
            ...
            :class="{
                ...
                'bg-white shadow-lg dark:bg-gray-900': config.theme === 'card' && config.colorScheme === 'light',
            }"
        >
            <!-- Slides Container -->
            <div
            ...
            <button
                ...
                class="absolute left-4 top-1/2 z-10 -translate-y-1/2 transform rounded-full bg-white p-2 shadow-lg transition-all duration-200 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-gray-800"
            ...
            <button
                ...
                @click="nextSlide"
                :disabled="!canGoNext"
                class="absolute right-4 top-1/2 z-10 -translate-y-1/2 transform rounded-full bg-white p-2 shadow-lg transition-all duration-200 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-gray-800"
            ...
            <!-- Dots Indicator -->
            <div v-if="showDots && totalSlides > 1" class="mt-6 flex justify-center space-x-2" role="tablist" aria-label="Testimonial slides">
                <button
                    v-for="(dot, index) in totalSlides"
                    :key="`dot-${index}`"
                    @click="goToSlide(index)"
                    :class="[
                        'h-3 w-3 rounded-full transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500',
                        index === currentSlide
        ...
        <div v-if="announceUpdates" :aria-live="announceUpdates ? 'polite' : 'off'" :aria-atomic="true" class="sr-only">
...
const nextSlide = () => {
    ...
    const nextIndex =
        props.config.infinite && currentSlide.value >= totalSlides.value - 1
            ? 0
...
const previousSlide = () => {
    ...
    const prevIndex =
        props.config.infinite && currentSlide.value <= 0 ? totalSlides.value - 1 : Math.max(currentSlide.value - slidesToScroll.value, 0);
...
const getSlideAccessibilityDescription = (slideIndex: number): string => {
    ...
    return testimonialsInSlide.map((testimonial) => `${testimonial.author.name}, ${testimonial.author.title}`).join('; ');
};
