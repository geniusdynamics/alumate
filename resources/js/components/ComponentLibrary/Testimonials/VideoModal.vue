<template>
    <Teleport to="body">
        ...
        >
            ...
            <!-- Modal Content -->
            <div ref="modalContent" :class="modalClasses" role="dialog" :aria-labelledby="modalTitleId" aria-modal="true" @click.stop>
                <!-- Modal Header -->
                <header class="flex items-center justify-between border-b border-gray-700 p-4">
                    <div class="flex items-center space-x-3">
                        ...
                        <!-- Author Info -->
                        <div>
                            <h2 :id="modalTitleId" class="text-lg font-semibold text-white">
                                {{ testimonial.author.name }}
                            </h2>
                            <p class="text-sm text-gray-300">
                                {{ testimonial.author.title }}
                                <span v-if="testimonial.author.company"> at {{ testimonial.author.company }} </span>
                            </p>
                        </div>
                    </div>
                    ...
                    <button
                        @click="closeModal"
                        class="rounded-lg p-2 text-gray-400 transition-colors hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50"
                ...
                <!-- Modal Footer -->
                <footer v-if="showFooter" class="bg-gray-800 p-4">
                    <!-- Quote -->
                    <blockquote class="mb-4">
                        <p class="leading-relaxed text-gray-200">"{{ testimonial.content.quote }}"</p>
                    </blockquote>
                    ...
                    >
                        <div class="flex space-x-1">
                            <svg
                                v-for="star in 5"
                                :key="`modal-star-${star}`"
                                :class="['h-4 w-4', star <= testimonial.content.rating ? 'fill-current text-yellow-400' : 'text-gray-600']"
                                viewBox="0 0 20 20"
                                :aria-hidden="true"
                            >
                                <path
                        ...
                        <span class="ml-2 text-sm text-gray-400"> {{ testimonial.content.rating }}/5 </span>
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            ...
                            <button
                                @click="handleShare"
                                class="flex items-center space-x-2 rounded-lg bg-gray-700 px-4 py-2 text-white transition-colors hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50"
                            >
                            ...
                            <button
                                @click="handleLike"
                                :class="[
                                    'flex items-center space-x-2 rounded-lg px-4 py-2 transition-colors focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50',
                                    isLiked ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-gray-700 text-white hover:bg-gray-600',
                                ]"
                            >
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path
