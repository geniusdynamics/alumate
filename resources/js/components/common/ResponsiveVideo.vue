<template>
    <div :class="containerClasses" role="region" :aria-label="ariaLabel">
        <!-- Video Element -->
        <video
            ref="videoElement"
            :class="videoClasses"
        ...
        >
            ...
            <source v-for="source in optimizedVideoSources" :key="source.src" :src="source.src" :type="source.type" :media="source.media" />
            ...
            <!-- Chapters -->
            <track v-if="chapters" kind="chapters" :src="chapters" srclang="en" label="Chapters" />

            <!-- Fallback content -->
            <p class="p-4 text-gray-600 dark:text-gray-400">
                Your browser doesn't support HTML5 video.
                <a :href="src.url" class="text-indigo-600 hover:text-indigo-500"> Download the video </a>
            </p>
        </video>
        ...
        >
            <svg v-if="!isPlaying" class="h-12 w-12 text-white" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="showQualitySelector && availableQualities.length > 1" class="absolute right-4 top-4 z-10">
            <div class="relative">
                <button
                    @click="toggleQualityMenu"
                    class="rounded bg-black bg-opacity-70 px-3 py-1 text-sm text-white transition-all hover:bg-opacity-90"
                ...
                <div v-if="showQualityMenu" class="absolute right-0 top-full mt-1 min-w-24 rounded bg-black bg-opacity-90 shadow-lg">
                    <button
                        v-for="quality in availableQualities"
                        :key="quality.label"
                        @click="selectQuality(quality)"
                        :class="[
                            'block w-full px-3 py-2 text-left text-sm text-white transition-colors hover:bg-gray-700',
        ...
        <!-- Bandwidth Warning -->
        <div v-if="showBandwidthWarning" class="absolute bottom-16 left-4 right-4 rounded bg-yellow-600 p-3 text-sm text-white">
            <div class="flex items-center justify-between">
                <span>Slow connection detected. Switch to lower quality?</span>
                <div class="flex space-x-2">
                    <button @click="acceptLowerQuality" class="rounded bg-yellow-700 px-3 py-1 transition-colors hover:bg-yellow-800">Yes</button>
                    <button @click="dismissBandwidthWarning" class="rounded bg-yellow-700 px-3 py-1 transition-colors hover:bg-yellow-800">No</button>
                </div>
            </div>
        </div>
        ...
        <div v-if="isLoading" class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50">
            <div class="text-center text-white">
                ...
                <div v-if="loadingProgress > 0" class="mx-auto mt-2 h-2 w-32 rounded-full bg-gray-700">
                    <div class="h-2 rounded-full bg-white transition-all duration-300" :style="{ width: `${loadingProgress}%` }"></div>
                </div>
            </div>
        </div>

        <!-- Error State -->
        <div v-if="hasError" class="absolute inset-0 flex items-center justify-center bg-gray-900 p-4 text-white">
            <div class="text-center">
                <svg class="mx-auto mb-4 h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <p class="mb-2 text-sm">{{ errorMessage }}</p>
                <div class="flex justify-center space-x-2">
                    <button @click="retryLoad" class="rounded bg-indigo-600 px-4 py-2 text-white transition-colors hover:bg-indigo-700">Retry</button>
                    <button
        ...
        <div
            ...
            class="absolute inset-x-0 bottom-0 max-h-32 overflow-y-auto bg-black bg-opacity-90 p-4 text-white"
        >
            <div class="mb-2 flex items-center justify-between">
                <h4 class="text-sm font-medium">Transcript</h4>
                <button @click="toggleTranscript" class="text-gray-400 hover:text-white" aria-label="Close transcript">
...
const chapters = computed(() => {
    return props.chapters || props.src.videoAsset?.chapters?.map((chapter) => `${chapter.time} ${chapter.title}`).join('\n');
});
...
const acceptLowerQuality = () => {
    const lowerQuality = availableQualities.value.find((q) => q.label === 'Mobile') || availableQualities.value[availableQualities.value.length - 1];
...
const tryLowerQuality = () => {
    ...
    const lowerQuality = availableQualities.value[currentIndex + 1] || availableQualities.value[availableQualities.value.length - 1];
