<template>
    <div
        class="connection-modal-overlay fixed inset-0 z-[50] h-full w-full overflow-y-auto bg-black/50 backdrop-blur-sm"
        @click="handleOverlayClick"
        role="dialog"
        aria-modal="true"
    >
        <div class="connection-modal relative top-20 mx-auto w-full max-w-md rounded-md border bg-white p-5 shadow-lg">
            <!-- Modal Header -->
            <div class="modal-header flex items-center justify-between border-b border-gray-200 pb-4">
                <h3 class="text-lg font-semibold text-gray-900">Send Connection Request</h3>
                <button @click="$emit('close')" class="text-gray-400 transition-colors hover:text-gray-600 focus:text-gray-600 focus:outline-none">
            ...
            <!-- Alumni Info -->
            <div class="alumni-info border-b border-gray-200 py-4">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p v-if="alumni.current_position" class="text-sm text-gray-600">
                            ...
                            <span v-if="alumni.current_position.company"> at {{ alumni.current_position.company }} </span>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <!-- Connection Reasons -->
            <div v-if="connectionReasons.length > 0" class="connection-reasons border-b border-gray-200 py-4">
                <h5 class="mb-3 text-sm font-medium text-gray-700">Why connect with {{ alumni.name.split(' ')[0] }}?</h5>
                <div class="reasons-list space-y-2">
                    <div v-for="reason in connectionReasons" :key="reason.type" class="reason-item flex items-center text-sm text-gray-600">
            ...
            <div class="message-form py-4">
                <label for="connection-message" class="mb-2 block text-sm font-medium text-gray-700"> Add a personal message (optional) </label>
                <textarea
                    id="connection-message"
                    v-model="message"
                    rows="4"
                    :placeholder="messagePlaceholder"
                    class="w-full resize-none rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-transparent focus:ring-2 focus:ring-blue-500"
                    :maxlength="500"
                ></textarea>
                ...
                <div v-if="suggestedMessages.length > 0" class="suggested-messages mt-3">
                    <p class="mb-2 text-xs text-gray-600">Quick suggestions:</p>
                    <div class="suggestions-grid grid grid-cols-1 gap-2">
                        <button
                            v-for="suggestion in suggestedMessages"
                            :key="suggestion"
                            @click="message = suggestion"
                            class="rounded border bg-gray-50 p-2 text-left text-xs text-gray-700 transition-colors hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        >
                            "{{ suggestion }}"
                        </button>
                    </div>
            ...
            <!-- Modal Actions -->
            <div class="modal-actions flex justify-end space-x-3 border-t border-gray-200 pt-4">
                <button
                    @click="$emit('close')"
                ...
                <button
                    @click="sendRequest"
                    :disabled="sending"
                    class="rounded-md border border-transparent bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                >
    ...
    setup(props, { emit }) {
        ...
        const connectionReasons = computed(() => {
            ...
            if (props.alumni.shared_circles && props.alumni.shared_circles.length > 0) {
                const circles = props.alumni.shared_circles
                    .slice(0, 2)
                    .map((c) => c.name)
                    .join(', ');
                reasons.push({
            ...
            if (props.alumni.shared_groups && props.alumni.shared_groups.length > 0) {
                const groups = props.alumni.shared_groups
                    .slice(0, 2)
                    .map((g) => g.name)
                    .join(', ');
                reasons.push({
        ...
        const suggestedMessages = computed(() => {
            ...
            if (props.alumni.mutual_connections && props.alumni.mutual_connections.length > 0) {
                ...
                suggestions.push(
                    `Hi ${firstName}, I noticed we have ${mutualName} as a mutual connection. I'd love to connect and expand our network!`,
                );
            }
            ...
            if (props.alumni.current_position) {
                suggestions.push(
                    `Hi ${firstName}, I'm interested in your work at ${props.alumni.current_position.company}. Would love to connect and learn more about your experience!`,
                );
            }
