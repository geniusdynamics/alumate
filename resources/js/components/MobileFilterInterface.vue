    ...
    <div class="mobile-filter-interface">
        ...
        >
            <div class="flex items-center justify-between">
                ...
                <ChevronDownIcon class="h-4 w-4 transition-transform duration-200" :class="{ 'rotate-180': isExpanded }" aria-hidden="true" />
            </div>
        </button>

        <!-- Filter Panel -->
        <div v-if="isExpanded" class="filter-mobile-panel" :id="filterId" role="region" aria-label="Filter options">
            <div class="space-y-6 p-4">
                <!-- Quick Filter Chips -->
                <div v-if="quickFilters.length > 0">
                    <h3 class="mb-3 text-sm font-medium text-gray-900 dark:text-white">Quick Filters</h3>
                    <div class="flex flex-wrap gap-2">
                        <button
                            v-for="filter in quickFilters"
                            :key="filter.id"
                            @click="toggleQuickFilter(filter)"
                            class="touch-target inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors"
                            :class="
                            ...
                            "
                        >
                            <component v-if="filter.icon" :is="filter.icon" class="mr-1.5 h-4 w-4" />
                            {{ filter.label }}
                        </button>
                    </div>
                </div>
                ...
                <div v-if="showCategoryFilter">
                    <h3 class="mb-3 text-sm font-medium text-gray-900 dark:text-white">Categories</h3>
                    <div class="grid grid-cols-2 gap-2">
                        <button
                            v-for="category in categories"
                            :key="category.value"
                            @click="toggleCategory(category.value)"
                            class="touch-target flex items-center justify-center rounded-lg border p-3 text-sm transition-colors"
                            :class="
                ...
                <div v-if="showLocationFilter">
                    <h3 class="mb-3 text-sm font-medium text-gray-900 dark:text-white">Location</h3>
                    <div class="space-y-3">
                        <input
                        ...
                        >
                            ...
                            >
                                <MapPinIcon class="mr-3 h-4 w-4 text-gray-400" />
                                <div class="flex-1">
                        ...
                        <div v-if="selectedLocations.length > 0" class="flex flex-wrap gap-2">
                            <span
                                ...
                                :key="location.id"
                                class="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-1 text-xs font-medium text-blue-800 dark:bg-blue-900/20 dark:text-blue-300"
                            >
                                {{ location.name }}
                                <button
                                    @click="removeLocation(location.id)"
                                    class="ml-1.5 rounded-full p-0.5 hover:bg-blue-200 dark:hover:bg-blue-800"
                                >
                ...
                <div v-if="showDateFilter">
                    <h3 class="mb-3 text-sm font-medium text-gray-900 dark:text-white">Date Range</h3>
                    <div class="space-y-3">
                        <!-- Quick Date Options -->
                        <div class="grid grid-cols-2 gap-2">
                            <button
                                ...
                                class="touch-target rounded-lg border p-2 text-sm transition-colors"
                                :class="
                                    selectedDateOption === dateOption.value
                                        ? 'border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-300'
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                <input v-model="dateFrom" type="date" class="input-mobile text-sm" @change="handleDateChange" />
                            </div>
                            <div>
                                <label class="mb-1 block text-xs text-gray-500 dark:text-gray-400">To</label>
                                <input v-model="dateTo" type="date" class="input-mobile text-sm" @change="handleDateChange" />
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div v-for="customFilter in customFilters" :key="customFilter.key" class="space-y-3">
                        ...
                        <div v-if="customFilter.type === 'multiselect'" class="space-y-2">
                            <button
                                ...
                                class="touch-target flex w-full items-center justify-between rounded-lg border p-3 text-left transition-colors"
                                :class="
                                    isCustomFilterOptionSelected(customFilter.key, option.value)
                                        ? 'border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-300'
                                        : 'border-gray-200 bg-white text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300'
                                "
                            >
                        ...
                        <div v-else-if="customFilter.type === 'select'" class="space-y-1">
                            <button
                                ...
                                class="touch-target flex w-full items-center justify-between rounded-lg border p-3 text-left transition-colors"
                                :class="
                                    customFilter.value === option.value
                                        ? 'border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-300'
                                        : 'border-gray-200 bg-white text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300'
                                "
                            >
                                <span>{{ option.label }}</span>
                                <div v-if="customFilter.value === option.value" class="h-2 w-2 rounded-full bg-blue-600"></div>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex space-x-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                    <button @click="applyFilters" class="btn-mobile-primary flex-1" :disabled="!hasChanges">
                        Apply Filters
                        <span v-if="activeFiltersCount > 0" class="ml-1">({{ activeFiltersCount }})</span>
                    </button>
                    <button @click="clearAllFilters" class="btn-mobile-secondary px-4" :disabled="activeFiltersCount === 0">Clear</button>
                </div>
            </div>
        </div>

        <!-- Backdrop -->
        <div v-if="isExpanded" class="fixed inset-0 z-40 bg-black bg-opacity-25" @click="closeFilters"></div>
    </div>
</template>
...
const emit = defineEmits(['filters-changed', 'filters-applied', 'filters-cleared']);
...
const activeFiltersCount = computed(() => {
    ...
    count += Object.values(customFilterValues.value).filter((v) => (Array.isArray(v) ? v.length > 0 : v !== null && v !== '')).length;
    return count;
});
