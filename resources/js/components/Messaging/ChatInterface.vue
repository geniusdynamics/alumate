    ...
    <div class="chat-interface">
        <div v-if="!conversation" class="no-conversation">
            <div class="py-12 text-center">
                ...
                <h3 class="mb-2 text-lg font-medium text-gray-900 dark:text-white">Select a conversation</h3>
        ...
        <div v-else class="conversation-view">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
                            <span v-if="conversation.type === 'direct' && isUserOnline(otherParticipant?.id)"> Online </span>
                            <span v-else-if="conversation.type === 'direct' && otherParticipant?.last_seen">
                                Last seen {{ formatTime(otherParticipant.last_seen) }}
                            </span>
                            <span v-else-if="conversation.type !== 'direct'"> {{ conversation.participants?.length || 0 }} members </span>
                        ...
                        <div v-if="showMenu" class="conversation-menu">
                            ...
                            <button @click="toggleMute" class="menu-item">
                                <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
                                    <path
                                        fill-rule="evenodd"
                                        d="M9.383 3.076A1 1 0 0110 4v12a1 1 0 01-1.617.793L4.617 13H2a1 1 0 01-1-1V8a1 1 0 011-1h2.617l3.766-3.793a1 1 0 011.617.793z"
                                        clip-rule="evenodd"
                                    />
                                </svg>
            ...
            <div class="messages-area" ref="messagesContainer">
                ...
                <div v-else-if="messages.length === 0" class="empty-messages">
                    <div class="py-8 text-center">
                        ...
                        <p class="mt-1 text-sm text-gray-400 dark:text-gray-500">Start the conversation!</p>
                    </div>
                </div>

                <div v-else class="messages-list">
                    <!-- Load more messages button -->
                    <div v-if="hasMoreMessages" class="py-4 text-center">
                        <button @click="loadMoreMessages" :disabled="loadingMoreMessages" class="btn btn-secondary btn-sm">
            ...
            <div class="message-input-area">
                <div v-if="replyingTo" class="reply-preview">
                    <div class="flex items-center justify-between border-l-4 border-blue-500 bg-gray-50 p-3 dark:bg-gray-800">
                        <div class="flex-1">
                            <p class="text-sm font-medium text-gray-900 dark:text-white">Replying to {{ replyingTo.user.name }}</p>
                ...
                <div class="message-input">
                    <div class="flex items-end space-x-3 p-4">
                        ...
                        <button
                            ...
                            class="flex-shrink-0 rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-700 dark:hover:text-gray-300"
                            title="Attach file"
                        >
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path
                        ...
                        <div class="flex-1">
                            <textarea
                                ...
                                rows="1"
                                class="w-full resize-none rounded-lg border border-gray-300 px-3 py-2 focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400"
                                :disabled="sending"
                            ></textarea>
                        </div>
        ...
        <!-- Modals -->
        <MessageSearchModal v-if="showSearchModal" :conversation="conversation" @close="showSearchModal = false" />

        <ParticipantsModal v-if="showParticipantsModal" :conversation="conversation" @close="showParticipantsModal = false" />
    </div>
</template>
...
const shouldShowAvatar = (message, index) => {
    ...
    return prevMessage.user.id !== message.user.id || new Date(message.created_at) - new Date(prevMessage.created_at) > 300000; // 5 minutes
};
...
.no-conversation {
    @apply flex flex-1 items-center justify-center bg-gray-50 dark:bg-gray-900;
}
...
.conversation-menu {
    @apply absolute right-0 top-full z-10 mt-2 w-48 rounded-lg border border-gray-200 bg-white py-1 shadow-lg dark:border-gray-700 dark:bg-gray-800;
}

.menu-item {
    @apply flex w-full items-center space-x-2 px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700;
}
