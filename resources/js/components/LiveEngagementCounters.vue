<template>
    <div class="live-engagement-counters">
        ...
        <div class="flex items-center space-x-4">
            ...
            <button
                @click="handleLike"
                class="flex items-center space-x-2 rounded-full px-3 py-1 transition-all duration-200 hover:bg-gray-100"
                :class="{ 'bg-red-50 text-red-500': userHasLiked }"
                :disabled="isLoading"
            >
                <HeartIcon :class="['h-5 w-5 transition-all duration-200', userHasLiked ? 'fill-current text-red-500' : 'text-gray-500']" />
                <span class="text-sm font-medium">
                    {{ formatCount(engagementCounts.likes) }}
                </span>

                <!-- Live update indicator -->
                <div v-if="recentLikes > 0" class="animate-bounce text-xs font-bold text-red-500">+{{ recentLikes }}</div>
            </button>
            ...
            <button
                ...
                :disabled="isLoading"
            >
                <ChatBubbleLeftIcon class="h-5 w-5 text-gray-500" />
                <span class="text-sm font-medium">
                    {{ formatCount(engagementCounts.comments) }}
                </span>

                <!-- Live update indicator -->
                <div v-if="recentComments > 0" class="animate-bounce text-xs font-bold text-blue-500">+{{ recentComments }}</div>
            </button>
            ...
            >
                ...
                <!-- Live update indicator -->
                <div v-if="recentShares > 0" class="animate-bounce text-xs font-bold text-green-500">+{{ recentShares }}</div>
            </button>
            ...
            <div v-if="showReactions" class="relative">
                <button
                    ...
                    :disabled="isLoading"
                >
                    <FaceSmileIcon class="h-5 w-5 text-gray-500" />
                    <span class="text-sm font-medium">
                ...
                <!-- Reactions Popup -->
                <div v-if="showReactionsPicker" class="absolute bottom-full left-0 z-10 mb-2 rounded-lg border bg-white p-2 shadow-lg">
        ...
        <div v-if="showRecentActivity && recentEngagements.length > 0" class="mt-3">
            <div class="mb-2 text-xs text-gray-500">Recent Activity:</div>
            <div class="space-y-1">
                <div
                    v-for="engagement in recentEngagements.slice(0, 3)"
                    :key="`${engagement.user.id}-${engagement.timestamp}`"
                    class="animate-fade-in flex items-center space-x-2 text-xs text-gray-600"
                >
                    <img :src="engagement.user.avatar_url" :alt="engagement.user.name" class="h-4 w-4 rounded-full" />
        ...
        <div v-if="showLiveUsers && liveUsers.length > 0" class="mt-2">
            <div class="flex items-center space-x-2">
                ...
                <span class="text-xs text-gray-500"> {{ liveUsers.length }} {{ liveUsers.length === 1 ? 'person' : 'people' }} viewing </span>
...
const handleReaction = async (reactionType: string) => {
    ...
    try {
        await router.post(
            `/api/posts/${props.postId}/reaction`,
            {
                type: reactionType,
            },
            {
