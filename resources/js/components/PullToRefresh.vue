        ...
        <div v-if="showIndicator" class="pull-to-refresh" :style="{ transform: `translateY(${Math.max(0, pullDistance - 60)}px)` }">
            <div class="pull-to-refresh-content">
                ...
                <div v-else class="flex items-center space-x-2 text-blue-600 dark:text-blue-400">
                    <ArrowDownIcon class="h-5 w-5 transition-transform duration-200" :class="{ 'rotate-180': pullDistance > threshold }" />
...
const { container, isRefreshing, pullDistance, showIndicator, indicatorText, triggerRefresh, resetPull } = usePullToRefresh({
