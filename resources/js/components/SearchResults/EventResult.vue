    ...
    <div class="event-result">
        ...
        <div v-if="attendingAlumni.length > 0" class="attending-alumni">
            ...
            <div class="alumni-list">
                <div v-for="alumnus in attendingAlumni.slice(0, 4)" :key="alumnus.id" class="alumnus-item">
                    <img v-if="alumnus.avatar_url" :src="alumnus.avatar_url" :alt="`${alumnus.name}'s avatar`" class="alumnus-avatar" />
                    <div v-else class="alumnus-avatar-placeholder">
                        {{ getInitials(alumnus.name) }}
                    </div>
                    <span class="alumnus-name">{{ alumnus.name }}</span>
                </div>
                <div v-if="attendingAlumni.length > 4" class="more-alumni">+{{ attendingAlumni.length - 4 }} more</div>
            </div>
        </div>

        <div class="event-actions">
            <button @click="viewEvent" class="action-btn primary">
            ...
            <button
                ...
                :disabled="isRSVPing"
            >
                <CheckIcon v-if="rsvpStatus === 'attending' && !isRSVPing" class="h-4 w-4" />
                <CalendarIcon v-else-if="!isRSVPing" class="h-4 w-4" />
                <LoadingSpinner v-else class="h-4 w-4" />
                {{ getRSVPButtonText() }}
            </button>

            <button @click="shareEvent" class="action-btn secondary">
                <ShareIcon class="h-4 w-4" />
                Share
            </button>

            <button @click="saveEvent" class="action-btn secondary" :class="{ saved: isSaved }" :disabled="isSaving">
...
const truncateDescription = computed(() => {
    ...
    return props.event.description.length > 200 ? props.event.description.substring(0, 200) + '...' : props.event.description;
});
...
const rsvpToEvent = async () => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const message = data.status === 'attending' ? 'RSVP confirmed! See you at the event.' : 'RSVP cancelled.';
...
.event-type {
    @apply rounded bg-purple-100 px-2 py-1 text-xs text-purple-800;
}
...
.tag {
    @apply inline-block rounded bg-gray-100 px-2 py-1 text-xs text-gray-700;
}

.attending-alumni {
    @apply space-y-3 rounded-lg border border-purple-200 bg-purple-50 p-4;
}

.alumni-header {
    @apply flex items-center space-x-2 font-medium text-purple-700;
}
...
.alumnus-avatar-placeholder {
    @apply flex h-6 w-6 items-center justify-center rounded-full bg-purple-200;
    @apply text-xs font-medium text-purple-700;
}
