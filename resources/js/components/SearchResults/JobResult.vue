    ...
    <div class="job-result">
        <div class="job-header">
            ...
            <div class="job-actions-header">
                ...
                <div v-if="job.posted_date" class="posted-date">Posted {{ formatDate(job.posted_date) }}</div>
            </div>
        </div>

        <div class="job-description">
            <p v-html="highlightText(truncateDescription(job.description), highlight.description)"></p>
            <button v-if="job.description && job.description.length > 200" @click="showFullDescription = !showFullDescription" class="show-more-btn">
        ...
        <div v-if="networkConnections.length > 0" class="network-connections">
            ...
            <div class="connections-list">
                <div v-for="connection in networkConnections.slice(0, 3)" :key="connection.id" class="connection-item">
                    <img v-if="connection.avatar_url" :src="connection.avatar_url" :alt="`${connection.name}'s avatar`" class="connection-avatar" />
                ...
                <div v-if="networkConnections.length > 3" class="more-connections">+{{ networkConnections.length - 3 }} more alumni</div>
            </div>
        </div>

        <div class="job-actions">
            <button @click="viewJob" class="action-btn primary">
                <EyeIcon class="h-4 w-4" />
                View Details
            </button>

            <button @click="applyToJob" class="action-btn primary" :disabled="isApplying">
                <PaperAirplaneIcon v-if="!isApplying" class="h-4 w-4" />
                <LoadingSpinner v-else class="h-4 w-4" />
                Apply Now
            </button>

            <button v-if="networkConnections.length > 0" @click="requestIntroduction" class="action-btn secondary" :disabled="isRequestingIntro">
                <UserPlusIcon v-if="!isRequestingIntro" class="h-4 w-4" />
                <LoadingSpinner v-else class="h-4 w-4" />
                Request Introduction
            </button>

            <button @click="saveJob" class="action-btn secondary" :class="{ saved: isSaved }" :disabled="isSaving">
                <BookmarkIcon v-if="!isSaving" class="h-4 w-4" />
                <LoadingSpinner v-else class="h-4 w-4" />
                {{ isSaved ? 'Saved' : 'Save' }}
            </button>
        </div>
...
const truncateDescription = computed(() => {
    ...
    return props.job.description.length > 200 ? props.job.description.substring(0, 200) + '...' : props.job.description;
});
...
.employment-type {
    @apply rounded bg-green-100 px-2 py-1 text-xs text-green-800;
}
...
.skill-tag {
    @apply inline-block rounded bg-blue-100 px-2 py-1 text-xs text-blue-800;
}

.network-connections {
    @apply space-y-3 rounded-lg border border-blue-200 bg-blue-50 p-4;
}

.connections-header {
    @apply flex items-center space-x-2 font-medium text-blue-700;
}
...
.connection-avatar-placeholder {
    @apply flex h-8 w-8 items-center justify-center rounded-full bg-blue-200;
...
.job-actions {
    @apply flex items-center space-x-2 border-t border-gray-200 pt-3;
}

.action-btn {
    @apply flex items-center space-x-1 rounded-md px-3 py-2 text-sm;
...
.action-btn.secondary.saved {
    @apply border-green-300 bg-green-50 text-green-600;
}
