<template>
    <div class="conversion-chart rounded-lg bg-white p-6 shadow dark:bg-gray-800">
        <!-- Chart Header -->
        ...
        <div class="chart-container">
            ...
            <template v-if="data.length > 0">
                <div v-if="chartType === 'line'" class="line-chart" role="img" :aria-label="`${title} line chart showing ${data.length} data points`">
                    <svg :width="svgWidth" :height="svgHeight" class="h-full w-full">
                        <!-- Grid lines -->
                        <defs>
                        ...
                        <circle
                            ...
                            stroke="white"
                            stroke-width="2"
                            class="cursor-pointer transition-colors hover:stroke-gray-400"
                        ...
                        <g v-for="(point, index) in data" :key="`tooltip-${index}`" class="hidden">
                            <rect
                                ...
                                opacity="0.9"
                            />
                            <text :x="getXCoordinate(index)" :y="getYCoordinate(point.rate) - 25" text-anchor="middle" fill="white" font-size="12">
                                {{ point.rate }}%
                            </text>
                        </g>
                    </svg>
                ...
                <div
                    ...
                    class="bar-chart"
                    role="img"
                    :aria-label="`${title} bar chart showing ${data.length} data points`"
                >
                    <div class="flex h-80 w-full items-end justify-around">
                        <div
                            v-for="(point, index) in data"
                            :key="`bar-${index}`"
                            class="mx-1 flex flex-1 flex-col items-center"
                            :style="{ height: '100%' }"
                        >
                            <!-- Bar -->
                            <div
                                class="bar cursor-pointer rounded-t bg-blue-600 transition-colors hover:bg-blue-700"
                                :style="{
                ...
                <div
                    ...
                    class="area-chart"
                    role="img"
                    :aria-label="`${title} area chart showing ${data.length} data points`"
                >
                    <svg :width="svgWidth" :height="svgHeight" class="h-full w-full">
                        <!-- Grid -->
                        <defs>
                ...
                <div v-else class="combined-chart" role="img" :aria-label="`${title} combined chart showing conversions and views`">
                    <svg :width="svgWidth" :height="svgHeight" class="h-full w-full">
                        ...
                        <path :d="rateLinePath" fill="none" stroke="#3b82f6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
            ...
            <!-- Empty State -->
            <div v-else class="empty-state flex h-80 flex-col items-center justify-center text-gray-500 dark:text-gray-400">
    ...
    <div class="mt-6 border-t border-gray-200 pt-6 dark:border-gray-700">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div class="text-center">
                <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ formatNumber(averageRate) }}%</div>
                <div class="text-sm text-gray-600 dark:text-gray-400">Average Rate</div>
            </div>
            <div class="text-center">
            ...
            <div class="text-center">
                <div :class="trendClass" class="flex items-center justify-center gap-1 text-2xl font-bold">
                    <svg v-if="trendDirection === 'up'" class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 17l9.2-9.2M17 17V7H7"></path>
                    </svg>
                    <svg v-else-if="trendDirection === 'down'" class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 7l-9.2 9.2M7 7v10h10"></path>
                    </svg>
                    {{ formatNumber(Math.abs(trendValue)) }}%
                </div>
                <div class="text-sm text-gray-600 dark:text-gray-400">7-Day Trend</div>
            </div>
        </div>
    </div>
    ...
    <div id="chart-description" class="sr-only">
        This chart displays conversion rate data over time. The x-axis represents dates and the y-axis represents conversion percentages. Current
        average conversion rate is {{ averageRate }}% with a {{ trendDirection }}ward trend of {{ Math.abs(trendValue) }}%.
    </div>
</template>
