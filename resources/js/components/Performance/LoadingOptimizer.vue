    ...
    <div class="loading-optimizer">
        ...
        <div v-if="showHints && performanceHints.length > 0" class="performance-hints">
            ...
            <div class="hints-list">
                <div v-for="hint in performanceHints" :key="hint.id" class="hint-item">
                    {{ hint.message }}
                </div>
            </div>
        </div>
...
.progress-bar {
    @apply h-2 w-48 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700;
}
...
.performance-hints {
    @apply mt-4 rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-800 dark:bg-blue-900/20;
}

.hints-header {
    @apply mb-2 flex items-center space-x-2 text-blue-700 dark:text-blue-300;
}
