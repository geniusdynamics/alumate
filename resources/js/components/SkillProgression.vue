<template>
    <div class="skill-progression">
        ...
        <div class="skill-selector mb-6">
            <label class="mb-2 block text-sm font-medium text-gray-700">Select Skill to Track</label>
        ...
        <div v-if="loading" class="loading-state">
            <div class="animate-pulse">
                <div class="mb-4 h-64 rounded-lg bg-gray-200"></div>
                <div class="space-y-3">
        ...
        <div v-else-if="progressionData" class="progression-content">
            ...
            <div class="skill-overview">
                <div class="skill-header">
                    ...
                    <div class="skill-badges">
                        ...
                        <span class="experience-badge"> {{ progressionData.years_experience }} years </span>
                    </div>
                </div>
            ...
            <div class="progression-milestones">
                ...
                <div class="milestones-timeline">
                    <div v-for="milestone in milestones" :key="milestone.id" class="milestone-item" :class="{ achieved: milestone.achieved }">
                        <div class="milestone-icon">
                            <Icon :name="milestone.icon" class="h-5 w-5" />
                        </div>
                        <div class="milestone-content">
                            ...
                            <div class="milestone-date" v-else>Achieved {{ formatDate(milestone.achieved_at) }}</div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="recommendations-grid">
                    <div v-for="recommendation in recommendations" :key="recommendation.id" class="recommendation-card">
                        <div class="recommendation-type">
                            <Icon :name="getRecommendationIcon(recommendation.type)" class="h-5 w-5" />
                            {{ recommendation.type }}
                        </div>
                        <h6 class="recommendation-title">{{ recommendation.title }}</h6>
                        <p class="recommendation-description">{{ recommendation.description }}</p>
                        <div class="recommendation-actions">
                            <a :href="recommendation.url" target="_blank" class="btn-primary btn-sm"> Start Learning </a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="comparison-chart">
                    <div class="comparison-item">
                        ...
                        <div class="comparison-bar">
                            <div class="comparison-fill your-level" :style="{ width: getUserLevelPercentage() + '%' }"></div>
                        </div>
                    ...
                    <div class="comparison-item">
                        ...
                        <div class="comparison-bar">
                            <div class="comparison-fill industry-average" :style="{ width: '65%' }"></div>
                        </div>
                    ...
                    <div class="comparison-item">
                        ...
                        <div class="comparison-bar">
                            <div class="comparison-fill top-performers" :style="{ width: '90%' }"></div>
                        </div>
        ...
        <div v-else-if="selectedSkillId" class="empty-state">
            <Icon name="chart-bar" class="mx-auto mb-4 h-12 w-12 text-gray-400" />
            <p class="text-center text-gray-600">No progression data available for this skill yet.</p>
        </div>
    </div>
</template>
...
.experience-badge {
    @apply rounded-full bg-orange-100 px-3 py-1 text-sm font-medium text-orange-700;
}
...
.chart-container {
    @apply relative h-64 rounded-lg bg-gray-50 p-4;
}
...
.chart-point {
    @apply absolute -translate-x-1/2 transform;
}

.point-dot {
    @apply h-3 w-3 rounded-full bg-blue-500;
}

.point-label {
    @apply mt-2 -translate-x-1/2 transform text-xs text-gray-600;
}

.point-value {
    @apply -mt-6 -translate-x-1/2 transform text-xs font-medium text-blue-600;
}
...
.milestone-item {
    @apply flex items-start space-x-4 rounded-lg p-4;
    @apply bg-gray-50;
}
...
.recommendation-type {
    @apply mb-2 flex items-center space-x-2 text-sm font-medium text-blue-600;
}
...
.comparison-bar {
    @apply h-6 flex-1 overflow-hidden rounded-full bg-gray-200;
}
