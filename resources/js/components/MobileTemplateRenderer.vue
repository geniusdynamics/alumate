    ...
    <div class="mobile-template-renderer">
        ...
        <header class="mobile-header">
            <div class="header-content">
                <button @click="toggleDeviceMenu" class="device-toggle-btn" :aria-expanded="deviceMenuOpen" aria-label="Switch device view">
                    ...
                    >
                        <svg class="orientation-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path
                                ...
                                stroke-width="2"
                                :d="isLandscape ? 'M6 14l6-6 6 6' : 'M18 10l-6 6-6-6'"
                            />
                        </svg>
                    </button>
        ...
        <footer class="mobile-controls-footer">
            <div class="controls-grid">
                <button @click="zoomIn" :disabled="zoomLevel >= maxZoom" class="control-btn zoom-in" aria-label="Zoom in">
                    <svg class="control-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                    </svg>
                </button>

                <button @click="resetZoom" :disabled="zoomLevel === 1" class="control-btn reset-zoom" aria-label="Reset zoom">
                    <span class="zoom-level">{{ Math.round(zoomLevel * 100) }}%</span>
                </button>

                <button @click="zoomOut" :disabled="zoomLevel <= minZoom" class="control-btn zoom-out" aria-label="Zoom out">
                    <svg class="control-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
.mobile-template-renderer {
    @apply flex h-full flex-col bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800;
}

/* Mobile Header */
.mobile-header {
    @apply border-b border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-slate-800;
}
...
.device-toggle-btn {
    ...
    @apply transition-colors hover:bg-slate-200 dark:hover:bg-slate-600;
...
/* Device Menu */
.device-menu {
    @apply mt-2 rounded-lg border border-slate-200 bg-white py-2 shadow-lg dark:border-slate-700 dark:bg-slate-800;
}

.device-option {
    @apply flex w-full items-center gap-3 px-4 py-3 text-sm;
    @apply transition-colors hover:bg-slate-50 dark:hover:bg-slate-600;
    @apply focus:ring-2 focus:ring-blue-500 focus:ring-offset-2;
}
...
.device-option-icon {
    @apply h-5 w-5 text-slate-500 dark:text-slate-400;
}
...
/* Mobile Preview Container */
.mobile-preview-container {
    @apply flex flex-1 items-center justify-center overflow-hidden p-6;
}

.device-frame {
    @apply rounded-lg border-4 border-slate-300 bg-black p-2 dark:border-slate-600;
    @apply transform shadow-2xl transition-transform duration-300;
}

.screen-content {
    @apply h-full w-full overflow-auto rounded bg-white dark:bg-slate-50;
...
.error-title {
    @apply mb-2 text-xl font-semibold text-slate-900 dark:text-slate-100;
}

.error-message {
    @apply mb-4 text-slate-600 dark:text-slate-400;
}
...
.empty-icon {
    @apply mb-4 h-16 w-16 text-slate-400;
}
...
/* Touch Instructions */
.touch-instructions {
    @apply absolute bottom-4 left-1/2 z-20 -translate-x-1/2 transform rounded-lg bg-slate-800 px-4 py-3 text-white shadow-lg;
}
...
.dismiss-btn {
    @apply rounded bg-blue-600 px-3 py-1 text-xs transition-colors hover:bg-blue-700;
}

/* Mobile Controls Footer */
.mobile-controls-footer {
    @apply border-t border-slate-200 bg-white px-6 py-4 dark:border-slate-700 dark:bg-slate-800;
}
...
.control-btn {
    @apply rounded-lg p-3 transition-colors disabled:cursor-not-allowed disabled:opacity-50;
    @apply hover:bg-slate-100 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:hover:bg-slate-700;
}
...
/* Keyboard Shortcuts Mobile */
.keyboard-shortcuts-mobile {
    @apply fixed bottom-0 left-0 right-0 z-30 border-t border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800;
    max-height: 80vh;
    overflow-y: auto;
}

.shortcuts-header {
    @apply flex items-center justify-between border-b border-slate-200 p-4 dark:border-slate-700;
}
...
.close-shortcuts-btn {
    @apply rounded-lg p-2 transition-colors hover:bg-slate-100 dark:hover:bg-slate-700;
}
...
.shortcut-key {
    @apply inline-flex items-center rounded bg-slate-200 px-2 py-1 text-xs text-slate-700 dark:bg-slate-600 dark:text-slate-300;
}
