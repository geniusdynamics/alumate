...
export class ComponentSerializer {
    ...
    private generateHeroHTML(component: Component, config: HeroComponentConfig): string {
        ...
        const ctaButtons = config.ctaButtons
            .map((cta) => `<a href="${cta.url}" class="cta-button cta-${cta.style} cta-${cta.size}">${cta.text}</a>`)
            .join('');
      ...
      <div class="testimonial-item" data-testimonial-id="${testimonial.id}">
        ...
        <div class="testimonial-author">
          ...
          <div class="author-info">
            ...
            ${config.showAuthorTitle && testimonial.author.title ? `<div class="author-title">${testimonial.author.title}</div>` : ''}
          </div>
        </div>
      </div>
    `,
    ...
    private generateMediaHTML(component: Component, config: MediaComponentConfig): string {
        const mediaItems = config.mediaAssets
            .slice(0, 6)
            .map((asset, index) => {
                if (asset.type === 'image') {
