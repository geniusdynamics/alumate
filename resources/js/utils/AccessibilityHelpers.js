...
export class FocusManager {
    ...
    isVisible(element) {
        ...
        return (
            style.display !== 'none' && style.visibility !== 'hidden' && style.opacity !== '0' && element.offsetWidth > 0 && element.offsetHeight > 0
        );
    }
     ...
     */
    isDisabled(element) {
        return element.disabled || element.getAttribute('aria-disabled') === 'true' || element.getAttribute('tabindex') === '-1';
    }
    ...
    createSkipLink(targetId, text = 'Skip to main content') {
        ...
        skipLink.className =
            'skip-link sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-50 focus:px-4 focus:py-2 focus:bg-blue-600 focus:text-white focus:rounded-lg focus:shadow-lg';
...
export class MobileAccessibilityManager {
    ...
    static addSwipeAlternative(element, onSwipe, direction = 'horizontal') {
        ...
        const handleKeyDown = (e) => {
            const keys = direction === 'horizontal' ? ['ArrowLeft', 'ArrowRight'] : ['ArrowUp', 'ArrowDown'];
