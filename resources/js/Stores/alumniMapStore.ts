...
export const useAlumniMapStore = defineStore('alumniMap', () => {
    ...
    const filteredAlumni = computed(() => {
        return alumni.value.filter((alumnus) => {
            // Apply filters
            if (filters.value.graduation_year.length > 0 && !filters.value.graduation_year.includes(alumnus.graduation_year)) {
                return false;
            }

            if (filters.value.industry.length > 0 && !filters.value.industry.includes(alumnus.industry || '')) {
                return false;
            }

            if (filters.value.country.length > 0 && !filters.value.country.includes(alumnus.country || '')) {
                return false;
            }

            if (filters.value.state.length > 0 && !filters.value.state.includes(alumnus.state || '')) {
                return false;
            }
            ...
            if (filters.value.search && filters.value.search.length > 0) {
                ...
                if (!fullName.includes(searchTerm) && !company.includes(searchTerm) && !position.includes(searchTerm)) {
                    return false;
                }
            }
