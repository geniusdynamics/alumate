...
export class TailwindStyleManager {
  ...
  private isApprovedColor(color: string): boolean {
    return this.brandColors.some(brandColor => 
      brandColor.value.toLowerCase() === color.toLowerCase()
    )
  }
  ...
  private isApprovedFont(fontFamily: string): boolean {
    return this.brandFonts.some(brandFont => 
      fontFamily.includes(brandFont.name)
    )
  }
