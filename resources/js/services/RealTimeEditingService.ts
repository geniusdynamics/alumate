...
export class RealTimeEditingService {
  ...
  public currentDevice = computed(() => 
    this.devices.find(d => d.id === this.editingState.deviceMode) || this.devices[0]
  )
  ...
  private triggerLivePreviewUpdate(): void {
    ...
    window.dispatchEvent(new CustomEvent('grapejs:preview-update', {
  ...
  private async autoSave(): Promise<void> {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('grapejs:auto-save', {
        detail: data
      }))
  ...
  public async save(): Promise<void> {
    ...
    try {
      ...
      // Emit save event
      window.dispatchEvent(new CustomEvent('grapejs:save', {
        detail: data
      }))
