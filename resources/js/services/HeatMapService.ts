...
export class HeatMapService {
    ...
    private checkBatchSize(): void {
        const totalData = this.clickData.length + this.scrollData.length + this.timeSpentData.length + this.ctaPerformanceData.length;
    ...
    private getCTAInfo(element: Element): {
        ...
        const ctaId = element.getAttribute('data-cta') || element.getAttribute('id') || element.className || 'unknown';

        const ctaText = element.textContent?.trim() || element.getAttribute('aria-label') || element.getAttribute('title') || '';

        const ctaType = element.tagName.toLowerCase() === 'button' ? 'button' : element.tagName.toLowerCase() === 'a' ? 'link' : 'other';
    ...
    private async flushData(): Promise<void> {
        if (this.clickData.length === 0 && this.scrollData.length === 0 && this.timeSpentData.length === 0 && this.ctaPerformanceData.length === 0) {
            return;
        }
