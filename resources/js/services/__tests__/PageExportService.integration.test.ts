...
import { PageExportService, pageExportService } from '../PageExportService'
...
describe('PageExportService - Integration Tests', () => {
  ...
  describe('Single Page Export', () => {
    ...
    it('rejects export when page belongs to different tenant', async () => {
      ...
      await expect(exportService.exportPage('page-1', 'tenant-1')).rejects.toThrow('Access denied: Page does not belong to the specified tenant')
    })
  ...
  describe('Bulk Page Export', () => {
    ...
    it('rejects bulk export when any page belongs to different tenant', async () => {
      ...
      await expect(exportService.exportPages(['page-1', 'page-2'], 'tenant-1')).rejects.toThrow('Access denied: Some pages do not belong to the specified tenant')
    })
  })
  ...
  describe('Export History and Retrieval', () => {
    ...
    it('verifies tenant ownership when retrieving export', async () => {
      ...
      await expect(exportService.getExport('export-1', 'tenant-1')).rejects.toThrow('Access denied: Export record does not belong to the specified tenant')
    })
  })
  ...
  describe('Export Deletion', () => {
    it('successfully deletes export record', async () => {
      ...
      expect(mockedFetch).toHaveBeenCalledWith('/api/exports/export-1', expect.objectContaining({
        method: 'DELETE'
      }))
    })
    ...
    it('verifies ownership before deletion', async () => {
      ...
      await expect(exportService.deleteExport('export-1', 'tenant-1')).rejects.toThrow('Access denied: Export record does not belong to the specified tenant')
    })
  })
  ...
  describe('Export Download', () => {
    ...
    it('verifies ownership before download', async () => {
      ...
      await expect(exportService.downloadExport('export-1', 'tenant-1')).rejects.toThrow('Access denied: Export does not belong to the specified tenant')
    })
  })
