...
class TemplateService {
    ...
    async fetchTemplates(params: TemplateSearchParams): Promise<TemplateCollection> {
        ...
        const response = await httpService.get<TemplateCollection>(`${this.baseUrl}?${queryParams.toString()}`);
    ...
    async searchTemplates(query: string, filters?: Partial<TemplateFilterOptions>): Promise<Template[]> {
        ...
        const response = await httpService.get<{ data: Template[] }>(`${this.baseUrl}/search?${queryParams.toString()}`);
    ...
    async fetchCategories(): Promise<{ value: string; label: string; count: number }[]> {
        const response = await httpService.get<{ data: Array<{ value: string; label: string; count: number }> }>(`${this.baseUrl}/categories`);
        return response.data.data;
    }
    ...
    async fetchTags(): Promise<{ value: string; label: string; count: number }[]> {
        const response = await httpService.get<{ data: Array<{ value: string; label: string; count: number }> }>(`${this.baseUrl}/tags`);
        return response.data.data;
    }
     ...
     */
    async generateResponsivePreview(
        templateId: number | string,
        customConfig: any = {},
    ): Promise<{
        responsivePreviews: {
    ...
    async toggleFavorite(templateId: number): Promise<{ is_favorited: boolean }> {
        const response = await httpService.post<{ is_favorited: boolean }>(`${this.baseUrl}/${templateId}/favorite`);
        return response.data;
    }
    ...
    async fetchRecentlyUsed(limit: number = 10): Promise<Template[]> {
        const response = await httpService.get<{ data: Template[] }>(`${this.baseUrl}/recent?limit=${limit}`);
        return response.data.data;
    }
