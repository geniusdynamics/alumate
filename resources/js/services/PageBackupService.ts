...
export class PageBackupService {
    ...
    async scheduleBackup(
        tenantId: string,
        config: Omit<BackupSchedule, 'id' | 'status' | 'createdAt' | 'updatedAt' | 'createdBy' | 'tenantId'>,
    ...
    private async processBackupJob(jobId: string): Promise<void> {
        ...
        try {
            ...
            const targets = backupJob.options?.targets || [{ type: 'all_pages' }, { type: 'all_templates' }, { type: 'component_library' }];
     ...
     * Helper methods
     */
    private async validateBackupSchedule(
        config: Omit<BackupSchedule, 'id' | 'status' | 'createdAt' | 'updatedAt' | 'createdBy' | 'tenantId'>,
    ): Promise<void> {
    ...
    private async updateRestoreRecord(
        restoreId: string,
        updates: Partial<RestoreResult & { status: string; completedAt?: Date; error?: string }>,
    ): Promise<void> {
        try {
