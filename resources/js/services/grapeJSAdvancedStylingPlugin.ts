...
const advancedStylingPlugin: Plugin<AdvancedStylingPluginOptions> = (editor: Editor, options = {}) => {
      ...
      properties: [
        {
          ...
          defaults: 'sans-serif',
          options: opts.brandFonts?.map(font => ({
            value: font,
            name: font.split(',')[0]
          })) || [],
          onChange: (property: any) => {
      ...
      properties: [
        {
          ...
          options: opts.brandColors?.map((color, index) => ({
            value: color,
  ...
  function updateTailwindClass(property: any, cssProperty: string): void {
    ...
    if (tailwindClass) {
      ...
      const filteredClasses = existingClasses.filter((cls: string) => 
        !cls.startsWith(getTailwindPrefixForProperty(cssProperty))
      )
