...
export class CustomCodeValidationService {
   ...
   */
  private async validateSyntax(code: string, type: CodeType): Promise<{
    errors: ValidationError[]
    warnings: ValidationWarning[]
  }> {
  ...
  private isValidCSSProperty(property: string): boolean {
    const validProperties = [
      ...
      'scroll-behavior', 'shape-image-threshold', 'shape-margin', 'shape-outside', 'tab-size',
      ...
      'text-shadow', 'text-transform', 'text-underline-offset', 'text-underline-position', 'top',
  ...
  private validateJavaScript(code: string): {
    ...
    lines.forEach((line, lineIndex) => {
      ...
      if (trimmedLine && !trimmedLine.endsWith(';') && !trimmedLine.endsWith('{') &&
          ...
          !trimmedLine.startsWith('while') && !trimmedLine.startsWith('function') &&
          !trimmedLine.startsWith('class') && !trimmedLine.includes('console.log(')) {
        warnings.push({
          line: lineNumber,
          column: line.length,
   ...
   */
  async lintCode(code: string, type: CodeType): Promise<{
    isValid: boolean
   ...
   */
  async analyzeCode(code: string, type: CodeType): Promise<{
    complexity: number
