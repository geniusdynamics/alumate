...
class TemplateSearchIndex {
    ...
    search(
        ...
        if (filters?.category) {
            filteredTemplates = filteredTemplates.filter((item) => item.template.category === filters.category);
        }

        if (filters?.audienceType) {
            filteredTemplates = filteredTemplates.filter((item) => item.template.audienceType === filters.audienceType);
        }

        if (filters?.campaignType) {
            filteredTemplates = filteredTemplates.filter((item) => item.template.campaignType === filters.campaignType);
        }
...
class TemplateUsageTracker {
    ...
    trackTemplateConfiguration(templateId: number, config: any): void {
        ...
        if (stats) {
            ...
            stats.popularConfigurations = stats.popularConfigurations.sort((a, b) => b.count - a.count).slice(0, 10);
    ...
    private updateRecentUsageCounts(): void {
        ...
        this.usageStats.forEach((stats, templateId) => {
            const recentCount = this.recentUsage.filter((usage) => usage.templateId === templateId && usage.timestamp > sevenDaysAgo).length;
