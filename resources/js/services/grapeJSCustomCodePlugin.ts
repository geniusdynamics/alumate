...
function addCustomCodeCommands(editor: grapesjs.Editor, options: CustomCodePluginOptions) {
    ...
    Commands.add('validate-custom-code', {
        async run(editor) {
            ...
            const customCodeComponents = components.filter((comp: any) => comp.get('type')?.startsWith('custom-'));
    ...
    Commands.add('execute-custom-javascript', {
        async run(editor) {
            const components = editor.getComponents();
            const jsComponents = components.filter((comp: any) => comp.get('type') === 'custom-javascript');
