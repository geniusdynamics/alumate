<template>
    <div class="success-metrics-tracking rounded-lg bg-white p-6 shadow-lg">
        <!-- Header -->
        <div class="mb-6 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2">
                ...
                >
                    <svg class="mr-2 h-4 w-4" :class="{ 'animate-spin': isRefreshing }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path
                ...
                <div class="text-xs text-gray-500">Last updated: {{ formatLastUpdated(lastUpdated) }}</div>
            </div>
        </div>
        ...
        <div class="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            <div
                ...
                :key="metric.id"
                class="metric-card rounded-lg border border-gray-200 bg-gradient-to-br from-white to-gray-50 p-6 transition-all duration-200 hover:border-blue-300 hover:shadow-md"
            >
                <!-- Metric Header -->
                <div class="mb-4 flex items-center justify-between">
                    <div class="flex items-center">
                        <div class="mr-3 flex h-12 w-12 items-center justify-center rounded-lg" :class="getMetricBackgroundColor(metric.category)">
                            <component :is="getMetricIcon(metric.category)" class="h-6 w-6" :class="getMetricIconColor(metric.category)" />
                        </div>
                        <div>
                    ...
                    <div v-if="metric.trending" class="flex items-center">
                        <svg class="h-4 w-4" :class="getTrendColor(metric.trend)" fill="currentColor" viewBox="0 0 20 20">
                            <path
                ...
                <div class="mb-4">
                    <div class="mb-2 flex items-center justify-between">
                        ...
                        <span class="text-sm font-medium text-gray-900"> {{ Math.round((metric.currentValue / metric.targetValue) * 100) }}% </span>
                    </div>
                ...
                <div class="flex items-center justify-between text-sm">
                    ...
                    <div v-if="metric.changeFromPrevious" class="flex items-center">
                        <span class="font-medium" :class="getChangeColor(metric.changeFromPrevious)">
                ...
                <!-- Verification Badge -->
                <div v-if="metric.verified" class="mt-3 flex items-center text-xs text-green-600">
        ...
        <!-- Performance Summary -->
        <div class="rounded-lg border border-blue-200 bg-gradient-to-r from-blue-50 to-green-50 p-6">
            <div class="grid grid-cols-1 gap-6 md:grid-cols-4">
                <div class="text-center">
                    <div class="mb-1 text-2xl font-bold text-blue-600">{{ metricsOnTrack }}</div>
                    <div class="text-sm text-gray-600">On Track</div>
                </div>
                <div class="text-center">
                    <div class="mb-1 text-2xl font-bold text-green-600">{{ metricsExceeding }}</div>
                    <div class="text-sm text-gray-600">Exceeding Goals</div>
                </div>
                <div class="text-center">
                    <div class="mb-1 text-2xl font-bold text-orange-600">{{ metricsBehind }}</div>
        ...
        <div v-if="insights.length > 0" class="mt-6">
            <h4 class="mb-3 font-semibold text-gray-900">Key Insights</h4>
            <div class="space-y-2">
                <div v-for="insight in insights" :key="insight.id" class="flex items-start rounded-lg border border-yellow-200 bg-yellow-50 p-3">
                    <svg class="mr-3 mt-0.5 h-5 w-5 flex-shrink-0 text-yellow-600" fill="currentColor" viewBox="0 0 20 20">
                        <path
...
const metricsExceeding = computed(() => props.metrics.filter((m) => (m.currentValue / m.targetValue) * 100 >= 100).length);

const metricsBehind = computed(() => props.metrics.filter((m) => (m.currentValue / m.targetValue) * 100 < 80).length);

const averageProgress = computed(() => {
    if (props.metrics.length === 0) return 0;
    const totalProgress = props.metrics.reduce((sum, metric) => sum + Math.min(100, (metric.currentValue / metric.targetValue) * 100), 0);
