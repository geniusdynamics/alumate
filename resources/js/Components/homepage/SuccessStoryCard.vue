    ...
    >
        ...
        <div class="story-content">
            ...
            <div class="metrics-preview">
                <div v-for="metric in topMetrics" :key="metric.type" class="metric-item">
        ...
        <div v-if="isExpanded" class="expanded-content">
            ...
            <div v-if="story.platformImpact.skillsAcquired.length" class="skills-acquired">
                ...
                <div class="skills-list">
                    <span v-for="skill in story.platformImpact.skillsAcquired" :key="skill" class="skill-tag">
                        {{ skill }}
                    </span>
                </div>
            </div>
            ...
            <div v-if="story.testimonialVideo" class="video-testimonial">
                <button @click.stop="playVideo" class="video-play-button" aria-label="Play video testimonial">
            ...
            <div class="social-sharing">
                ...
                <div class="share-buttons">
                    <button @click.stop="shareStory('linkedin')" class="share-button linkedin" aria-label="Share on LinkedIn">
                        <svg viewBox="0 0 24 24" fill="currentColor">
                            <path
                                d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"
                            />
                    ...
                    <button @click.stop="shareStory('twitter')" class="share-button twitter" aria-label="Share on Twitter">
                        <svg viewBox="0 0 24 24" fill="currentColor">
                            <path
                    ...
                    <button @click.stop="shareStory('copy')" class="share-button copy" aria-label="Copy link">
                        <svg viewBox="0 0 24 24" fill="currentColor">
                            <path
        ...
        <button class="expand-button" @click.stop="toggleExpanded" :aria-label="isExpanded ? 'Collapse story' : 'Expand story'">
...
<style scoped>
.success-story-card {
    @apply cursor-pointer rounded-lg bg-white shadow-md transition-all duration-300 hover:shadow-lg;
    @apply overflow-hidden border border-gray-200;
}
...
.alumni-name {
    @apply mb-1 text-xl font-semibold text-gray-900;
}

.current-role {
    @apply mb-1 text-lg font-medium text-gray-700;
}
...
.metrics-preview {
    @apply mb-4 flex gap-6 rounded-lg bg-gray-50 p-4;
}
...
.career-stage {
    @apply flex-1 rounded-lg p-3 text-center;
}
...
.timeframe {
    @apply text-center text-sm font-medium text-gray-600;
}

.expanded-content {
    @apply space-y-8 border-t border-gray-200 px-6 pb-4 pt-4;
}

.career-progression-section {
    @apply -mx-2 rounded-lg bg-gray-50 p-4;
}

.before-after-section {
    @apply -mx-2 rounded-lg bg-blue-50 p-4;
}
...
.impact-item {
    @apply rounded-lg bg-blue-50 p-3 text-center;
}
...
.milestone-item {
    @apply flex gap-4 rounded-lg bg-gray-50 p-4;
}
...
.milestone-type {
    @apply inline-block rounded-full bg-blue-100 px-2 py-1 text-xs capitalize text-blue-800;
}
...
.video-play-button {
    @apply inline-flex items-center gap-2 rounded-lg bg-red-600 px-6 py-3 text-white transition-colors hover:bg-red-700;
}
...
.expand-button {
    @apply flex w-full items-center justify-center gap-2 bg-gray-50 p-4 font-medium text-blue-600 transition-colors hover:bg-gray-100;
}
