<template>
    <div class="contextual-cta" :class="[`cta-${cta.variant || 'default'}`, `cta-${cta.size || 'medium'}`, { 'cta-urgent': cta.urgent }]">
        <div class="cta-content">
            <!-- Icon or visual element -->
            <div v-if="cta.icon || cta.image" class="cta-visual">
                <component v-if="cta.icon" :is="cta.icon" class="cta-icon" :class="{ 'icon-animated': cta.animated }" />
                <img v-else-if="cta.image" :src="cta.image" :alt="cta.imageAlt || ''" class="cta-image" />
            </div>
            ...
            <div class="cta-actions">
                <button
                    class="cta-button"
                    :class="[`button-${cta.buttonVariant || 'primary'}`, { 'button-loading': isLoading }]"
                    @click="handleClick"
                    :disabled="isLoading"
                >
                ...
                <!-- Secondary action -->
                <button v-if="cta.secondaryAction" class="cta-secondary-button" @click="handleSecondaryClick">
            ...
            <div v-if="cta.trustBadges && cta.trustBadges.length" class="cta-trust-badges">
                <div v-for="badge in cta.trustBadges" :key="badge.name" class="trust-badge" :title="badge.description">
...
<<<<<<< HEAD:resources/js/components/homepage/ContextualCTA.vue
import type { AudienceType, ContextualCTAData, CTAClickEvent } from '@/Types/homepage';
=======
import type { AudienceType, ContextualCTAData, CTAClickEvent } from '@/types/homepage';
>>>>>>> origin/db1:resources/js/Components/homepage/ContextualCTA.vue
