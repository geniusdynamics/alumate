    ...
    <div class="kpi-cards-container">
        <div class="kpi-grid">
            <div
                v-for="kpi in kpis"
                :key="kpi.id"
                :class="['kpi-card', kpi.status && `kpi-${kpi.status}`, { 'kpi-clickable': kpi.clickable }]"
                @click="handleKpiClick(kpi)"
            >
                <!-- KPI Header -->
                <div class="kpi-header">
                    <div class="kpi-icon-wrapper">
                        <component v-if="kpi.icon" :is="kpi.icon" :class="['kpi-icon', kpi.iconColor || 'text-gray-600']" />
                        <div v-else :class="['kpi-icon-placeholder', kpi.iconColor || 'bg-gray-400']"></div>
                    </div>

                    <div class="kpi-trend" v-if="kpi.trend">
                        <svg v-if="kpi.trend.direction === 'up'" class="trend-icon trend-up" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div v-if="kpi.lastUpdated || kpi.target" class="kpi-footer">
                    <div v-if="kpi.target" class="kpi-target">Target: {{ formatValue(kpi.target, kpi.format) }}</div>
                ...
                <div v-if="kpi.progress" class="kpi-progress">
                    ...
                    <div class="progress-text">{{ kpi.progress.percentage }}% of target</div>
                </div>
...
.kpi-footer {
    ...
    @apply border-t border-gray-100 pt-4 dark:border-gray-700;
}
...
.progress-bar {
    @apply mb-2 h-2 w-full rounded-full bg-gray-200 dark:bg-gray-700;
}
