    ...
    <div class="detailed-metrics-container">
        <div class="metrics-header">
            ...
            <div class="metrics-controls">
                <select v-model="selectedCategory" @change="updateMetrics" class="category-selector">
                ...
                <select v-model="selectedTimeRange" @change="updateMetrics" class="time-range-selector">
        ...
        <div class="metrics-grid">
            <div
                ...
                :key="metric.id"
                :class="['metric-card', `metric-${metric.status}`, { 'metric-clickable': metric.drillDown }]"
            ...
            >
                <div class="metric-header">
                    ...
                    <div class="metric-trend" v-if="metric.trend">
                        ...
                        <svg v-else class="trend-icon trend-neutral" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div v-if="metric.threshold" class="metric-threshold">
                    <div class="threshold-bar">
                        <div class="threshold-fill" :style="{ width: `${getThresholdPercentage(metric)}%` }" :class="getThresholdColor(metric)"></div>
                    </div>
                ...
                <div v-if="metric.subMetrics" class="sub-metrics">
                    <div v-for="subMetric in metric.subMetrics" :key="subMetric.id" class="sub-metric">
                ...
                <div v-if="metric.lastUpdated" class="metric-updated">Last updated: {{ formatTime(metric.lastUpdated) }}</div>
            </div>
        </div>
...
const currentMetrics = computed(() => {
    return metricsData.value.filter((metric) => selectedCategory.value === 'all' || metric.category === selectedCategory.value);
});
...
.metric-card {
    @apply rounded-lg border bg-gray-50 p-4 dark:bg-gray-700;
...
.metric-value {
    @apply mb-1 text-2xl font-bold text-gray-900 dark:text-white;
}
...
.threshold-bar {
    @apply mb-2 h-2 w-full rounded-full bg-gray-200 dark:bg-gray-600;
}
...
.sub-metrics {
    @apply mb-4 space-y-1 rounded-md bg-white p-3 dark:bg-gray-800;
}
