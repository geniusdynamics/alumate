...
<template>
    ...
    >
        <!-- Critical Alert Icon -->
        <svg v-if="severity === 'critical'" class="alert-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
        ...
        <!-- High Alert Icon -->
        <svg v-else-if="severity === 'high'" class="alert-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>

        <!-- Medium Alert Icon -->
        <svg v-else-if="severity === 'medium'" class="alert-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>

        <!-- Low Alert Icon -->
        <svg v-else-if="severity === 'low'" class="alert-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>

        <!-- Success/Resolved Icon -->
        <svg v-else-if="severity === 'success' || severity === 'resolved'" class="alert-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>

        <!-- Warning Icon -->
        <svg v-else-if="severity === 'warning'" class="alert-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
...
.alert-count {
    ...
    @apply bg-red-500 text-xs font-bold text-white;
    @apply flex h-5 min-w-5 items-center justify-center rounded-full;
    @apply px-1;
}

.size-xs .alert-count {
    @apply -right-1 -top-1 h-4 min-w-4 text-xs;
}

.size-sm .alert-count {
    @apply -right-1.5 -top-1.5 h-4 min-w-4 text-xs;
}

.size-lg .alert-count {
    @apply -right-2.5 -top-2.5 h-6 min-w-6 text-sm;
}

.size-xl .alert-count {
    @apply -right-3 -top-3 h-6 min-w-6 text-sm;
}

/* Status Dot */
.status-dot {
    @apply absolute -bottom-0.5 -right-0.5;
    @apply h-2 w-2 rounded-full border border-white dark:border-gray-800;
}
