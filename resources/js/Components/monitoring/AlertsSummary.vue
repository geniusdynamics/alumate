        ...
        <div class="alerts-list">
            ...
            <div v-else class="alerts-items">
                <div
                    ...
                    :key="alert.id"
                    :class="['alert-item', `alert-${alert.severity}`, { 'alert-acknowledged': alert.acknowledged }]"
            ...
            <div v-if="filteredAlerts.length > maxDisplayed" class="show-more">
                <button @click="showAllAlerts" class="show-more-button">View All {{ filteredAlerts.length }} Alerts</button>
            </div>
        </div>
    </div>
</template>
...
.alert-stats {
    @apply mb-6 grid grid-cols-4 gap-4;
}
...
.alert-item {
    @apply flex cursor-pointer items-start gap-3 rounded-lg border p-4;
