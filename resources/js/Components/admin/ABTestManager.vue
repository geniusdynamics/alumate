<template>
    <div class="ab-test-manager">
        ...
        <div class="mb-6 flex items-center justify-between">
            ...
            <button @click="showCreateModal = true" class="rounded-lg bg-blue-600 px-4 py-2 font-medium text-white hover:bg-blue-700">
                Create New Test
            </button>
        </div>
        ...
        <div class="mb-6 rounded-lg border bg-white p-4 shadow-sm">
            <div class="flex flex-wrap gap-4">
                <div>
                    ...
                    <select v-model="filters.audience" @change="loadTests" class="rounded-md border border-gray-300 px-3 py-2 text-sm">
                ...
                <div>
                    ...
                    <select v-model="filters.status" @change="loadTests" class="rounded-md border border-gray-300 px-3 py-2 text-sm">
        ...
        <div class="rounded-lg border bg-white shadow-sm">
            ...
            <div v-else-if="tests.length === 0" class="p-6 text-center text-gray-500">No A/B tests found. Create your first test to get started.</div>

            <div v-else class="divide-y divide-gray-200">
                <div v-for="test in tests" :key="test.id" class="p-6 hover:bg-gray-50">
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            <div class="mb-2 flex items-center gap-3">
                                <h3 class="text-lg font-medium text-gray-900">{{ test.name }}</h3>
                                <span :class="getStatusBadgeClass(test.status)" class="rounded-full px-2 py-1 text-xs font-medium">
                                    {{ test.status.charAt(0).toUpperCase() + test.status.slice(1) }}
                                </span>
                                <span :class="getAudienceBadgeClass(test.audience)" class="rounded-full px-2 py-1 text-xs font-medium">
                                    {{ getAudienceLabel(test.audience) }}
                                </span>
                            </div>

                            <div class="mb-3 text-sm text-gray-600">
                                <span v-if="test.start_date"> Started: {{ formatDate(test.start_date) }} </span>
                                <span v-if="test.end_date" class="ml-4"> Ended: {{ formatDate(test.end_date) }} </span>
                                <span class="ml-4"> Traffic: {{ test.traffic_allocation }}% </span>
                            </div>

                            <!-- Test Statistics -->
                            <div v-if="testStats[test.id]" class="mt-4 grid grid-cols-4 gap-4">
                                ...
                                <div class="rounded-lg bg-gray-50 p-3">
                                    ...
                                    <div class="text-lg font-semibold text-gray-900">${{ testStats[test.id].totalValue.toLocaleString() }}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="ml-4 flex items-center gap-2">
                            <button @click="viewResults(test.id)" class="text-sm font-medium text-blue-600 hover:text-blue-800">View Results</button>
                            <button
                            ...
                            <button @click="editTest(test)" class="text-sm font-medium text-gray-600 hover:text-gray-800">Edit</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        >
            <div class="mx-4 max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-lg bg-white shadow-xl">
                ...
                <form @submit.prevent="saveTest" class="space-y-6 p-6">
                    <!-- Basic Information -->
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <label class="mb-2 block text-sm font-medium text-gray-700">Test Name</label>
                            <input
                        ...
                        <div>
                            ...
                            <select v-model="testForm.audience" required class="w-full rounded-md border border-gray-300 px-3 py-2">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <label class="mb-2 block text-sm font-medium text-gray-700">Traffic Allocation (%)</label>
                            <input
                    ...
                    <div>
                        <div class="mb-4 flex items-center justify-between">
                            ...
                            <button type="button" @click="addVariant" class="text-sm font-medium text-blue-600 hover:text-blue-800">
                                Add Variant
                            </button>
                        </div>

                        <div class="space-y-4">
                            <div v-for="(variant, index) in testForm.variants" :key="index" class="rounded-lg border border-gray-200 p-4">
                                ...
                                <div class="mb-4 grid grid-cols-3 gap-4">
                                    <div>
                                        <label class="mb-1 block text-sm font-medium text-gray-700">Variant ID</label>
                                        <input
                                            v-model="variant.id"
                                            type="text"
                                            required
                                    ...
                                    <div>
                                        <label class="mb-1 block text-sm font-medium text-gray-700">Weight (%)</label>
                                        <input
                                ...
                                <!-- Component Overrides -->
                                <div>
                                    <label class="mb-2 block text-sm font-medium text-gray-700">Component Overrides</label>
                                    <div class="space-y-2">
                                        <div
                                        ...
                                        >
                                            ...
                                            <textarea
                                                ...
                                                rows="2"
                                                class="flex-2 rounded-md border border-gray-300 px-3 py-2 text-sm"
                                            ></textarea>
                                            <button
                                                type="button"
                                        ...
                                        <button type="button" @click="addOverride(variant)" class="text-sm text-blue-600 hover:text-blue-800">
                                            Add Override
                                        </button>
                                    </div>
                                </div>
                    ...
                    <div>
                        <div class="mb-4 flex items-center justify-between">
                            ...
                            <button type="button" @click="addConversionGoal" class="text-sm font-medium text-blue-600 hover:text-blue-800">
                                Add Goal
                            </button>
                        </div>

                        <div class="space-y-4">
                            <div v-for="(goal, index) in testForm.conversionGoals" :key="index" class="rounded-lg border border-gray-200 p-4">
                                <div class="mb-4 flex items-start justify-between">
                                ...
                                <div class="grid grid-cols-4 gap-4">
                                    <div>
                                        <label class="mb-1 block text-sm font-medium text-gray-700">Goal ID</label>
                                        <input
                                            v-model="goal.id"
                                            type="text"
                                            required
                                    ...
                                    <div>
                                        ...
                                        <select v-model="goal.type" required class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm">
                    ...
                    <div class="flex justify-end gap-3 border-t border-gray-200 pt-6">
                        <button
                            type="button"
                            @click="closeModals"
                            class="rounded-md bg-gray-100 px-4 py-2 font-medium text-gray-700 hover:bg-gray-200"
                        >
                            Cancel
                        </button>
                        <button
        ...
        >
            <div class="mx-4 max-h-[90vh] w-full max-w-6xl overflow-y-auto rounded-lg bg-white shadow-xl">
                <div class="border-b border-gray-200 px-6 py-4">
                ...
                <div v-if="selectedTestResults" class="p-6">
                    <!-- Test Overview -->
                    <div class="mb-6">
                        <h3 class="mb-2 text-lg font-medium text-gray-900">{{ selectedTestResults.testName }}</h3>
                    ...
                    <div
                        v-if="selectedTestResults.statisticalSignificance"
                        class="mb-6 rounded-lg p-4"
                        :class="
                            selectedTestResults.statisticalSignificance.significant
                    ...
                    <div class="space-y-4">
                        ...
                        >
                            <div class="mb-4 flex items-start justify-between">
                                ...
                                <div v-if="selectedTestResults.winner === variant.variantId" class="text-sm font-medium text-green-600">Winner</div>
                            </div>

                            <div class="grid grid-cols-3 gap-4">
                                <div v-for="goal in variant.goals" :key="goal.goalId" class="rounded-lg bg-gray-50 p-3">
                ...
                <div class="flex justify-end border-t border-gray-200 px-6 py-4">
                    <button @click="showResultsModal = false" class="rounded-md bg-gray-100 px-4 py-2 font-medium text-gray-700 hover:bg-gray-200">
                        Close
                    </button>
                </div>
            </div>
...
const resetForm = () => {
    ...
    testForm.conversionGoals = [{ id: 'trial_signup', name: 'Trial Signup', type: 'trial_signup', value: 100 }];
};
