        ...
        <div v-else class="analytics-grid">
            ...
            <div class="analytics-section">
                ...
                <div class="popular-queries">
                    <div v-for="(query, index) in analytics.popular_queries || []" :key="index" class="query-item">
            ...
            <div class="analytics-section">
                ...
                <div class="trends-chart">
                    <div v-for="(trend, index) in analytics.search_trends || []" :key="index" class="trend-bar">
                        <div class="trend-date">{{ formatDate(trend.date) }}</div>
                        <div class="trend-bar-container">
                            <div class="trend-bar-fill" :style="{ width: `${(trend.count / maxTrendCount) * 100}%` }"></div>
                        </div>
            ...
            <div class="analytics-section">
                ...
                <div class="popular-filters">
                    <div v-for="(filterGroup, filterType) in analytics.popular_filters || {}" :key="filterType" class="filter-group">
                        <h5 class="filter-type">{{ formatFilterType(filterType) }}</h5>
                        <div class="filter-values">
                            <span v-for="(value, index) in filterGroup.slice(0, 5)" :key="index" class="filter-value">
                                {{ value }}
                            </span>
                        </div>
                    </div>
...
.query-item {
    @apply flex items-center justify-between rounded bg-gray-50 px-3 py-2;
}
...
.trend-date {
    @apply w-16 flex-shrink-0 text-sm text-gray-600;
}

.trend-bar-container {
    @apply h-4 flex-1 overflow-hidden rounded-full bg-gray-200;
}
...
.trend-count {
    @apply w-8 text-right text-sm text-gray-700;
}
