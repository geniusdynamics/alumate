...
<template>
    <div :class="fieldClasses" @touchstart="handleTouchStart" @touchend="handleTouchEnd">
        <!-- Field Label -->
        <label v-if="field.label" :for="fieldId" :class="labelClasses">
            {{ field.label }}
        ...
        <!-- Help Text -->
        <div v-if="field.helpText" :id="helpTextId" class="help-text">
            {{ field.helpText }}
        </div>
...
.field-radio {
    @apply h-5 w-5 border-gray-300 text-blue-600 dark:border-gray-600;
    @apply focus:ring-2 focus:ring-blue-500;
    @apply bg-white dark:bg-gray-800;
    margin-top: 2px;
}
...
.field-disabled .field-radio {
    @apply cursor-not-allowed bg-gray-100 dark:bg-gray-700;
}
...
@media (hover: none) and (pointer: coarse) {
    ...
    .field-checkbox,
    .field-radio {
        @apply h-6 w-6; /* Larger checkboxes/radios on touch devices */
    }
}
