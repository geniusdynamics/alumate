<template>
    <div class="reunion-memory-wall">
        ...
        <div class="mb-6 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
                ...
                <button
                    ...
                    class="rounded-md bg-purple-600 px-4 py-2 text-sm text-white hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500"
                >
                    Share Memory
                </button>
            </div>
        ...
        <div v-else-if="memories.length > 0" class="space-y-6">
            <div v-for="memory in memories" :key="memory.id" class="overflow-hidden rounded-lg bg-white shadow-md">
                <!-- Memory header -->
                <div class="p-6 pb-4">
                    <div class="mb-4 flex items-start justify-between">
                        ...
                        <!-- Featured badge -->
                        <div v-if="memory.is_featured" class="rounded-full bg-yellow-100 px-2 py-1 text-xs font-medium text-yellow-800">Featured</div>
                    </div>

                    <!-- Memory title -->
                    <h3 class="mb-3 text-lg font-semibold text-gray-900">{{ memory.title }}</h3>

                    <!-- Memory content -->
                    <div class="prose prose-sm mb-4 max-w-none text-gray-700">
                        <p>{{ memory.content }}</p>
                    </div>
                    ...
                    <div v-if="memory.media_urls && memory.media_urls.length > 0" class="mb-4">
                        <div class="grid grid-cols-2 gap-2 md:grid-cols-3">
                            <img
                                ...
                                class="h-24 w-full cursor-pointer rounded-md object-cover hover:opacity-90"
                                @click="openMediaLightbox(url, memory.media_urls)"
                            />
                        </div>
                        <p v-if="memory.media_urls.length > 6" class="mt-2 text-sm text-gray-500">+{{ memory.media_urls.length - 6 }} more photos</p>
                    </div>
                    ...
                    <!-- Memory date -->
                    <div v-if="memory.memory_date" class="mb-4 text-sm text-gray-500">
                ...
                <div class="flex items-center justify-between border-t bg-gray-50 px-6 py-4">
                    <div class="flex items-center space-x-6">
                        ...
                        >
                            <svg class="h-4 w-4" :fill="memory.is_liked_by_user ? 'currentColor' : 'none'" stroke="currentColor" viewBox="0 0 24 24">
                                <path
                                    stroke-linecap="round"
                                    stroke-linejoin="round"
                                    stroke-width="2"
                        ...
                        <!-- Comment button -->
                        <button @click="toggleComments(memory)" class="flex items-center space-x-1 text-sm text-gray-600 hover:text-blue-600">
                    ...
                    <div class="flex items-center text-sm text-gray-500">
                        <component :is="getTypeIcon(memory.type)" class="mr-1 h-4 w-4" />
                        <span class="capitalize">{{ memory.type }}</span>
                    </div>
                </div>
                ...
                <div v-if="showComments[memory.id]" class="border-t bg-gray-50">
                    <div class="p-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="mt-2 flex justify-end">
                                        <button
                                            ...
                                            class="rounded-md bg-purple-600 px-3 py-1 text-sm text-white hover:bg-purple-700 disabled:cursor-not-allowed disabled:opacity-50"
                                        >
                                            Comment
                                        </button>
                                    </div>
                        ...
                        <div class="text-sm text-gray-500">Comments will be loaded here...</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div v-else class="py-12 text-center">
            ...
            <h3 class="mb-2 text-lg font-medium text-gray-900">No memories shared yet</h3>
            <p class="mb-4 text-gray-600">Be the first to share a memory from your time together!</p>
            <button v-if="canAddMemory" @click="showMemoryModal = true" class="rounded-md bg-purple-600 px-4 py-2 text-white hover:bg-purple-700">
                Share First Memory
            </button>
        </div>

        <!-- Memory creation modal -->
        <MemoryModal v-if="showMemoryModal" :event-id="eventId" @close="showMemoryModal = false" @created="handleMemoryCreated" />
    </div>
</template>
