<template>
    <div class="swipeable-tabs">
        ...
        <div class="tabs-mobile" ref="tabsContainer" role="tablist" :aria-label="tabsAriaLabel || 'Tab navigation'">
            ...
            >
                <component v-if="tab.icon" :is="tab.icon" class="mb-1 h-4 w-4" aria-hidden="true" />
                <span>{{ tab.label }}</span>
                <span
                    v-if="tab.badge"
        ...
        <div v-if="showIndicators" class="mt-4 flex justify-center space-x-2">
            <button
                v-for="(tab, index) in tabs"
                :key="`indicator-${tab.id}`"
                @click="selectTab(tab.id, index)"
                class="h-2 w-2 rounded-full transition-colors"
                :class="activeTab === tab.id ? 'bg-blue-600 dark:bg-blue-400' : 'bg-gray-300 dark:bg-gray-600'"
            />
        </div>
        ...
        <div
            v-if="showSwipeHint"
            class="fixed bottom-20 left-1/2 z-50 -translate-x-1/2 transform rounded-full bg-black bg-opacity-75 px-4 py-2 text-sm text-white"
        >
