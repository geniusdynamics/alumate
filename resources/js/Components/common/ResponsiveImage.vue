<template>
    <div :class="containerClasses" role="img" :aria-label="alt">
        <!-- Main Image -->
        <img
            ref="imageElement"
            :class="imageClasses"
        ...
        <!-- Loading State -->
        <div v-if="isLoading && !hasError" :class="loadingClasses">
            <div class="h-full w-full animate-pulse bg-gray-300 dark:bg-gray-600"></div>
        </div>
...
const srcSet = computed(() => {
    ...
    return props.src.srcSet.map((source) => `${source.url} ${source.width}w`).join(', ');
});
