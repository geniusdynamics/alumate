<template>
    <div class="reunion-list">
        <div class="mb-6">
            <h2 class="mb-4 text-2xl font-bold text-gray-900">Class Reunions</h2>

            <!-- Filters -->
            <div class="mb-6 rounded-lg bg-white p-4 shadow">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                    <div>
                        <label class="mb-1 block text-sm font-medium text-gray-700"> Graduation Year </label>
                        <input
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-medium text-gray-700"> Time Period </label>
                        <select
        ...
        <div v-if="upcomingMilestones.length > 0" class="mb-8">
            <h3 class="mb-4 text-lg font-semibold text-gray-900">Your Upcoming Reunion Milestones</h3>
        ...
        <!-- Reunions Grid -->
        <div v-else-if="reunions.length > 0" class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            <ReunionCard v-for="reunion in reunions" :key="reunion.id" :reunion="reunion" @view="viewReunion" @register="registerForReunion" />
        </div>
        ...
        <div v-else class="py-12 text-center">
            ...
            <h3 class="mb-2 text-lg font-medium text-gray-900">No reunions found</h3>
            <p class="mb-4 text-gray-600">
                {{ hasFilters ? 'Try adjusting your filters to see more results.' : 'No reunions have been scheduled yet.' }}
            </p>
            <button v-if="hasFilters" @click="clearFilters" class="rounded-md bg-gray-600 px-4 py-2 text-white hover:bg-gray-700">
                Clear Filters
            </button>
        </div>
