    ...
    <div class="job-card" :class="{ applied: job.has_applied }">
        ...
        <div v-if="job.match_score?.top_reasons" class="match-reasons">
            ...
            <ul class="reasons-list">
                <li v-for="reason in job.match_score.top_reasons.slice(0, 2)" :key="reason" class="reason-item">
                    {{ reason }}
                </li>
            </ul>
        </div>
        ...
        <!-- Posted Date -->
        <div class="posted-date">Posted {{ formatDate(job.posted_at) }}</div>

        <!-- Action Buttons -->
        <div class="action-buttons">
            <button @click="$emit('view-details', job)" class="btn btn-secondary">View Details</button>

            <button v-if="!job.has_applied" @click="$emit('apply', job)" class="btn btn-primary">Apply Now</button>
