    ...
    >
        ...
        <div class="mb-4 flex items-start justify-between">
            ...
            <div class="flex items-center space-x-2">
                <span :class="getStatusClass(mentorship.status)" class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium">
        ...
        <!-- Mentor Info -->
        <div class="mb-4 rounded-md bg-gray-50 p-3 dark:bg-gray-700">
            <div class="flex items-center space-x-3">
        ...
        <div class="mb-4 space-y-3">
            ...
            <!-- Focus Areas -->
            <div
                v-if="mentorship.focus_areas && mentorship.focus_areas.length > 0"
            ...
            <!-- Meeting Type -->
            <div class="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400">
                <VideoCameraIcon v-if="mentorship.meeting_type === 'virtual'" class="h-4 w-4" />
        ...
        <div class="mb-4">
            <div class="mb-2 flex items-center justify-between">
                ...
                <span class="text-sm text-gray-600 dark:text-gray-400"
                    >{{ mentorship.sessions_completed || 0 }}/{{ mentorship.total_sessions || 10 }} sessions</span
                >
            </div>
            <div class="h-2 w-full rounded-full bg-gray-200 dark:bg-gray-700">
                <div class="h-2 rounded-full bg-green-600 transition-all duration-300" :style="{ width: getProgressPercentage() + '%' }"></div>
            </div>
        </div>

        <!-- Goals & Objectives -->
        <div v-if="mentorship.goals && mentorship.goals.length > 0" class="mb-4">
            <h4 class="mb-2 text-sm font-medium text-gray-900 dark:text-white">Current Goals</h4>
            <ul class="space-y-1">
                <li v-for="goal in mentorship.goals.slice(0, 3)" :key="goal.id" class="flex items-start text-sm text-gray-600 dark:text-gray-400">
                    <CheckCircleIcon v-if="goal.completed" class="mr-2 mt-0.5 h-4 w-4 flex-shrink-0 text-green-500" />
                    <ClockIcon v-else class="mr-2 mt-0.5 h-4 w-4 flex-shrink-0 text-yellow-500" />
        ...
        <!-- Recent Activity -->
        <div v-if="mentorship.recent_activity" class="mb-4 rounded-md bg-blue-50 p-3 dark:bg-blue-900/20">
            <h4 class="mb-2 text-sm font-medium text-gray-900 dark:text-white">Recent Activity</h4>
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ mentorship.recent_activity.description }}</p>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ formatDate(mentorship.recent_activity.date) }}</p>
        </div>

        <!-- Upcoming Tasks -->
        <div v-if="mentorship.upcoming_tasks && mentorship.upcoming_tasks.length > 0" class="mb-4">
            <h4 class="mb-2 text-sm font-medium text-gray-900 dark:text-white">Upcoming Tasks</h4>
            <div class="space-y-2">
                <div v-for="task in mentorship.upcoming_tasks.slice(0, 2)" :key="task.id" class="flex items-center justify-between text-sm">
        ...
        <div class="flex space-x-3">
            <button
                ...
                @click="joinSession"
                class="flex-1 rounded-md bg-green-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-green-700"
            >
                Join Session
            </button>
        ...
        <!-- Rating & Feedback -->
        <div v-if="mentorship.rating" class="mt-4 border-t border-gray-200 pt-4 dark:border-gray-700">
            <div class="flex items-center justify-between">
                ...
                <button @click="provideFeedback" class="text-sm text-blue-600 hover:text-blue-800 dark:text-blue-400">Update Rating</button>
            </div>
        </div>
    </div>
</template>
