    ...
    <div class="advanced-search">
        ...
        <div class="search-input-container">
            <div class="relative">
                ...
                />
                <button @click="performSearch" class="search-button" :disabled="isSearching" aria-label="Search">
                    <SearchIcon v-if="!isSearching" class="h-5 w-5" />
                    <LoadingSpinner v-else class="h-5 w-5" />
                </button>
            </div>

            <!-- Search Suggestions -->
            <div v-if="showSuggestions && suggestions.length > 0" class="suggestions-dropdown" role="listbox" aria-label="Search suggestions">
                <div
        ...
        <div v-if="hasSearched" class="search-results">
            ...
            <div class="results-header">
                <div class="results-info">
                    <span class="results-count"> {{ searchResults.total.toLocaleString() }} results </span>
                    <span v-if="searchResults.took" class="search-time"> ({{ searchResults.took }}ms) </span>
                </div>
                ...
                <div class="results-actions">
                    ...
                    <select v-model="sortBy" @change="performSearch" class="sort-select" aria-label="Sort results">
                    ...
                    <button v-if="canSaveSearch" @click="saveCurrentSearch" class="save-search-btn" :disabled="isSavingSearch">
            ...
            <div class="results-list">
                <div v-for="result in searchResults.hits" :key="`${result.type}-${result.id}`" class="result-item" :class="`result-${result.type}`">
                    <!-- User Result -->
                    <UserResult v-if="result.type === 'user'" :user="result.source" :highlight="result.highlight" :score="result.score" />

                    <!-- Post Result -->
                    <PostResult v-else-if="result.type === 'post'" :post="result.source" :highlight="result.highlight" :score="result.score" />

                    <!-- Job Result -->
                    <JobResult v-else-if="result.type === 'job'" :job="result.source" :highlight="result.highlight" :score="result.score" />

                    <!-- Event Result -->
                    <EventResult v-else-if="result.type === 'event'" :event="result.source" :highlight="result.highlight" :score="result.score" />
                </div>
            </div>
        ...
        <div v-if="hasSearched && searchResults.total === 0" class="no-results">
            ...
            <p class="no-results-message">Try adjusting your search terms or filters to find what you're looking for.</p>
...
.search-input {
    @apply w-full rounded-lg border border-gray-300 px-4 py-3 pr-12 text-lg;
...
.suggestions-dropdown {
    @apply absolute left-0 right-0 top-full z-10 mt-1;
...
.suggestion-type {
    @apply rounded bg-gray-100 px-2 py-1 text-xs text-gray-500;
}
...
.filter-toggle {
    @apply flex items-center space-x-2 rounded-md border border-gray-300 px-3 py-2;
...
.save-search-btn {
    ...
    @apply rounded-md bg-blue-600 text-white hover:bg-blue-700;
    @apply disabled:cursor-not-allowed disabled:opacity-50;
}
...
.pagination {
    @apply mt-8 flex items-center justify-center space-x-4;
}

.pagination-btn {
    @apply rounded-md border border-gray-300 px-4 py-2;
    @apply hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50;
}
...
.clear-search-btn {
    @apply rounded-md bg-gray-600 px-4 py-2 text-white hover:bg-gray-700;
}
