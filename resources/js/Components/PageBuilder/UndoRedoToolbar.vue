<template>
    <div class="undo-redo-toolbar">
        ...
        <div class="history-indicator">
            ...
            <span class="history-count"> {{ currentHistoryIndex + 1 }} / {{ totalHistoryItems }} </span>
        </div>
        ...
        <div class="autosave-status">
            ...
            >
                <Icon :name="getSaveIcon()" :class="['h-3 w-3', { 'animate-spin': isSaving }]" />
            </div>
        ...
        <button @click="toggleHistoryPanel" :class="['panel-toggle', { active: showHistoryPanel }]" title="Show history panel">
        ...
        <div v-if="showHistoryPanel" class="history-panel">
            ...
            <div class="history-list">
                ...
                >
                    <div class="item-icon">
                        <Icon :name="getHistoryIcon(item.type)" class="h-4 w-4" />
                    </div>
...
const clearHistory = () => {
    if (confirm('Are you sure you want to clear the history? This action cannot be undone.')) {
        ...
        realTimeEditingService.undoRedoState.history = [realTimeEditingService.undoRedoState.history[currentHistoryIndex.value]];
...
<style scoped>
.undo-redo-toolbar {
    @apply relative flex items-center space-x-4 border-b border-gray-200 bg-white px-4 py-2 dark:border-gray-700 dark:bg-gray-800;
}
...
.action-btn {
    @apply flex items-center space-x-2 rounded bg-gray-100 px-3 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600;
}

.action-btn.disabled {
    @apply cursor-not-allowed opacity-50 hover:bg-gray-100 dark:hover:bg-gray-700;
}
...
.history-dot {
    @apply h-2 w-2 cursor-pointer rounded-full transition-colors;
}
...
.history-panel {
    @apply absolute right-0 top-full z-50 mt-1 w-80 rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800;
}
...
.history-item {
    @apply flex items-center space-x-3 border-b border-gray-100 p-3 last:border-b-0 dark:border-gray-700;
}
...
.clear-history-btn,
.export-history-btn {
    @apply flex items-center space-x-2 rounded px-3 py-2 text-xs font-medium transition-colors;
}
