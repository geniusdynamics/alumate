<template>
    <div class="campaign-progress rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <div class="mb-6">
            <h3 class="mb-2 text-lg font-semibold text-gray-900">Campaign Progress</h3>
        ...
        <div class="mb-6">
            <div class="mb-2 flex items-center justify-between">
                <span class="text-2xl font-bold text-gray-900"> ${{ formatAmount(campaign.raised_amount) }} </span>
            ...
            <div class="mb-2 h-3 w-full rounded-full bg-gray-200">
                <div
                    class="h-3 rounded-full bg-gradient-to-r from-green-500 to-green-600 transition-all duration-500 ease-out"
            ...
            <div class="flex items-center justify-between text-sm text-gray-600">
                ...
                <span v-if="campaign.progress_percentage >= 100" class="font-medium text-green-600"> 🎉 Goal Reached! </span>
                <span v-else> ${{ formatAmount(campaign.goal_amount - campaign.raised_amount) }} to go </span>
            </div>
        </div>
        ...
        <div v-if="campaign.allow_peer_fundraising && peerFundraisingStats" class="mb-6 rounded-lg bg-blue-50 p-4">
            <h4 class="mb-2 text-sm font-medium text-blue-900">Peer Fundraising</h4>
        ...
        <div v-if="recentDonations.length > 0" class="mb-6">
            <h4 class="mb-3 text-sm font-medium text-gray-900">Recent Donations</h4>
            <div class="max-h-32 space-y-2 overflow-y-auto">
                <div v-for="donation in recentDonations.slice(0, 5)" :key="donation.id" class="flex items-center justify-between text-sm">
                    <div class="flex items-center">
                        <div class="mr-2 flex h-6 w-6 items-center justify-center rounded-full bg-gray-300">
        ...
        <div class="border-t pt-4">
            <h4 class="mb-3 text-sm font-medium text-gray-900">Share Campaign</h4>
            <div class="flex space-x-2">
                <button
                    @click="shareToFacebook"
                    class="flex-1 rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700"
                >
                    Facebook
                </button>
                <button
                    @click="shareToTwitter"
                    class="flex-1 rounded-md bg-blue-400 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-500"
                >
                    Twitter
                </button>
                <button
                    @click="shareToLinkedIn"
                    class="flex-1 rounded-md bg-blue-700 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-800"
                >
                    LinkedIn
                </button>
                <button
        ...
        <div v-if="milestones.length > 0" class="mt-6 border-t pt-4">
            <h4 class="mb-3 text-sm font-medium text-gray-900">Milestones</h4>
            <div class="space-y-2">
                <div v-for="milestone in milestones" :key="milestone.percentage" class="flex items-center justify-between text-sm">
                    <div class="flex items-center">
                        <div
                            :class="[
                                'mr-2 h-4 w-4 rounded-full',
                                campaign.progress_percentage >= milestone.percentage ? 'bg-green-500' : 'bg-gray-300',
                            ]"
                        ></div>
