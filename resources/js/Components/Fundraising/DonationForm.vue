<template>
    <div class="donation-form rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <div class="mb-6">
            <h3 class="mb-2 text-xl font-semibold text-gray-900">Make a Donation</h3>
        ...
        <form @submit.prevent="submitDonation" class="space-y-6">
            <!-- Donation Amount -->
            <div>
                <label class="mb-2 block text-sm font-medium text-gray-700"> Donation Amount * </label>
                <div class="relative">
                    ...
                    <input
                        ...
                        step="0.01"
                        required
                        class="block w-full rounded-md border border-gray-300 py-2 pl-7 pr-12 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="0.00"
                    />
                </div>
            ...
            <div v-if="campaign.allow_anonymous_donations">
                <label class="flex items-center">
                    <input v-model="form.is_anonymous" type="checkbox" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
            ...
            <!-- Donation Message -->
            <div>
                <label class="mb-2 block text-sm font-medium text-gray-700"> Message (Optional) </label>
                <textarea
            ...
            <div v-if="peerFundraisers.length > 0">
                <label class="mb-2 block text-sm font-medium text-gray-700"> Support a Peer Fundraiser (Optional) </label>
                <select
                    v-model="form.peer_fundraiser_id"
                    class="block w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                    <option value="">Donate directly to campaign</option>
                    <option v-for="fundraiser in peerFundraisers" :key="fundraiser.id" :value="fundraiser.id">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 gap-3">
                    <label
                        ...
                        :key="method.value"
                        class="flex cursor-pointer items-center rounded-md border border-gray-300 p-3 hover:bg-gray-50"
                        :class="{ 'border-blue-500 bg-blue-50': form.payment_method === method.value }"
                    >
                        <input v-model="form.payment_method" :value="method.value" type="radio" class="text-blue-600 focus:ring-blue-500" required />
                        <div class="ml-3 flex items-center">
                            <component :is="method.icon" class="mr-2 h-6 w-6" />
                            <div>
            ...
            <div>
                <label class="flex items-start">
                    ...
                    <span class="ml-2 text-sm text-gray-700">
                        I agree to the
                        <a href="/terms" target="_blank" class="text-blue-600 hover:text-blue-700"> Terms and Conditions </a>
            ...
            <div class="border-t pt-4">
                <button
                    ...
                    class="w-full rounded-md bg-blue-600 px-4 py-3 font-medium text-white transition-colors hover:bg-blue-700 disabled:bg-gray-400"
                >
                    {{ loading ? 'Processing...' : `Donate $${form.amount || 0}` }}
                </button>

                <p class="mt-2 text-center text-xs text-gray-500">Your donation will be processed securely. You will receive a confirmation email.</p>
            </div>
        </form>
