<template>
    <div class="virtual-event-viewer">
        ...
        <div class="meeting-header rounded-t-lg border-b bg-gray-50 p-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    <div class="flex items-center space-x-2">
                        <Icon :name="getPlatformIcon(event.meeting_platform)" class="h-5 w-5" />
                ...
                <div class="flex items-center space-x-2">
                    ...
                    >
                        <Icon :name="isRecording ? 'stop-circle' : 'play-circle'" class="mr-1 h-4 w-4" />
        ...
        <div class="meeting-viewer relative bg-black" :style="{ height: viewerHeight }">
            <!-- Jitsi Meet Embed -->
            <div v-if="event.meeting_platform === 'jitsi' && event.meeting_embed_allowed && embedUrl" class="h-full w-full">
                <iframe
                    ref="jitsiFrame"
                    :src="embedUrl"
            ...
            <div v-else class="flex h-full flex-col items-center justify-center space-y-6 text-white">
                <div class="text-center">
                    <Icon :name="getPlatformIcon(event.meeting_platform)" class="mx-auto mb-4 h-16 w-16 text-gray-300" />
                    <h3 class="mb-2 text-xl font-semibold">Join {{ getPlatformName(event.meeting_platform) }} Meeting</h3>
                    <p class="mb-6 max-w-md text-gray-300">
                        This meeting is hosted on {{ getPlatformName(event.meeting_platform) }}. Click the button below to join in a new window.
                    </p>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    >
                        <Icon name="external-link" class="mr-2 h-5 w-5" />
                        Join Meeting
                    </a>

                    <div v-if="meetingCredentials.password" class="text-center">
                        <p class="mb-1 text-sm text-gray-300">Meeting Password:</p>
                        <code class="rounded bg-gray-800 px-3 py-1 font-mono text-green-400">
            ...
            <div v-if="error" class="absolute inset-0 flex items-center justify-center bg-red-50">
                <div class="max-w-md text-center text-red-600">
                    <Icon name="alert-circle" class="mx-auto mb-4 h-12 w-12" />
                    <h3 class="mb-2 text-lg font-semibold">Unable to Load Meeting</h3>
                    <p class="mb-4 text-sm">{{ error }}</p>
                    <button @click="retryLoad" class="rounded-lg bg-red-600 px-4 py-2 text-white transition-colors hover:bg-red-700">
                        Try Again
                    </button>
                </div>
            </div>
        ...
        <!-- Meeting Controls -->
        <div class="meeting-controls rounded-b-lg border-t bg-gray-50 p-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    <!-- Meeting Info -->
                    <div class="text-sm text-gray-600">
                        <span class="font-medium">Meeting ID:</span>
                        <code class="ml-1 rounded bg-gray-200 px-2 py-1 font-mono">
                            {{ getMeetingId() }}
                        </code>
                    </div>

                    <!-- Copy Link -->
                    <button @click="copyMeetingLink" class="flex items-center space-x-1 text-sm text-blue-600 hover:text-blue-700">
                ...
                <div class="flex items-center space-x-2">
                    <button
                        ...
                        class="rounded px-3 py-1 text-sm text-gray-600 transition-colors hover:bg-gray-200 hover:text-gray-900"
                    >
        ...
        >
            <div class="w-full max-w-md rounded-lg bg-white p-6">
                <h3 class="mb-4 text-lg font-semibold">Meeting Settings</h3>
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <button @click="showMeetingSettings = false" class="px-4 py-2 text-gray-600 transition-colors hover:text-gray-800">Cancel</button>
                    <button @click="saveMeetingSettings" class="rounded-lg bg-blue-600 px-4 py-2 text-white transition-colors hover:bg-blue-700">
                        Save Changes
                    </button>
                </div>
            </div>
