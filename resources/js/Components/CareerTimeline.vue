<template>
    <div class="career-timeline">
        ...
        <!-- Career Stats -->
        <div class="mb-8 grid grid-cols-1 gap-4 md:grid-cols-4" v-if="progression">
        ...
        <!-- Career Goals -->
        <CareerGoals v-if="canEdit && suggestions.length > 0" :suggestions="suggestions" class="mb-8" />
        ...
        <div class="relative">
            ...
            <!-- Timeline items -->
            <div class="space-y-8">
                <div v-for="(item, index) in timeline" :key="`${item.type}-${item.data.id}-${index}`" class="relative flex items-start">
                    <!-- Timeline dot -->
                    <div class="flex w-16 flex-shrink-0 justify-center">
                        <div :class="['h-4 w-4 rounded-full border-4 bg-white', getTimelineDotColor(item)]"></div>
                    </div>
                    ...
                    <div class="ml-4 flex-1">
                        ...
                        <div v-else-if="item.type === 'career_end'" class="text-sm italic text-gray-500">
            ...
            <!-- Empty state -->
            <div v-if="timeline.length === 0" class="py-12 text-center">
                <BriefcaseIcon class="mx-auto mb-4 h-12 w-12 text-gray-400" />
                <h3 class="mb-2 text-lg font-medium text-gray-900">No career information yet</h3>
        ...
        <!-- Modals -->
        <AddCareerModal v-if="showAddCareerModal" :career-entry="editingCareerEntry" @close="closeAddCareerModal" @saved="handleCareerEntrySaved" />

        <MilestoneModal v-if="showMilestoneModal" :milestone="editingMilestone" @close="closeMilestoneModal" @saved="handleMilestoneSaved" />
    </div>
</template>
