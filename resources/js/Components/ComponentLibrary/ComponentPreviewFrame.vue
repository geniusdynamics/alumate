<template>
    <div class="component-preview-frame" :class="frameClasses" :style="frameStyles">
        <!-- Sandboxed Preview Container -->
        <div ref="previewContainer" class="preview-container h-full w-full overflow-auto" :class="containerClasses">
            ...
            <div class="component-render-area">
                ...
                <div v-else class="flex h-64 items-center justify-center rounded-lg bg-gray-50 text-gray-500 dark:bg-gray-900 dark:text-gray-400">
            ...
            <!-- Grid Overlay -->
            <div v-if="showGrid" class="grid-overlay pointer-events-none absolute inset-0 z-10" aria-hidden="true"></div>

            <!-- Rulers -->
            <div v-if="showRulers" class="rulers pointer-events-none absolute inset-0 z-10" aria-hidden="true">
                <!-- Horizontal Ruler -->
                <div
                    class="horizontal-ruler absolute left-0 right-0 top-0 h-4 border-b border-gray-300 bg-gray-100 dark:border-gray-600 dark:bg-gray-700"
                >
                    <div
                        ...
                        :style="{ left: `${i * 10}px` }"
                    >
                        <span v-if="i % 10 === 0" class="absolute left-1 top-0 text-xs text-gray-600 dark:text-gray-400">
                            {{ i * 10 }}
                        </span>
                    </div>
                </div>
                ...
                <!-- Vertical Ruler -->
                <div
                    class="vertical-ruler absolute bottom-0 left-0 top-0 w-4 border-r border-gray-300 bg-gray-100 dark:border-gray-600 dark:bg-gray-700"
                >
                    <div
                    ...
                    >
                        <span
                            v-if="i % 10 === 0"
                            class="absolute left-0 top-1 origin-left -rotate-90 transform text-xs text-gray-600 dark:text-gray-400"
                        >
                            {{ i * 10 }}
                        </span>
                    </div>
            ...
            <div
                v-if="isLoading"
                class="loading-overlay absolute inset-0 z-20 flex items-center justify-center bg-white bg-opacity-75 dark:bg-gray-900 dark:bg-opacity-75"
            >
                <div class="text-center">
                    <Icon name="arrow-path" class="mx-auto mb-2 h-8 w-8 animate-spin text-indigo-600" />
            ...
            <div
                v-if="renderError"
                class="error-overlay absolute inset-0 z-20 flex items-center justify-center bg-red-50 bg-opacity-75 dark:bg-red-900"
            >
                <div class="mx-auto max-w-md p-6 text-center">
                    <Icon name="exclamation-triangle" class="mx-auto mb-4 h-12 w-12 text-red-600 dark:text-red-400" />
                    <h3 class="mb-2 text-lg font-medium text-red-900 dark:text-red-100">Preview Error</h3>
...
.device-tablet .preview-container {
    @apply h-[1024px] max-h-full w-[768px] max-w-full;
}

.device-mobile .preview-container {
    @apply h-[667px] max-h-full w-[375px] max-w-full;
}
...
/* Ruler styles */
.horizontal-ruler {
    background-image: repeating-linear-gradient(90deg, transparent, transparent 9px, rgba(107, 114, 128, 0.3) 9px, rgba(107, 114, 128, 0.3) 10px);
}

.vertical-ruler {
    background-image: repeating-linear-gradient(0deg, transparent, transparent 9px, rgba(107, 114, 128, 0.3) 9px, rgba(107, 114, 128, 0.3) 10px);
}
...
@media (max-width: 768px) {
    .device-tablet .preview-container,
    .device-mobile .preview-container {
        @apply h-auto min-h-[400px] w-full;
    }
}
