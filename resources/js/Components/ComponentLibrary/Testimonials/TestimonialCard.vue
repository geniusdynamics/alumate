<template>
    <article :class="cardClasses" role="article" :aria-labelledby="headingId" @click="handleCardClick">
        <!-- Video Thumbnail (for video testimonials) -->
        <div v-if="testimonial.content.type === 'video' && testimonial.content.videoAsset" class="group relative mb-4">
            <ResponsiveImage
                :src="videoThumbnail"
            ...
            <button
                ...
                @click="handleVideoClick"
                class="group absolute inset-0 flex items-center justify-center rounded-lg bg-black bg-opacity-30 transition-all duration-200 hover:bg-opacity-50"
                :aria-label="`Play video testimonial by ${testimonial.author.name}`"
            >
                <div class="relative">
                    <svg class="h-12 w-12 text-white transition-transform duration-200 group-hover:scale-110" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M8 5v14l11-7z" />
                    </svg>
                    <!-- Pulse animation -->
                    <div class="absolute inset-0 h-12 w-12 animate-ping rounded-full border-2 border-white opacity-30"></div>
                </div>
            </button>

            <!-- Video Duration Badge -->
            <div v-if="videoDuration" class="absolute bottom-2 right-2 rounded bg-black bg-opacity-70 px-2 py-1 text-xs text-white">
                {{ formatDuration(videoDuration) }}
            </div>

            <!-- Video Quality Badge -->
            <div v-if="videoQuality" class="absolute right-2 top-2 rounded bg-black bg-opacity-70 px-2 py-1 text-xs text-white">
                {{ videoQuality }}
            </div>
            ...
            <div
                ...
                class="absolute left-2 top-2 flex items-center rounded bg-black bg-opacity-70 px-2 py-1 text-xs text-white"
                title="Captions available"
            >
                <svg class="mr-1 h-3 w-3" fill="currentColor" viewBox="0 0 24 24">
                    <path
                        d="M19 4H5c-1.11 0-2 .9-2 2v12c0 1.1.89 2 2 2h14c1.11 0 2-.9 2-2V6c0-1.1-.89-2-2-2zm-8 7H9.5v-.5h-2v3h2V13H11v1c0 .55-.45 1-1 1H7c-.55 0-1-.45-1-1v-4c0-.55.45-1 1-1h3c.55 0 1 .45 1 1v1zm7 0h-1.5v-.5h-2v3h2V13H18v1c0 .55-.45 1-1 1h-3c-.55 0-1-.45-1-1v-4c0-.55.45-1 1-1h3c.55 0 1 .45 1 1v1z"
                    />
                </svg>
                CC
            </div>
        ...
        <div class="testimonial-card-content flex-1">
            ...
            <blockquote class="mb-4">
                <svg
                    v-if="theme !== 'minimal'"
                    class="mb-3 h-6 w-6 text-gray-400 dark:text-gray-500"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                >
            ...
            >
                <div class="flex space-x-1">
                    <svg
                        v-for="star in 5"
                        :key="`star-${star}`"
                        :class="['h-4 w-4', star <= testimonial.content.rating ? 'fill-current text-yellow-400' : 'text-gray-300 dark:text-gray-600']"
                        viewBox="0 0 20 20"
                        :aria-hidden="true"
                    >
                        <path
                ...
                <span class="ml-2 text-xs text-gray-600 dark:text-gray-400"> {{ testimonial.content.rating }}/5 </span>
            </div>
        </div>
        ...
        <footer class="testimonial-card-footer mt-auto pt-4">
            <div class="flex items-center space-x-3">
                ...
                <div class="min-w-0 flex-1">
                    <cite :id="headingId" class="block text-sm font-medium not-italic text-gray-900 dark:text-white">
                        {{ testimonial.author.name }}
                    </cite>

                    <div class="space-y-0.5 text-xs text-gray-600 dark:text-gray-400">
                        <div v-if="config.showAuthorTitle && testimonial.author.title" class="truncate">
                            {{ testimonial.author.title }}
                        </div>

                        <div v-if="config.showAuthorCompany && testimonial.author.company" class="truncate">
                            {{ testimonial.author.company }}
                        </div>

                        <div v-if="config.showAuthorGraduationYear && testimonial.author.graduationYear" class="text-gray-500 dark:text-gray-500">
                ...
                <div v-if="testimonial.content.verified" class="flex-shrink-0" :title="'Verified testimonial'">
                    <svg class="h-4 w-4 text-green-500" fill="currentColor" viewBox="0 0 20 20" aria-label="Verified">
                        <path
                            fill-rule="evenodd"
            ...
            <!-- Date -->
            <div v-if="config.showDate && testimonial.content.dateCreated" class="mt-2 text-xs text-gray-500 dark:text-gray-400">
