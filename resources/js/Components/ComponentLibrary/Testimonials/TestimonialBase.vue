<template>
    <section :class="testimonialSectionClasses" role="region" :aria-labelledby="sectionId" :aria-label="config.ariaLabel || 'Customer testimonials'">
        <!-- Section Header (optional) -->
        <div v-if="config.title || config.description" class="mb-8 text-center">
            <h2 v-if="config.title" :id="sectionId" class="mb-4 text-3xl font-bold text-gray-900 dark:text-white">
                {{ config.title }}
            </h2>
            <p v-if="config.description" class="mx-auto max-w-3xl text-lg text-gray-600 dark:text-gray-300">
                {{ config.description }}
            </p>
        </div>
        ...
        <!-- Loading State -->
        <div v-if="isLoading" class="grid gap-6" :class="getGridClasses()" role="status" aria-label="Loading testimonials">
            <TestimonialSkeleton v-for="n in getSkeletonCount()" :key="`skeleton-${n}`" :layout="config.layout" :theme="config.theme" />
        </div>
        ...
        <!-- Empty State -->
        <div v-else class="py-12 text-center" role="status" aria-label="No testimonials found">
            <div class="mx-auto max-w-md">
                <svg class="mx-auto mb-4 h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path
                ...
                <h3 class="mb-2 text-lg font-medium text-gray-900 dark:text-white">No testimonials found</h3>
                ...
                <button
                    ...
                    class="mt-4 inline-flex items-center rounded-md border border-transparent bg-indigo-100 px-4 py-2 text-sm font-medium text-indigo-600 hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:bg-indigo-900 dark:text-indigo-400 dark:hover:bg-indigo-800"
                >
                    Clear filters
                </button>
            </div>
        ...
        <div v-if="config.enableInfiniteScroll && hasMoreItems" class="mt-8 text-center">
            <button
                @click="loadMoreItems"
                :disabled="isLoadingMore"
                class="inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-6 py-3 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
            >
                <svg
                    v-if="isLoadingMore"
        ...
        <div v-if="config.announceUpdates" :aria-live="announceUpdates ? 'polite' : 'off'" :aria-atomic="true" class="sr-only">
...
const showPagination = computed(() => {
    return props.config.itemsPerPage && !props.config.enableInfiniteScroll && totalPages.value > 1;
});
...
const handleTestimonialInteraction = (event: { type: 'view' | 'like' | 'share' | 'play' | 'pause'; testimonial: Testimonial; data?: any }) => {
