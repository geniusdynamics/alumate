<template>
    <section :class="heroClasses" :style="heroStyles" role="banner" :aria-labelledby="headingId">
        <!-- Background Media -->
        <div v-if="config.backgroundMedia" class="absolute inset-0 overflow-hidden" :class="{ 'z-0': true }">
            <!-- Video Background -->
            <ResponsiveVideo
            ...
            <!-- Overlay -->
            <div v-if="config.backgroundMedia.overlay" class="absolute inset-0" :style="overlayStyles" />
        </div>

        <!-- Content Container -->
        <div class="container relative z-10 mx-auto h-full px-4 sm:px-6 lg:px-8">
            <div :class="contentContainerClasses">
                <div :class="contentWrapperClasses">
                    <!-- Heading -->
                    <component :is="`h${config.headingLevel}`" :id="headingId" :class="headlineClasses" v-html="config.headline" />

                    <!-- Subheading -->
                    <p v-if="config.subheading" :class="subheadingClasses" v-html="config.subheading" />

                    <!-- Description -->
                    <p v-if="config.description" :class="descriptionClasses" v-html="config.description" />

                    <!-- Statistics -->
                    <div v-if="config.statistics && config.statistics.length > 0" class="mt-8 sm:mt-12">
                        <StatisticsDisplay
                    ...
                    <!-- CTA Buttons -->
                    <div v-if="config.ctaButtons && config.ctaButtons.length > 0" class="mt-8 sm:mt-12">
                        <div :class="ctaContainerClasses">
                            ...
                            >
                                <Icon v-if="cta.icon" :name="cta.icon" class="mr-2 h-5 w-5" aria-hidden="true" />
                                {{ cta.text }}
                            </component>
                        </div>
                    </div>
        ...
        <div
            v-if="isLoading"
            class="absolute inset-0 z-20 flex items-center justify-center bg-gray-100 dark:bg-gray-900"
            role="status"
            aria-label="Loading hero content"
        >
            <div class="w-full max-w-2xl animate-pulse space-y-4 px-4">
                <div class="mx-auto h-8 w-3/4 rounded bg-gray-300 dark:bg-gray-700"></div>
                <div class="mx-auto h-4 w-1/2 rounded bg-gray-300 dark:bg-gray-700"></div>
                <div class="mx-auto h-4 w-2/3 rounded bg-gray-300 dark:bg-gray-700"></div>
            </div>
        </div>
    </section>
</template>
...
const heroStyles = computed(() => {
    ...
    if (!props.config.backgroundMedia) {
        ...
        const gradientStyles = getBackgroundGradient(props.config.variantStyling?.colorScheme, props.config.audienceType);
...
const gradientStyles = computed(() => {
    ...
    const colorStops = gradient.colors.map((c) => `${c.color} ${c.stop}%`).join(', ');
...
const getCtaButtonClasses = (cta: CTAButton) => {
    ...
    if (cta.style === 'primary') {
        return [...baseClasses, ...variantStyles.value.cta];
    }
