<template>
    <div :class="bannerClasses" :style="bannerStyles" @click="handleBannerClick">
        <!-- Background Image -->
        <ResponsiveImage
        ...
        <!-- Overlay -->
        <div v-if="config.overlay?.enabled" class="cta-banner__overlay" :style="overlayStyles" aria-hidden="true" />

        <!-- Content Container -->
        <div :class="contentContainerClasses">
            <div :class="contentClasses">
                <!-- Title -->
                <h2 v-if="config.title" :class="titleClasses">
                    {{ config.title }}
                </h2>

                <!-- Subtitle -->
                <h3 v-if="config.subtitle" :class="subtitleClasses">
                    {{ config.subtitle }}
                </h3>

                <!-- Description -->
                <p v-if="config.description" :class="descriptionClasses">
                    {{ config.description }}
                </p>

                <!-- CTA Buttons -->
                <div v-if="config.primaryCTA || config.secondaryCTA" :class="ctaContainerClasses">
                    <!-- Primary CTA -->
                    <CTAButton
...
<<<<<<< HEAD:resources/js/components/ComponentLibrary/CTAs/CTABanner.vue
import type { CTABannerConfig, CTAComponentConfig } from '@/Types/components';
=======
import type { CTABannerConfig, CTAComponentConfig } from '@/types/Components';
>>>>>>> origin/db1:resources/js/Components/ComponentLibrary/CTAs/CTABanner.vue
...
/* Container */
.cta-banner__container {
    @apply relative z-20 flex h-full items-center justify-center;
}
...
.cta-banner__subtitle {
    @apply mb-3 text-xl font-semibold md:text-2xl;
}

.cta-banner__description {
    @apply mb-6 max-w-2xl text-lg md:text-xl;
}
...
@media (max-width: 768px) {
    ...
    .cta-banner__container--mobile-overlay .cta-banner__content {
        @apply absolute inset-0 flex flex-col items-center justify-center p-4;
    }
}
