<template>
    ...
    >
        ...
        <div :class="['statistics-grid', `grid-${gridColumns.desktop}-${gridColumns.tablet}-${gridColumns.mobile}`, `gap-${spacing}`]">
            <slot :statistics="processedStatistics" :is-visible="isVisible" :animate="shouldAnimate" />
        </div>

        <!-- Error state -->
        <div v-if="hasErrors && showErrors" class="statistics-error" role="alert" aria-live="polite">
            <p class="error-message">{{ errorMessage }}</p>
            <button v-if="allowRetry" @click="retryDataLoad" class="retry-button" type="button">Retry Loading Data</button>
        </div>

        <!-- Loading state -->
        <div v-if="isLoading" class="statistics-loading" aria-live="polite" aria-label="Loading statistics data">
            <div class="loading-skeleton">
                <div v-for="n in skeletonCount" :key="n" class="skeleton-item" :style="{ animationDelay: `${n * 0.1}s` }" />
            </div>
        </div>
    </div>
</template>
...
const titleClasses = computed(() => ['statistics-title', `theme-${theme.value}`]);
...
.statistics-description {
    @apply mx-auto max-w-2xl text-gray-600 dark:text-gray-300;
}
...
.statistics-error {
    @apply rounded-lg border border-red-200 bg-red-50 p-4 text-center dark:border-red-800 dark:bg-red-900/20;
}

.error-message {
    @apply mb-3 text-red-700 dark:text-red-300;
}

.retry-button {
    @apply rounded-md bg-red-600 px-4 py-2 text-white transition-colors duration-200 hover:bg-red-700;
}
...
.skeleton-item {
    @apply h-24 animate-pulse rounded-lg bg-gray-200 dark:bg-gray-700;
}
...
.theme-modern {
    @apply rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 p-8 dark:from-blue-900/20 dark:to-indigo-900/20;
}
