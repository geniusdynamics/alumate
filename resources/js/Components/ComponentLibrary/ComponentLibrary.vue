<template>
    <div class="component-library" :class="containerClasses" role="application" :aria-label="ariaLabel">
        <!-- Header Section -->
        <header class="component-library__header">
            <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-gray-600 dark:text-gray-300">Browse and customize Components for your pages</p>
                </div>
                ...
                <div class="relative max-w-md flex-1">
                    ...
                    <input
                        ...
                        class="block w-full rounded-md border border-gray-300 bg-white py-2 pl-10 pr-3 leading-5 placeholder-gray-500 focus:border-indigo-500 focus:placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400"
        ...
        <!-- Category Navigation -->
        <nav class="component-library__nav" role="tablist" :aria-label="navAriaLabel">
            <div class="border-b border-gray-200 dark:border-gray-700">
                <div class="scrollbar-hide flex space-x-8 overflow-x-auto">
                    <button
                    ...
                    >
                        <Icon :name="category.icon" class="mr-2 h-5 w-5" :aria-hidden="true" />
        ...
        <!-- Filters and Actions -->
        <div class="component-library__filters flex flex-col gap-4 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center space-x-4">
                ...
                <select
                    v-model="sortBy"
                    class="rounded-md border border-gray-300 bg-white px-3 py-1 text-sm text-gray-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
            ...
            <div class="flex items-center space-x-2">
                ...
                >
                    <Icon name="heart" class="mr-1 h-4 w-4" />
                    Favorites
                </button>

                <button
                    @click="clearRecentlyUsed"
                    class="rounded-md px-3 py-1 text-sm text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"
        ...
        <main class="component-library__content">
            <!-- Loading State -->
            <div v-if="isLoading" class="grid gap-6" :class="getGridClasses()" role="status" aria-label="Loading Components">
                <ComponentSkeleton v-for="n in 12" :key="`skeleton-${n}`" :view-mode="viewMode" />
            </div>

            <!-- Empty State -->
            <div v-else-if="filteredComponents.length === 0" class="py-12 text-center" role="status">
                <div class="mx-auto max-w-md">
                    <Icon name="search" class="mx-auto mb-4 h-12 w-12 text-gray-400" aria-hidden="true" />
                    ...
                    <button
                        ...
                        class="inline-flex items-center rounded-md border border-transparent bg-indigo-100 px-4 py-2 text-sm font-medium text-indigo-600 hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:bg-indigo-900 dark:text-indigo-400 dark:hover:bg-indigo-800"
                    >
                        Clear filters
                    </button>
                </div>
            ...
            <!-- Components Display -->
            <div v-else :class="getContentClasses()" role="tabpanel" :id="`panel-${activeCategory}`" :aria-labelledby="`tab-${activeCategory}`">
                <ComponentCard
            ...
            <div v-if="totalPages > 1" class="mt-8 flex items-center justify-between">
                ...
                <nav class="flex items-center space-x-2" aria-label="Pagination">
                    <button
                        ...
                        class="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600"
...
const ariaLabel = computed(() => `Component library with ${props.Components.length} Components across ${categories.value.length} categories`);

const searchAriaLabel = computed(() => `Search ${filteredComponents.value.length} Components`);

const navAriaLabel = computed(() => 'Component categories');
...
const filteredComponents = computed(() => {
    ...
    if (searchQuery.value.trim()) {
        ...
        Components = Components.filter(
            (c) => c.name.toLowerCase().includes(query) || c.description?.toLowerCase().includes(query) || c.type.toLowerCase().includes(query),
        );
    }
...
const totalPages = computed(() => Math.ceil(filteredComponents.value.length / itemsPerPage.value));

const startIndex = computed(() => (currentPage.value - 1) * itemsPerPage.value);

const endIndex = computed(() => Math.min(startIndex.value + itemsPerPage.value, filteredComponents.value.length));
...
const hasActiveFilters = computed(() => searchQuery.value.trim() !== '' || showFavoritesOnly.value);
...
const setActiveCategory = (categoryId: ComponentCategory) => {
    ...
    announceToScreenReader(
        `Switched to ${categories.value.find((c) => c.id === categoryId)?.name} category. ${getCategoryCount(categoryId)} Components available.`,
    );
