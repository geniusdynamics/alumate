<template>
    <div :class="containerClasses" role="region" :aria-label="ariaLabel">
        ...
        <div v-if="config.title || config.description" class="mb-6">
            <h2 v-if="config.title" :class="titleClasses" :id="titleId">
                {{ config.title }}
            </h2>
            <p v-if="config.description" class="mt-2 text-gray-600 dark:text-gray-400" :aria-describedby="titleId">
                {{ config.description }}
            </p>
        </div>
        ...
        <!-- Loading State -->
        <div v-if="isLoading" class="absolute inset-0 flex items-center justify-center bg-white bg-opacity-90 dark:bg-gray-900 dark:bg-opacity-90">
        ...
        <!-- Error State -->
        <div v-if="hasError" class="absolute inset-0 flex items-center justify-center bg-gray-50 p-4 dark:bg-gray-800">
            <div class="text-center">
                <svg class="mx-auto mb-4 h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <p class="mb-4 text-sm text-gray-600 dark:text-gray-400">{{ errorMessage }}</p>
                <button
                    @click="retryLoad"
                    class="rounded-lg bg-indigo-600 px-4 py-2 text-white transition-colors hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                >
                    Try Again
                </button>
            </div>
...
const ariaLabel = computed(() => {
    return props.config.accessibility?.ariaLabel || props.config.title || 'Media component';
});
