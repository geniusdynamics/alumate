<template>
    ...
    >
        <!-- Video Grid/Layout -->
        <div :class="videoContainerClasses" role="region" :aria-label="videoContainerAriaLabel">
            <div v-for="(video, index) in optimizedVideos" :key="video.id" :class="videoItemClasses">
                <!-- Video Player -->
                <ResponsiveVideo
                    :src="video"
                ...
                >
                    <h3 v-if="video.title" class="mb-1 text-lg font-semibold text-white">
                        {{ video.title }}
                    </h3>
                    <p v-if="video.description" class="line-clamp-2 text-sm text-gray-200">
                        {{ video.description }}
                    </p>

                    <!-- Video Duration -->
                    <div v-if="video.videoAsset?.duration" class="absolute right-4 top-4 rounded bg-black bg-opacity-70 px-2 py-1 text-xs text-white">
                        {{ formatDuration(video.videoAsset.duration) }}
                    </div>
                </div>

                <!-- Video Loading State -->
                <div v-if="videoLoadingStates[index]" class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50">
                ...
                <!-- Video Error State -->
                <div v-if="videoErrorStates[index]" class="absolute inset-0 flex items-center justify-center bg-gray-900 p-4 text-white">
                    <div class="text-center">
                        ...
                        <p class="mb-2 text-sm">Unable to load video</p>
                        <button @click="retryVideo(index)" class="rounded bg-indigo-600 px-4 py-2 text-white transition-colors hover:bg-indigo-700">
                            Retry
                        </button>
                    </div>
                </div>
                ...
                <div v-if="config.accessibility.keyboardNavigation" class="sr-only" :aria-live="videoPlayingStates[index] ? 'polite' : 'off'">
        ...
        <div v-if="optimizedVideos.length > 1 && showPlaylist" class="mt-6">
            <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-white">Video Playlist</h3>
            <div class="space-y-2">
                <button
                    ...
                    :class="[
                        ...
                        {
                            'border border-indigo-200 bg-indigo-50 dark:border-indigo-800 dark:bg-indigo-900/20': currentVideoIndex === index,
                ...
                >
                    <!-- Video Thumbnail -->
                    <div class="h-12 w-16 flex-shrink-0 overflow-hidden rounded bg-gray-200 dark:bg-gray-700">
                        <ResponsiveImage
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="mt-1 flex items-center space-x-2">
                            <span v-if="video.videoAsset?.duration" class="text-xs text-gray-500 dark:text-gray-400">
                                {{ formatDuration(video.videoAsset.duration) }}
                            </span>
                            <span
                    ...
                    <div class="flex-shrink-0">
                        <svg v-if="!videoPlayingStates[index]" class="h-5 w-5 text-gray-400" fill="currentColor" viewBox="0 0 24 24">
...
<<<<<<< HEAD:resources/js/components/ComponentLibrary/Media/VideoEmbed.vue
import type { MediaAsset, MediaComponentConfig } from '@/Types/components';
=======
import type { MediaAsset, MediaComponentConfig } from '@/types/Components';
>>>>>>> origin/db1:resources/js/Components/ComponentLibrary/Media/VideoEmbed.vue
...
const optimizedVideos = computed(() => {
    return props.config.mediaAssets
...
const videoContainerAriaLabel = computed(() => {
    return props.config.accessibility?.ariaLabel || `Video collection with ${optimizedVideos.value.length} videos`;
});
...
const generateMobileVideoVariant = (asset: MediaAsset) => {
    ...
    const quality =
        props.config.optimization.compressionLevel === 'high' ? 'high' : props.config.optimization.compressionLevel === 'medium' ? 'medium' : 'low';
