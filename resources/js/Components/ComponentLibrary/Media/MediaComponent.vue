<template>
    <div :class="containerClasses" role="region" :aria-label="containerAriaLabel">
        <!-- Image Gallery -->
        <ImageGallery
        ...
        <div v-else class="flex h-64 items-center justify-center rounded-lg bg-gray-100 dark:bg-gray-800">
            <div class="text-center">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Unsupported media type: {{ config.type }}</p>
            </div>
        </div>
    </div>
</template>
...
const containerAriaLabel = computed(() => {
    ...
    return props.config.accessibility?.ariaLabel || typeLabels[props.config.type] || 'Media component';
});
