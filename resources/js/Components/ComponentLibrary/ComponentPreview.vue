<template>
    <div class="component-preview" :class="containerClasses" role="application" :aria-label="ariaLabel">
        <!-- Preview Header -->
                ...
                <div class="flex items-center space-x-4">
                    <div class="flex items-center space-x-2">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ getCategoryName(component.category) }} Component Preview</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <div class="flex items-center space-x-1">
                            <button
                                ...
                                class="p-1 text-gray-500 hover:text-gray-700 disabled:cursor-not-allowed disabled:opacity-50 dark:text-gray-400 dark:hover:text-gray-300"
                                aria-label="Zoom out"
                            >
                                <Icon name="minus" class="h-4 w-4" />
                            </button>
                            <span class="min-w-[3rem] text-center text-sm text-gray-600 dark:text-gray-400">
        ...
        <div class="component-preview__content flex flex-1 overflow-hidden">
            ...
            <main class="flex flex-1 flex-col overflow-hidden">
                <!-- Preview Toolbar -->
                <div
                    class="preview-toolbar flex items-center justify-between border-b border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800"
                >
                    <div class="flex items-center space-x-4">
                        <!-- Refresh Preview -->
                        <button
                            ...
                            class="inline-flex items-center px-3 py-1 text-sm text-gray-600 hover:text-gray-900 disabled:opacity-50 dark:text-gray-400 dark:hover:text-white"
                            aria-label="Refresh preview"
                        >
                            <Icon :name="isRefreshing ? 'arrow-path' : 'arrow-path'" :class="['mr-1 h-4 w-4', { 'animate-spin': isRefreshing }]" />
                            Refresh
                        </button>
                        ...
                        >
                            <Icon
                                ...
                                :class="['mr-1 h-4 w-4', { 'animate-spin': isRunningA11yTest }]"
                            />
                            A11y Test
                        </button>
                        ...
                        >
                            <Icon
                                ...
                                :class="['mr-1 h-4 w-4', { 'animate-spin': isRunningPerfTest }]"
                            />
                            Performance
                        </button>
                    </div>
                ...
                <div class="preview-frame-container flex flex-1 items-center justify-center overflow-auto bg-gray-100 p-6 dark:bg-gray-900">
                    <div
                        :class="getPreviewFrameClasses()"
                        :style="getPreviewFrameStyles()"
                        class="preview-frame overflow-hidden rounded-lg bg-white shadow-lg transition-all duration-300 dark:bg-gray-800"
                    >
                        <!-- Device Frame (for mobile/tablet) -->
                        <div v-if="previewDevice !== 'desktop'" class="device-frame" :class="getDeviceFrameClasses()">
                            <!-- Device Header -->
                            <div class="device-header flex items-center justify-center bg-gray-800 p-2">
                                <div class="flex items-center space-x-1">
                ...
                <!-- Preview Status Bar -->
                <div
                    class="preview-status-bar flex items-center justify-between border-t border-gray-200 bg-gray-50 p-3 text-sm dark:border-gray-700 dark:bg-gray-800"
                >
                    <div class="flex items-center space-x-4">
                        <!-- Loading Status -->
                        <div v-if="isLoading" class="flex items-center text-gray-600 dark:text-gray-400">
                            <Icon name="arrow-path" class="mr-2 h-4 w-4 animate-spin" />
                            Loading preview...
                        </div>
                    ...
                    <div v-if="accessibilityScore" class="flex items-center space-x-2">
                        <Icon
                            ...
                            class="h-4 w-4"
                        />
                        <span :class="getAccessibilityScoreClasses(accessibilityScore.score)"> A11y: {{ accessibilityScore.score }}/100 </span>
                    </div>
                </div>
            </main>

            <!-- Configuration Sidebar -->
            <aside class="component-preview__sidebar w-80 overflow-auto border-l border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
                <div class="p-6">
                    <!-- Configuration Tabs -->
                    <div class="mb-6">
                    ...
                    <div class="config-panels">
                        <!-- Sample Data Panel -->
                        <div v-show="activeConfigTab === 'data'" id="config-panel-data" role="tabpanel" aria-labelledby="tab-data">
                            <h3 class="mb-4 text-lg font-medium text-gray-900 dark:text-white">Sample Data Configuration</h3>
                            ...
                            <div v-if="useSampleData" class="space-y-4">
                                ...
                                <div v-if="component.category === 'hero' || component.category === 'testimonials'">
                                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"> Audience Type </label>
                                    <select
                                ...
                                <!-- Data Variation -->
                                <div>
                                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"> Data Variation </label>
                                    <select
                                ...
                                <!-- Content Length -->
                                <div>
                                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"> Content Length </label>
                                    <select
                        ...
                        <!-- Theme Panel -->
                        <div v-show="activeConfigTab === 'theme'" id="config-panel-theme" role="tabpanel" aria-labelledby="tab-theme">
                            <h3 class="mb-4 text-lg font-medium text-gray-900 dark:text-white">Theme Configuration</h3>
                        ...
                        <!-- Settings Panel -->
                        <div v-show="activeConfigTab === 'settings'" id="config-panel-settings" role="tabpanel" aria-labelledby="tab-settings">
                            <h3 class="mb-4 text-lg font-medium text-gray-900 dark:text-white">Preview Settings</h3>
                            ...
                            <!-- Show Grid -->
                            <div class="mb-4 flex items-center justify-between">
                                <label class="text-sm font-medium text-gray-700 dark:text-gray-300"> Show Grid </label>
                                <button
                                    @click="toggleGrid"
                            ...
                            <!-- Show Rulers -->
                            <div class="mb-4 flex items-center justify-between">
                                <label class="text-sm font-medium text-gray-700 dark:text-gray-300"> Show Rulers </label>
                                <button
                                    @click="toggleRulers"
                        ...
                        <!-- Test Results Panel -->
                        <div v-show="activeConfigTab === 'tests'" id="config-panel-tests" role="tabpanel" aria-labelledby="tab-tests">
                            ...
                            <div v-if="accessibilityScore" class="mb-6">
                                <h4 class="mb-3 text-sm font-medium text-gray-700 dark:text-gray-300">Accessibility Score</h4>
                                <div class="rounded-lg bg-gray-50 p-4 dark:bg-gray-700">
                                    ...
                                    <div v-if="accessibilityScore.issues.length > 0" class="space-y-2">
                                        <h5 class="text-xs font-medium uppercase tracking-wide text-gray-600 dark:text-gray-400">Issues Found</h5>
                                        <ul class="space-y-1">
                                            <li
                            ...
                            <div v-if="performanceMetrics" class="mb-6">
                                <h4 class="mb-3 text-sm font-medium text-gray-700 dark:text-gray-300">Performance Metrics</h4>
        ...
        <!-- Share Modal -->
        <SharePreviewModal v-if="showShareModal" :preview-url="shareableUrl" :component="component" @close="closeShareModal" />
...
const ariaLabel = computed(() => `Component preview for ${props.component.name} - ${getCategoryName(props.component.category)} component`);
...
const getDarkModeToggleThumbClasses = () => [isDarkMode.value ? 'translate-x-5' : 'translate-x-0'];
...
const getAutoRefreshToggleThumbClasses = () => [autoRefresh.value ? 'translate-x-5' : 'translate-x-0'];
...
const getGridToggleThumbClasses = () => [showGrid.value ? 'translate-x-5' : 'translate-x-0'];
...
const getRulersToggleThumbClasses = () => [showRulers.value ? 'translate-x-5' : 'translate-x-0'];
...
const getAccessibilityIconClasses = (score: number) => [score >= 90 ? 'text-green-500' : score >= 70 ? 'text-yellow-500' : 'text-red-500'];

const getAccessibilityScoreClasses = (score: number) => [
    'text-sm font-medium',
    score >= 90 ? 'text-green-600 dark:text-green-400' : score >= 70 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400',
];
...
.device-screen {
    @apply overflow-hidden rounded-b-md bg-white;
}
...
.preview-frame.show-rulers::after {
    content: '';
    @apply pointer-events-none absolute left-0 right-0 top-0 z-10 h-4;
    background: linear-gradient(90deg, transparent 0px, rgba(0, 0, 0, 0.2) 1px, transparent 1px);
