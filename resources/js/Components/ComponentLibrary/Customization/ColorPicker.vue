            ...
            >
                ...
                <div class="color-picker-content" ref="dialogContent" @keydown.esc="closeDialog">
                    ...
                    <div class="color-picker-header">
                        ...
                        <button type="button" @click="closeDialog" class="color-picker-close" aria-label="Close color picker">
                    ...
                    <div class="color-picker-interface">
                        ...
                        <div class="color-palette" ref="saturationLightness" @mousedown="startDrag" @touchstart="startDrag">
                            <div class="color-palette-overlay" :style="{ backgroundColor: hslToHex(hue, 1, 0.5) }">
                    ...
                    <div class="color-preview-section">
                        ...
                        <div class="color-preview">
                            <div class="color-swatch-large" :style="{ backgroundColor: currentDisplayColor }"></div>
                        ...
                        <div class="input-mode-toggle" v-if="showAlpha">
                            <button type="button" @click="inputMode = 'rgb'" :class="{ active: inputMode === 'rgb' }" class="mode-button">RGB</button>
                            <button type="button" @click="inputMode = 'hsl'" :class="{ active: inputMode === 'hsl' }" class="mode-button">HSL</button>
                        </div>
                    </div>
                    ...
                    <div class="color-picker-actions">
                        <button type="button" @click="resetToInitialColor" class="btn-secondary">Reset</button>
                        <div class="action-group">
                            <button type="button" @click="closeDialog" class="btn-secondary">Cancel</button>
                            <button type="button" @click="confirmColor" class="btn-primary" :disabled="!isValidColor">Apply Color</button>
                        </div>
                    </div>
                </div>
            </div>
...
const dragStart: { x: number; y: number; element: HTMLElement } | null = null;
...
const validateHex = () => {
    if (!isValidColor.value) {
        ...
        if (props.showAlpha) {
            hexValue.value += Math.round(alpha.value * 255)
                .toString(16)
                .padStart(2, '0');
        }
    }
...
.color-current {
    @apply flex items-center gap-3 rounded-lg border border-gray-300 bg-white p-3 shadow-sm dark:border-gray-600 dark:bg-gray-800;
}

.color-swatch {
    @apply h-8 w-8 cursor-pointer rounded-md border-2 border-gray-300 transition-transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2;
}

.color-value {
    @apply min-w-20 font-mono text-sm text-gray-700 dark:text-gray-300;
}
...
.color-picker-content {
    @apply relative max-h-[90vh] w-full max-w-sm overflow-hidden rounded-lg bg-white shadow-xl dark:bg-gray-800;
}
...
.color-picker-close {
    @apply rounded-md p-1 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-700;
}
...
.color-palette {
    @apply relative h-40 w-full cursor-crosshair overflow-hidden rounded-lg bg-gray-200;
...
.color-cursor {
    @apply pointer-events-none absolute -ml-2.5 -mt-2.5 h-5 w-5 rounded-full border-2 border-white shadow-md;
}

.hue-slider {
    @apply relative h-4 w-full cursor-col-resize overflow-hidden rounded-lg;
    background: linear-gradient(90deg, #ff0000 0%, #ffff00 16.67%, #00ff00 33.33%, #00ffff 50%, #0000ff 66.67%, #ff00ff 83.33%, #ff0000 100%);
}

.hue-cursor {
    @apply pointer-events-none absolute top-0 -ml-1.5 h-full w-3 border border-gray-300 bg-white shadow-sm;
}
...
.alpha-slider-bg {
    ...
    background-image:
        linear-gradient(90deg, #808080 0%, transparent 100%),
...
.color-preview-section {
    @apply space-y-4 px-4 pb-4;
}

.color-preview {
    @apply flex items-center gap-4 rounded-lg bg-gray-50 p-4 dark:bg-gray-700;
}

.color-swatch-large {
    @apply h-12 w-12 rounded-lg border-2 border-gray-300 shadow-sm;
}
...
.color-input {
    @apply rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:bg-gray-700 dark:text-white;
}
...
.input-mode-toggle {
    @apply flex gap-1 rounded-md bg-gray-100 p-1 dark:bg-gray-800;
}

.mode-button {
    @apply rounded px-3 py-1 text-xs font-medium text-gray-600 transition-colors hover:bg-gray-200 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-200;
}
...
.preset-color {
    @apply h-6 w-6 rounded border border-gray-300 transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1;
}

/* Actions */
.color-picker-actions {
    @apply flex items-center justify-between border-t border-gray-200 bg-gray-50 px-4 py-4 dark:border-gray-600 dark:bg-gray-700;
}
...
.btn-primary {
    @apply rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50;
}

.btn-secondary {
    @apply rounded-md bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700;
}
