<template>
    <div :class="['achievement-badge', `rarity-${achievement.rarity}`, { featured: isFeatured, earned: isEarned }]" @click="$emit('click')">
        <div class="badge-container">
            <div class="badge-icon">
                <Icon :name="achievement.icon || achievement.category_icon" :class="['icon', `text-${rarityColor}-500`]" />
            </div>
            ...
            <div class="badge-content">
                ...
                <div v-if="earnedAt" class="earned-date">Earned {{ formatDate(earnedAt) }}</div>
            </div>
            ...
            <div v-if="showActions" class="badge-actions">
                <button
                    ...
                    :class="['feature-btn', { featured: isFeatured }]"
...
<style scoped>
.achievement-badge {
    @apply cursor-pointer rounded-lg border-2 border-gray-200 bg-white p-4 shadow-md transition-all duration-200 hover:shadow-lg;
}
...
.badge-icon {
    @apply flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-gray-100;
}
...
.badge-meta span {
    @apply rounded-full bg-gray-100 px-2 py-1 text-gray-700;
}
...
.feature-btn {
    @apply rounded-full p-2 text-gray-400 transition-colors hover:text-yellow-500;
}
...
.progress-bar {
    @apply relative mt-3 h-2 rounded-full bg-gray-200;
}

.progress-fill {
    @apply h-full rounded-full bg-blue-500 transition-all duration-300;
}
