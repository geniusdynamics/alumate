    ...
    <div class="peak-usage-chart">
        <div class="chart-header">
            ...
            <div class="chart-toggle">
                <button @click="viewMode = 'hourly'" class="toggle-button" :class="{ active: viewMode === 'hourly' }">Hourly</button>
                <button @click="viewMode = 'daily'" class="toggle-button" :class="{ active: viewMode === 'daily' }">Daily</button>
            </div>
        </div>
        ...
        <div class="chart-container">
            <div v-if="viewMode === 'hourly'" class="hourly-chart">
                <div class="hour-bars">
                    <div v-for="hour in hourlyData" :key="hour.hour" class="hour-bar">
                        <div class="hour-fill" :style="{ height: `${getHourPercentage(hour.usage)}%` }" :class="getHourColor(hour.hour)"></div>
            ...
            <div v-else class="daily-chart">
                <div class="day-items">
                    <div v-for="(usage, day) in data.daily" :key="day" class="day-item">
                        <div class="day-info">
                            <span class="day-name">{{ formatDayName(day) }}</span>
                            <span class="day-usage">{{ usage }}</span>
                        </div>
                        <div class="day-bar">
                            <div class="day-fill" :style="{ width: `${getDayPercentage(usage)}%` }" :class="getDayColor(day)"></div>
                        </div>
                    </div>
                </div>
            </div>
...
.hour-bars {
    @apply flex h-full items-end justify-between space-x-1;
}

.hour-bar {
    @apply flex flex-1 flex-col items-center space-y-1;
}
