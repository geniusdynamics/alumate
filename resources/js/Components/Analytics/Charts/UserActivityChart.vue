...
const peakDay = computed(() => {
    ...
    const peak = processedData.value.reduce((max, current) => (current.count > max.count ? current : max));
...
.period-select {
    @apply rounded-md border border-gray-300 px-3 py-1 text-sm dark:border-gray-600;
...
.chart-stats {
    @apply grid grid-cols-3 gap-4 border-t border-gray-200 pt-4 dark:border-gray-600;
}
