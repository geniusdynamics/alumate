<template>
    <div class="rounded-lg bg-white p-6 shadow-sm dark:bg-gray-800">
        ...
        <div v-if="trends.growth_rate !== undefined" class="mt-4 flex items-center justify-between text-sm">
            ...
            <span :class="['font-medium', trends.growth_rate >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400']">
        ...
        <div v-if="trends.forecasted_next_month" class="mt-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            <span class="ml-1 text-xs"> ({{ trends.forecasted_next_month.confidence }} confidence) </span>
        </div>
    </div>
</template>
...
const chartData = computed(() => {
    ...
    return Object.entries(props.trends.monthly_trends)
        .map(([date, data]) => ({
            date,
            amount: data.amount,
            count: data.count,
