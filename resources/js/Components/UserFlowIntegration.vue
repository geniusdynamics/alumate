    ...
    <div class="user-flow-integration">
        ...
        <div v-if="notifications.length > 0" class="fixed right-4 top-4 z-50 space-y-2">
            <div
                ...
                class="max-w-sm rounded-md p-4 shadow-lg transition-all duration-300"
            >
                <div class="flex items-center justify-between">
                    <p class="text-sm font-medium">{{ notification.message }}</p>
                    <button @click="dismissNotification(notification.id)" class="ml-2 text-white hover:text-gray-200">
        ...
        <div v-if="jobUpdates.length > 0" class="job-updates">
            <div v-for="update in jobUpdates" :key="update.id" class="job-update" :data-job-id="update.jobId" :data-saved="update.saved">
                <!-- This will be used by the UserFlowIntegration service to update UI -->
            </div>
        </div>
