<template>
    <div class="location-filter">
        <div class="filter-section">
            ...
            <select v-model="localFilters.graduation_year" multiple class="form-select form-select-sm" @change="emitFilters">
        ...
        <div class="filter-section">
            ...
            <select v-model="localFilters.industry" multiple class="form-select form-select-sm" @change="emitFilters">
        ...
        <div class="filter-section">
            ...
            <select v-model="localFilters.country" multiple class="form-select form-select-sm" @change="emitFilters">
        ...
        <div class="filter-actions">
            ...
            <button class="btn btn-sm btn-primary" @click="toggleAdvanced">
        ...
        <div v-if="showAdvanced" class="advanced-filters">
            ...
            <div class="filter-section">
                ...
                <div class="distance-filter">
                    <input v-model.number="localFilters.distance" type="range" min="5" max="500" step="5" class="form-range" @input="emitFilters" />
                ...
                <div class="visibility-options">
                    ...
                    <div class="form-check form-check-inline">
                        <input
                            ...
                            @change="emitFilters"
                        />
                        <label class="form-check-label" for="visibility-alumni"> Alumni Only </label>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div v-if="hasActiveFilters" class="active-filters">
            ...
            <div class="filter-tags">
                <span v-for="year in localFilters.graduation_year" :key="`year-${year}`" class="filter-tag">
                    {{ year }}
                    <button @click="removeFilter('graduation_year', year)">×</button>
                </span>

                <span v-for="industry in localFilters.industry" :key="`industry-${industry}`" class="filter-tag">
                    {{ industry }}
                    <button @click="removeFilter('industry', industry)">×</button>
                </span>

                <span v-for="country in localFilters.country" :key="`country-${country}`" class="filter-tag">
                    {{ country }}
                    <button @click="removeFilter('country', country)">×</button>
                </span>

                <span v-for="state in localFilters.state" :key="`state-${state}`" class="filter-tag">
                    {{ state }}
...
const hasActiveFilters = computed(() => {
    return (
        localFilters.value.graduation_year.length > 0 ||
...
const loadFilterOptions = async () => {
    try {
        ...
        // Fallback data
        graduationYears.value = Array.from({ length: 50 }, (_, i) => new Date().getFullYear() - i);
        ...
        countries.value = ['United States', 'Canada', 'United Kingdom', 'Germany', 'France', 'Australia', 'Japan', 'Singapore', 'India', 'Brazil'];

        states.value = ['California', 'New York', 'Texas', 'Florida', 'Illinois', 'Pennsylvania', 'Ohio', 'Georgia', 'North Carolina', 'Michigan'];
    }
};
