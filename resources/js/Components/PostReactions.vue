    ...
    <div class="post-reactions">
        ...
        <div class="reaction-buttons mb-2 flex items-center space-x-2">
            <button
                ...
                :class="[
                    'reaction-btn flex items-center space-x-1 rounded-full px-3 py-1 text-sm transition-all duration-200',
        ...
        <!-- Reaction Summary -->
        <div v-if="totalReactions > 0" class="reaction-summary mb-2 text-sm text-gray-600">
            <button @click="showReactionModal = true" class="flex items-center space-x-1 hover:underline">
                <div class="flex -space-x-1">
                    <span v-for="(reaction, index) in topReactions" :key="reaction.type" :class="reaction.icon" class="text-xs"></span>
                </div>
        ...
        >
            <div class="mx-4 max-h-96 w-full max-w-md overflow-y-auto rounded-lg bg-white p-6" @click.stop>
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="text-lg font-semibold">Reactions</h3>
                    <button @click="showReactionModal = false" class="text-gray-400 hover:text-gray-600">
                ...
                <!-- Users List -->
                <div class="space-y-2">
                    <div v-for="user in reactionUsers" :key="user.id" class="flex items-center space-x-3">
                        <img :src="user.avatar_url || '/default-avatar.png'" :alt="user.name" class="h-8 w-8 rounded-full" />
                        <div>
                            <div class="text-sm font-medium">{{ user.name }}</div>
                            <div class="text-xs text-gray-500">@{{ user.username }}</div>
                        </div>
                    </div>
                    <div v-if="loadingUsers" class="py-4 text-center">
