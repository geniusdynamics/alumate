    ...
    <div class="content-editor">
        ...
        <div class="editor-toolbar">
            ...
            <div class="toolbar-tools">
                ...
                <div class="history-tools">
                    <button @click="undo" :disabled="!canUndo" class="tool-btn" :aria-label="`Undo ${lastUndoAction}`" title="Undo (Ctrl+Z)">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" />
                        </svg>
                    </button>
                    <button @click="redo" :disabled="!canRedo" class="tool-btn" :aria-label="`Redo ${lastRedoAction}`" title="Redo (Ctrl+Y)">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="save-indicator" :class="{ saving: isSaving, saved: justSaved }">
                    ...
                    <svg v-else-if="justSaved" class="h-4 w-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="editor-content">
            <div
                ...
                :class="{ 'view-visual': activeView === 'visual', 'view-code': activeView === 'code', 'view-split': activeView === 'split' }"
            >
                <!-- Visual Editor -->
                <div class="visual-editor" v-if="activeView === 'visual' || activeView === 'split'">
                    <div class="blocks-container" :class="{ 'dragging-over': isDragOver }">
                        ...
                        <!-- Add Block Drop Zone -->
                        <div
                            class="add-block-drop-zone"
                            :class="{ active: showBlockDropZone }"
                            @drop="onBlockDrop($event, content.blocks.length)"
                            @dragover.prevent
                        >
                ...
                <div class="code-editor" v-if="activeView === 'code' || activeView === 'split'">
                    ...
                    <textarea v-model="contentJson" class="json-editor" spellcheck="false" :aria-label="`Content structure JSON editor`"></textarea>
        ...
        <div v-if="showSettings" class="settings-panel">
            ...
            <div class="settings-content">
                ...
                <div class="setting-group">
                    ...
                    <label class="setting-label">
                        <input v-model="settings.realTimePreview" type="checkbox" class="setting-checkbox" />
                ...
                <div class="setting-group">
                    <h4>Accessibility</h4>
                    <label class="setting-label">
                        <input v-model="settings.showAccessibilityWarnings" type="checkbox" class="setting-checkbox" />
...
.add-block-btn {
    @apply flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-white hover:bg-blue-700;
    @apply focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 dark:focus:ring-offset-gray-800;
...
.block-type-btn {
    @apply flex items-center gap-3 rounded-lg border border-gray-200 p-3 dark:border-gray-600;
    @apply bg-gray-50 transition-colors hover:bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600;
    @apply focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 dark:focus:ring-offset-gray-800;
    @apply text-left focus:outline-none;
}

.block-type-icon {
    @apply h-5 w-5 flex-shrink-0 text-gray-600 dark:text-gray-400;
}
...
.content-block {
    @apply relative overflow-hidden rounded-lg border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800;
    @apply transition-all duration-200 hover:border-gray-300 hover:shadow-md dark:hover:border-gray-600;
    @apply focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 dark:focus:ring-offset-gray-800;
...
.block-controls {
    @apply absolute left-3 top-3 z-10 flex items-center gap-2 rounded bg-white px-2 py-1 opacity-0 shadow transition-opacity dark:bg-gray-800;
}

.block-grip {
    @apply cursor-move p-1 text-gray-400;
}

.block-control-btn {
    @apply rounded p-1 text-gray-400 transition-colors hover:text-gray-600 dark:hover:text-gray-300;
}
...
.block-content {
    @apply cursor-text p-6;
}

.block-hidden-indicator {
    @apply absolute right-2 top-2 rounded bg-gray-100 px-2 py-1 text-xs text-gray-600 dark:bg-gray-700 dark:text-gray-400;
}
...
.add-block-drop-zone {
    @apply rounded-lg border-2 border-dashed border-gray-300 py-12 transition-colors dark:border-gray-600;
    @apply pointer-events-none opacity-0;
}

.add-block-drop-zone.active {
    @apply pointer-events-auto border-blue-500 bg-blue-50 opacity-100 dark:bg-blue-900/20;
}
...
.code-header {
    @apply flex items-center justify-between border-b border-gray-200 bg-gray-100 px-6 py-4 dark:border-gray-700 dark:bg-gray-800;
}
...
.format-btn {
    @apply rounded bg-blue-600 px-3 py-1 text-sm text-white transition-colors hover:bg-blue-700;
}

.json-editor {
    @apply flex-1 resize-none border-0 bg-gray-100 p-4 font-mono text-sm text-gray-900 dark:bg-gray-800 dark:text-gray-100;
    @apply focus:outline-none;
}

.json-error {
    @apply border-t border-red-200 bg-red-100 px-6 py-2 text-sm text-red-800 dark:border-red-800 dark:bg-red-900 dark:text-red-200;
}
...
/* Settings Panel */
.settings-panel {
    @apply absolute right-6 top-20 w-80 rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800;
...
.drag-preview {
    @apply flex items-center gap-3 rounded-lg bg-white px-6 py-4 shadow-xl dark:bg-gray-800;
}

.loading-content {
    @apply flex flex-col items-center gap-4 rounded-lg bg-white px-6 py-4 shadow-xl dark:bg-gray-800;
}
...
@media (max-width: 768px) {
    ...
    .block-editor-sidebar,
    .settings-panel {
        @apply inset-x-0 inset-y-auto w-full;
    }
    ...
    .view-split .visual-editor {
        @apply border-b border-r-0 border-gray-200 dark:border-gray-700;
    }
}
</style>
