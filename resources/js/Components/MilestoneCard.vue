<template>
    <div class="rounded-lg border border-green-200 bg-gradient-to-r from-green-50 to-blue-50 p-6 shadow-sm transition-shadow hover:shadow-md">
        ...
        <div class="mb-4 flex items-start justify-between">
            <div class="flex items-start space-x-4">
                ...
                <div class="flex-shrink-0">
                    <div :class="['flex h-12 w-12 items-center justify-center rounded-full', getIconBackground(milestone.type)]">
                        <component :is="getIcon(milestone.type)" :class="['h-6 w-6', getIconColor(milestone.type)]" />
                    </div>
                </div>
                ...
                <div class="flex-1">
                    ...
                    <p class="mb-2 text-sm text-gray-600">{{ formatMilestoneType(milestone.type) }}</p>
                    ...
                    <div class="flex items-center space-x-4 text-sm text-gray-600">
                        ...
                        <span v-if="milestone.company" class="flex items-center">
                            <BuildingOfficeIcon class="mr-1 h-4 w-4" />
                            {{ milestone.company }}
                        </span>
            ...
            <div v-if="canEdit" class="flex items-center space-x-2">
                <button @click="$emit('edit', milestone)" class="p-2 text-gray-400 transition-colors hover:text-gray-600" title="Edit milestone">
        ...
        <div v-if="milestone.description" class="mb-4">
            <p class="leading-relaxed text-gray-700">{{ milestone.description }}</p>
        </div>
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-2">
                <span :class="['inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium', getTypeColor(milestone.type)]">
                    {{ formatMilestoneType(milestone.type) }}
                </span>

                <span :class="['inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium', getVisibilityColor(milestone.visibility)]">
            ...
            <div class="flex items-center space-x-2">
                <button
                    v-if="!canEdit"
                    @click="congratulate"
                    class="inline-flex items-center rounded-full bg-green-100 px-3 py-1 text-xs font-medium text-green-700 transition-colors hover:bg-green-200"
                >
                    <HandThumbUpIcon class="mr-1 h-3 w-3" />
                    Congratulate
                </button>

                <button
                    @click="share"
                    class="inline-flex items-center rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-700 transition-colors hover:bg-blue-200"
                >
