<template>
    <form :id="formId" :class="formClasses" @submit.prevent="handleSubmit" :autocomplete="autocomplete" novalidate>
        <!-- Form Header -->
        ...
        <div class="form-actions" :class="actionsLayout">
            ...
            >
                <slot name="submit-icon">
                    ...
                    <loading-spinner v-else class="h-4 w-4" />
                </slot>
                {{ submitLabel }}
            </touch-optimized-button>
        </div>
...
watch(
    () => [isValid.value, props.disabled, props.readonly],
    () => {
        canSubmit.value = isValid.value && !props.disabled && !props.readonly;
    },
);
...
.form-title {
    @apply mb-2 text-2xl font-semibold text-gray-900 dark:text-white;
...
.form-message {
    @apply flex items-start gap-3 rounded-lg p-4 transition-all duration-300;
    min-height: 48px; /* Touch-friendly minimum */
}

.form-message.success {
    @apply border border-green-200 bg-green-50 text-green-800 dark:border-green-800 dark:bg-green-900/20 dark:text-green-300;
}

.form-message.error {
    @apply border border-red-200 bg-red-50 text-red-800 dark:border-red-800 dark:bg-red-900/20 dark:text-red-300;
}

.form-message.warning {
    @apply border border-yellow-200 bg-yellow-50 text-yellow-800 dark:border-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300;
}

.form-message.info {
    @apply border border-blue-200 bg-blue-50 text-blue-800 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-300;
}
...
.message-dismiss {
    @apply ml-2 flex-shrink-0 rounded p-1 hover:bg-black/10 dark:hover:bg-white/10;
    min-width: 32px;
    min-height: 32px;
}
...
/* Form Terms */
.form-terms {
    @apply mt-6 rounded-lg bg-gray-50 p-4 dark:bg-gray-800;
}
...
/* Form Footer */
.form-footer {
    @apply mt-8 border-t border-gray-200 pt-6 dark:border-gray-700;
}
...
.form-message-enter-from {
    @apply translate-y-2 transform opacity-0;
}
