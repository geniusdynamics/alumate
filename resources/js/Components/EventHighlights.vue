    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex space-x-2">
                <select v-model="filters.type" @change="loadHighlights" class="rounded-md border border-gray-300 px-3 py-2 text-sm">
                ...
                <select v-model="filters.sort_by" @change="loadHighlights" class="rounded-md border border-gray-300 px-3 py-2 text-sm">
        ...
        <!-- Highlights Grid -->
        <div v-else-if="highlights.length > 0" class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            <div
            ...
            >
                <!-- Featured Badge -->
                <div v-if="highlight.is_featured" class="bg-yellow-400 px-2 py-1 text-xs font-medium text-yellow-900">⭐ Featured</div>

                <!-- Media -->
                <div v-if="highlight.media_urls && highlight.media_urls.length > 0" class="aspect-w-16 aspect-h-9">
                    <img v-if="highlight.type === 'photo'" :src="highlight.media_urls[0]" :alt="highlight.title" class="h-48 w-full object-cover" />
                    <video v-else-if="highlight.type === 'video'" :src="highlight.media_urls[0]" class="h-48 w-full object-cover" controls></video>
                </div>
                ...
                <div class="p-4">
                    ...
                    <p v-if="highlight.description" class="mb-3 text-sm text-gray-600">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            ...
                            <button @click="shareHighlight(highlight)" class="flex items-center space-x-1 text-sm text-gray-500 hover:text-blue-600">
                            ...
                            <button @click="showComments(highlight)" class="flex items-center space-x-1 text-sm text-gray-500 hover:text-green-600">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path
                                        stroke-linecap="round"
                                        stroke-linejoin="round"
        ...
        <EventHighlightModal :show="showCreateModal" :event="event" @close="showCreateModal = false" @created="onHighlightCreated" />

        <!-- Comments Modal -->
        <EventHighlightComments :show="showCommentsModal" :highlight="selectedHighlight" @close="showCommentsModal = false" />
    </div>
</template>
