<template>
    <div class="saved-searches">
        <div class="saved-searches-header">
            ...
            <button @click="showCreateModal = true" class="create-search-btn" v-if="!showCreateModal">
                ...
                <form @submit.prevent="saveSearch" class="modal-form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="notification-options">
                            <label class="checkbox-item">
                                <input v-model="searchForm.email_alerts" type="checkbox" class="checkbox-input" />
                            ...
                            <div v-if="searchForm.email_alerts" class="alert-frequency">
                                ...
                                <select id="alert-frequency" v-model="searchForm.alert_frequency" class="form-select">
                    ...
                    <div class="form-actions">
                        ...
                        <button type="submit" class="save-btn" :disabled="isSaving">
                            <LoadingSpinner v-if="isSaving" class="h-4 w-4" />
        ...
        <div v-if="savedSearches.length > 0" class="saved-searches-list">
            <div v-for="search in savedSearches" :key="search.id" class="saved-search-item">
                <div class="search-info">
                    <div class="search-header">
                        <h4 class="search-name">{{ search.name }}</h4>
                        <div class="search-meta">
                            <span class="search-date"> Saved {{ formatDate(search.created_at) }} </span>
                    ...
                    <div class="search-stats">
                        <span class="last-run"> Last run: {{ search.last_run_at ? formatDate(search.last_run_at) : 'Never' }} </span>
                        <span v-if="search.last_result_count !== null" class="result-count"> {{ search.last_result_count }} results </span>
                    </div>
                </div>
                ...
                <div class="search-actions">
                    ...
                    >
                        <PlayIcon v-if="isRunning !== search.id" class="h-4 w-4" />
                        <LoadingSpinner v-else class="h-4 w-4" />
                    </button>

                    <button @click="editSearch(search)" class="action-btn edit-btn" :title="'Edit search: ' + search.name">
                    ...
                    <button @click="deleteSearch(search)" class="action-btn delete-btn" :title="'Delete search: ' + search.name">
        ...
        <div v-else-if="!isLoading" class="empty-state">
            ...
            <p class="empty-message">Save your searches to quickly access them later and get notified of new results.</p>
        </div>
...
const saveSearch = async () => {
    ...
    try {
        const url = editingSearch.value ? `/api/saved-searches/${editingSearch.value.id}` : '/api/saved-searches';
...
.modal-content {
    @apply mx-4 w-full max-w-md rounded-lg bg-white shadow-xl;
...
.cancel-btn {
    @apply rounded-md border border-gray-300 px-4 py-2 text-gray-700;
...
.query-text {
    @apply rounded bg-gray-100 px-2 py-1 font-mono text-sm text-gray-700;
}
...
.action-btn {
    @apply rounded p-2 text-gray-400 hover:text-gray-600;
    @apply transition-colors;
}
