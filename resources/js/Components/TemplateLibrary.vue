    ...
    <div class="template-library">
        ...
        <div class="search-filters">
            ...
            <div class="filter-controls">
                ...
                <div class="filter-group">
                    ...
                    <select v-model="selectedAudienceType" class="filter-select" @change="applyFilters">
                ...
                <div class="filter-group" v-if="campaignTypes.length > 0">
                    ...
                    <select v-model="selectedCampaignType" class="filter-select" @change="applyFilters" :aria-label="'Filter by campaign type'">
                ...
                <div class="filter-group">
                    ...
                    <select v-model="showPremiumOnly" class="filter-select" @change="applyFilters" :aria-label="'Filter by premium status'">
                ...
                <div class="filter-group" v-if="showFavorites">
                    ...
                    <select v-model="showRecentsOnly" class="filter-select" @change="applyFilters" :aria-label="'Filter by recency'">
                ...
                <div class="filter-group" v-if="showFavorites">
                    ...
                    <select v-model="showFavoritesOnly" class="filter-select" @change="applyFilters" :aria-label="'Show favorite templates'">
                        <option value="">All Templates</option>
        ...
        <div v-else-if="error" class="error-state">
            <div class="error-content">
                ...
                <button @click="fetchTemplates" class="retry-btn">Try Again</button>
            </div>
        </div>
        ...
        <div v-else class="templates-container">
            ...
            <div v-else class="empty-state">
                <div class="empty-content">
                    ...
                    <button v-if="searchQuery || hasActiveFilters" @click="clearFilters" class="reset-btn">Clear Search & Filters</button>
                </div>
            </div>
            ...
            <div v-if="templates.total > templates.perPage" class="pagination-container">
                <div class="pagination">
                    ...
                    <button @click="goToPage(currentPage + 1)" :disabled="currentPage === templates.lastPage" class="pagination-btn">Next</button>
                </div>
            </div>
        </div>
    </div>
    ...
    try {
        ...
        const campaigns = [
            { value: 'onboarding', label: 'Onboarding', count: templates.value.data.filter((t) => t.campaignType === 'onboarding').length },
            {
                ...
                count: templates.value.data.filter((t) => t.campaignType === 'event_promotion').length,
            },
            ...
            { value: 'networking', label: 'Networking', count: templates.value.data.filter((t) => t.campaignType === 'networking').length },
            {
                ...
                count: templates.value.data.filter((t) => t.campaignType === 'career_Services').length,
            },
...
const scrollToFocusedTemplate = () => {
    nextTick(() => {
        ...
        if (container && focusedTemplateIndex.value >= 0) {
            const templateElement = container.querySelector(`[data-template-index="${focusedTemplateIndex.value}"]`) as HTMLElement;
            if (templateElement) {
...
.view-toggle {
    @apply flex overflow-hidden rounded-lg border border-gray-300 dark:border-gray-600;
}

.view-btn {
    @apply px-3 py-2 text-gray-600 transition-colors hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white;
}
...
.search-input {
    @apply w-full rounded-lg border border-gray-300 bg-white py-3 pl-10 pr-4 text-gray-900 placeholder-gray-500 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400;
}
...
.filter-select {
    @apply w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white;
}

.clear-filters-btn {
    @apply w-full rounded-lg bg-gray-100 px-4 py-2 text-gray-600 transition-colors hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-400 dark:hover:bg-gray-600;
}
...
.error-icon {
    @apply mx-auto mb-4 h-12 w-12 text-red-500;
}
...
.active-filters {
    @apply flex flex-wrap items-center gap-2 border-b border-gray-200 px-6 py-4 dark:border-gray-700;
}
...
.filter-tag {
    @apply inline-flex items-center rounded-full bg-blue-100 px-3 py-1 text-sm text-blue-700 dark:bg-blue-900/30 dark:text-blue-300;
}
...
.clear-all-btn {
    @apply px-3 py-1 text-sm text-gray-600 underline hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200;
}

.templates-grid {
    @apply grid grid-cols-1 gap-6 p-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4;
}
...
.empty-icon {
    @apply mx-auto mb-4 h-12 w-12 text-gray-400;
}
...
.pagination-btn {
    @apply rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-600 transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700;
}
...
.pagination-number {
    @apply rounded px-3 py-2 text-gray-600 transition-colors hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white;
}
