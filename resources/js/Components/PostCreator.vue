<template>
    <div class="post-creator rounded-lg border bg-white p-6 shadow-sm">
        <form @submit.prevent="submitPost">
            ...
            <div class="mb-4 flex items-start space-x-3">
                ...
                <div class="flex-1">
                    ...
                    <div class="mt-1 flex items-center space-x-2">
                        <select v-model="form.visibility" class="rounded-md border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="mb-4">
                <div
                    ...
                    @input="updateContent"
                    @paste="handlePaste"
                    class="min-h-[120px] resize-none rounded-lg border border-gray-300 p-4 focus:border-transparent focus:ring-2 focus:ring-blue-500"
                    :class="{ 'border-red-300': errors.content }"
                    placeholder="What's on your mind?"
                ></div>
                <div class="mt-2 flex items-center justify-between">
                    <div class="text-sm text-gray-500">{{ contentLength }}/5000 characters</div>
                    <div class="flex space-x-2">
                        <button type="button" @click="formatText('bold')" class="p-1 text-gray-600 hover:text-gray-800" title="Bold">
                            <strong>B</strong>
                        </button>
                        <button type="button" @click="formatText('italic')" class="p-1 text-gray-600 hover:text-gray-800" title="Italic">
                            <em>I</em>
                        </button>
                        <button type="button" @click="insertLink" class="p-1 text-gray-600 hover:text-gray-800" title="Add Link">🔗</button>
                    </div>
                </div>
                <div v-if="errors.content" class="mt-1 text-sm text-red-500">
                    {{ errors.content }}
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                >
                    <input ref="fileInput" type="file" multiple accept="image/*,video/*,.pdf,.doc,.docx" @change="handleFileSelect" class="hidden" />
                ...
                <!-- Media Preview -->
                <div v-if="uploadedMedia.length" class="mt-4 grid grid-cols-2 gap-4 md:grid-cols-3">
                    <div v-for="(media, index) in uploadedMedia" :key="index" class="group relative">
                        <div class="aspect-square overflow-hidden rounded-lg bg-gray-100">
                            ...
                            />
                            <div v-else-if="media.type === 'video'" class="flex h-full w-full items-center justify-center bg-gray-200">
                                <svg class="h-12 w-12 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                                    <path
                                        d="M2 6a2 2 0 012-2h6l2 2h6a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6zM5 8a1 1 0 000 2v3a1 1 0 001 1h3a1 1 0 001-1V9a1 1 0 100-2H6a1 1 0 00-1 1z"
                                    />
                                </svg>
                            </div>
                        ...
                        <button
                            ...
                            class="absolute -right-2 -top-2 flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-sm text-white opacity-0 transition-opacity hover:bg-red-600 group-hover:opacity-100"
                        >
                            ×
                        </button>
                        <p class="mt-1 truncate text-xs text-gray-500">{{ media.original_name }}</p>
                    </div>
                </div>
            </div>
            ...
            <div v-if="form.visibility === 'groups' || form.visibility === 'specific'" class="mb-4">
                <h4 class="mb-2 text-sm font-medium text-gray-700">Select Audience</h4>

                <!-- Groups Selection -->
                <div v-if="form.visibility === 'groups' || form.visibility === 'specific'" class="mb-3">
                    <label class="mb-1 block text-sm text-gray-600">Groups</label>
                    <div class="max-h-32 overflow-y-auto rounded-md border border-gray-300 p-2">
                        <label v-for="group in userGroups" :key="group.id" class="flex items-center space-x-2 rounded px-2 py-1 hover:bg-gray-50">
                            <input
                                type="checkbox"
                                :value="group.id"
                ...
                <div v-if="form.visibility === 'specific'" class="mb-3">
                    <label class="mb-1 block text-sm text-gray-600">Circles</label>
                    <div class="max-h-32 overflow-y-auto rounded-md border border-gray-300 p-2">
                        <label v-for="circle in userCircles" :key="circle.id" class="flex items-center space-x-2 rounded px-2 py-1 hover:bg-gray-50">
                            <input
                                type="checkbox"
                                :value="circle.id"
            ...
            <!-- Post Type Selection -->
            <div class="mb-4">
                <label class="mb-1 block text-sm text-gray-600">Post Type</label>
                <select v-model="form.post_type" class="w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="mb-4">
                <label class="flex items-center space-x-2">
                    <input type="checkbox" v-model="isScheduled" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
            ...
            <div class="flex items-center justify-between border-t border-gray-200 pt-4">
                <div class="flex space-x-2">
                    <button
                        ...
                        class="rounded-md bg-gray-100 px-4 py-2 text-sm text-gray-700 hover:bg-gray-200 disabled:opacity-50"
                    >
                        Save Draft
                    </button>
                    <button
                ...
                <div class="flex space-x-2">
                    <button type="button" @click="resetForm" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Cancel</button>
                    <button
                        type="submit"
        ...
        <div v-if="draftSaved" class="fixed bottom-4 right-4 rounded-md bg-green-500 px-4 py-2 text-white shadow-lg">Draft saved!</div>
    </div>
</template>
