...
export class ComponentGroupingService {
     ...
     */
    public createGroup(name: string, description: string, category?: ComponentCategory, tags: string[] = []): ComponentGroup {
    ...
    public getGroupsForComponent(componentId: string): ComponentGroup[] {
        return this.defaultGroups.filter((group) => group.Components.includes(componentId));
    }
     ...
     */
    public getComponentHierarchy(Components: Component[], relationships: ComponentRelationship[]): any[] {
        const hierarchy: any[] = [];
        const processedComponents = new Set<string>();

        // Find root Components (no parents)
        const rootComponents = Components.filter((component) => {
            const hasParent = relationships.some((rel) => rel.type === 'child' && rel.componentId === component.id);
            return !hasParent;
        });
        ...
        rootComponents.forEach((component) => {
            if (!processedComponents.has(component.id)) {
                const node = this.buildHierarchyNode(component, Components, relationships, processedComponents);
                hierarchy.push(node);
            }
        });
    ...
    private buildHierarchyNode(
        ...
        const childRelationships = relationships.filter((rel) => rel.type === 'parent' && rel.componentId === component.id);
        ...
        childRelationships.forEach((rel) => {
            ...
            if (childComponent && !processedComponents.has(childComponent.id)) {
                const childNode = this.buildHierarchyNode(childComponent, allComponents, relationships, processedComponents);
     ...
     */
    private addToSearchIndex(index: Map<string, Component[]>, key: string, component: Component): void {
        if (!index.has(key)) {
            index.set(key, []);
        }
     ...
     */
    public searchComponents(query: string, searchIndex: Map<string, Component[]>): Component[] {
