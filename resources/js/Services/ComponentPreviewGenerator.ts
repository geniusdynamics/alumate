...
export class ComponentPreviewGenerator {
     ...
     */
    async generateBatchPreviews(Components: Component[], options: PreviewOptions = {}): Promise<Map<string, PreviewResult>> {
     ...
     */
    generatePlaceholderPreview(componentName: string, category: ComponentCategory, options: PreviewOptions = {}): PreviewResult {
        try {
            const opts = { ...this.defaultOptions, ...options };
    ...
    private async drawHeroPreview(component: Component, config: HeroComponentConfig, options: PreviewOptions): Promise<void> {
        if (!this.ctx) return;
        ...
        if (config.ctaButtons && config.ctaButtons.length > 0) {
            ...
            this.ctx.font = '12px Arial, sans-serif';
            this.ctx.textAlign = 'center';
            this.ctx.fillText(this.truncateText(config.ctaButtons[0].text, 12), buttonX + buttonWidth / 2, buttonY + buttonHeight / 2 + 4);
        }
    ...
    private async drawFormPreview(component: Component, config: FormComponentConfig, options: PreviewOptions): Promise<void> {
        if (!this.ctx) return;
        ...
        for (let i = 0; i < fieldsToShow; i++) {
            ...
            if (field.placeholder) {
                ...
                this.ctx.fillText(this.truncateText(field.placeholder, 25), padding + 20, fieldY + 17);
            }
        }
    ...
    private async drawTestimonialPreview(component: Component, config: TestimonialComponentConfig, options: PreviewOptions): Promise<void> {
        if (!this.ctx) return;
    ...
    private async drawStatisticsPreview(component: Component, config: StatisticsComponentConfig, options: PreviewOptions): Promise<void> {
        if (!this.ctx) return;
    ...
    private async drawCTAPreview(component: Component, config: CTAComponentConfig, options: PreviewOptions): Promise<void> {
        if (!this.ctx) return;
        ...
        if (config.type === 'banner' && config.bannerConfig) {
            ...
            if (config.bannerConfig.title) {
                this.ctx.fillText(this.truncateText(config.bannerConfig.title, 25), padding + width / 2, padding + height / 2 - 20);
            }

            if (config.bannerConfig.subtitle) {
                this.ctx.font = '11px Arial, sans-serif';
                this.ctx.fillText(this.truncateText(config.bannerConfig.subtitle, 35), padding + width / 2, padding + height / 2);
            }
    ...
    private async drawMediaPreview(component: Component, config: MediaComponentConfig, options: PreviewOptions): Promise<void> {
        if (!this.ctx) return;
    ...
    private drawPlaceholder(componentName: string, category: ComponentCategory, options: PreviewOptions): void {
        ...
        this.ctx.font = 'bold 14px Arial, sans-serif';
        this.ctx.textAlign = 'center';
        this.ctx.fillText(this.truncateText(componentName, 20), padding + width / 2, padding + height / 2 - 10);

        // Draw category
        this.ctx.fillStyle = '#6b7280';
        this.ctx.font = '11px Arial, sans-serif';
        this.ctx.fillText(category.charAt(0).toUpperCase() + category.slice(1), padding + width / 2, padding + height / 2 + 10);
    }
