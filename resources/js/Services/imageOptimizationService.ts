...
class ImageOptimizationService {
    ...
    public async generateOptimizedImgElement(config: ResponsiveImageConfig): Promise<string> {
        const { src, alt, sizes, loading = 'lazy', priority = false } = config;
     ...
     */
    private async generateSrcSet(baseUrl: string, breakpoints: number[], options: ImageOptimizationOptions = {}): Promise<string> {
        const sources: string[] = [];
     ...
     */
    public generateOptimizedUrl(baseUrl: string, options: ImageOptimizationOptions = {}): string {
        const { quality = 85, format = this.getBestSupportedFormat(), width, height, crop = false, gravity = 'center' } = options;
     ...
     */
    public generateBackgroundImage(
        src: string,
        config: {
        ...
        const { size = 'cover', position = 'center', preload = false } = config;
     ...
     */
    public async batchOptimizeImages(images: string[], options: ImageOptimizationOptions = {}): Promise<string[]> {
    ...
    private estimateOptimizationSavings(src: string): number {
        ...
        const avgAvifRatio = 0.2; // AVIF provides even better compression
        const avgFallbackRatio = 0.8; // JPEG fallback size

        const bestRatio = this.formatSupport.avif ? avgAvifRatio : this.formatSupport.webp ? avgWebpRatio : avgFallbackRatio;
...
export type { ImageFormat, ImageOptimizationOptions, ImageVariant, ResponsiveImageConfig };
