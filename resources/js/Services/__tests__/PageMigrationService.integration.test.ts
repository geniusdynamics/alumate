...
import { PageMigrationService } from '../PageMigrationService';
...
describe('PageMigrationService - Integration Tests', () => {
    ...
    describe('Migration Creation', () => {
        it('successfully creates a migration with tenant validation', async () => {
            const migrationConfig = {
                ...
                items: [{ type: 'pages' as const }, { type: 'templates' as const }],
                options: {
    ...
    describe('Migration Execution', () => {
        it('successfully executes a migration', async () => {
            ...
            expect(tenantValidationService.validateTenantAccess).toHaveBeenCalledWith('tenant-1');
            expect(mockedFetch).toHaveBeenCalledWith(
                '/api/migrations/migration-1/execute',
                expect.objectContaining({
                    method: 'POST',
                }),
            );
        ...
        it('handles migration execution with complex results', async () => {
            ...
            const mockResult = {
                ...
                failedItems: [{ type: 'pages', id: 'page-2', name: 'Error Page', error: 'Validation failed' }],
                skippedItems: [{ type: 'assets', id: 'asset-1', name: 'Large Image', reason: 'File too large' }],
                errors: [{ type: 'connection_error', message: 'Database connection timeout', occurredAt: new Date() }],
                warnings: [{ type: 'data_loss_warning', message: 'Some metadata may be lost', occurredAt: new Date() }],
                duration: 5000,
                startedAt: new Date(),
                completedAt: new Date(),
            };
    ...
    describe('Migration Management', () => {
        ...
        it('cancels a running migration', async () => {
            ...
            expect(mockedFetch).toHaveBeenCalledWith(
                '/api/migrations/migration-1/cancel',
                expect.objectContaining({
                    method: 'POST',
                }),
            );
    ...
    describe('Tenant Migration', () => {
        ...
        it('handles tenant migration with conflicts', async () => {
            ...
            const mockResult = {
                success: false,
                migratedItems: [],
                failedItems: [{ type: 'pages', id: 'page-1', name: 'Home Page', error: 'Conflict detected' }],
                errors: [{ type: 'conflict_error', message: 'Data conflict in target tenant', occurredAt: new Date() }],
                warnings: [],
                duration: 5000,
                startedAt: new Date(),
                completedAt: new Date(),
    ...
    describe('Concurrent Operations', () => {
        it('handles multiple simultaneous migrations', async () => {
            ...
            for (let i = 0; i < 5; i++) {
                promises.push(
                    migrationService.createMigration('tenant-1', {
                        ...migrationConfig,
    ...
    describe('Migration Rollback', () => {
        it('handles migration rollback', async () => {
            ...
            const mockRollbackResult = {
                success: true,
                rolledBackItems: [{ type: 'pages', id: 'page-1', name: 'Home Page', rolledBackAt: new Date() }],
                failedRollbacks: [],
                duration: 2000,
                startedAt: new Date(),
                completedAt: new Date(),
