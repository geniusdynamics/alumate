<template>
    <div class="min-h-screen bg-gray-100 py-8 dark:bg-gray-900">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div v-if="activeTab === 'builder'" class="space-y-8">
                <div class="overflow-hidden rounded-lg bg-white shadow-lg dark:bg-gray-800">
                    <div class="border-b border-gray-200 p-6 dark:border-gray-700">
                        <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Drag-and-Drop Form Builder</h2>
                        <p class="mt-2 text-gray-600 dark:text-gray-300">
                            Build forms visually by dragging field types from the sidebar and configuring their properties.
                        </p>
                    </div>

                    <FormBuilder :initial-config="builderConfig" @update:config="updateBuilderConfig" @preview="previewForm" @save="saveForm" />
                </div>
            </div>
            ...
            <div v-if="activeTab === 'templates'" class="space-y-8">
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                    <div
                        ...
                        :key="template.id"
                        class="cursor-pointer overflow-hidden rounded-lg bg-white shadow-lg transition-shadow hover:shadow-xl dark:bg-gray-800"
                    ...
                    >
                        <div class="p-6">
                            <div class="mb-4 flex items-center justify-between">
                                ...
                                <span :class="['rounded-full px-2 py-1 text-xs font-medium', getCategoryColor(template.category)]">
                                    {{ template.category }}
                                </span>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                <span class="text-sm text-gray-500 dark:text-gray-400"> {{ template.config.fields.length }} fields </span>
                                <button
                                    class="rounded bg-blue-100 px-3 py-1 text-sm text-blue-700 hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-blue-900/30 dark:text-blue-300 dark:hover:bg-blue-900/50"
            ...
            <div v-if="activeTab === 'live'" class="space-y-8">
                <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
                    ...
                    <div class="overflow-hidden rounded-lg bg-white shadow-lg dark:bg-gray-800">
                        <div class="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Lead Capture Form</h3>
                            <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">Individual alumni signup form with validation</p>
                        </div>

                        <div class="p-6">
                            <FormBase :config="leadCaptureTemplate.config" @submit="handleFormSubmit" @validation-change="handleValidationChange" />
                        </div>
                    </div>
                    ...
                    <div class="overflow-hidden rounded-lg bg-white shadow-lg dark:bg-gray-800">
                        <div class="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Demo Request Form</h3>
                            <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">Institution demo request with qualification fields</p>
                        </div>

                        <div class="p-6">
                            <FormBase :config="demoRequestTemplate.config" @submit="handleFormSubmit" @validation-change="handleValidationChange" />
                        </div>
                    </div>
                </div>
                ...
                <div class="mx-auto max-w-2xl">
                    <div class="overflow-hidden rounded-lg bg-white shadow-lg dark:bg-gray-800">
                        <div class="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Contact Form</h3>
                            <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">General contact form with anti-spam protection</p>
                        </div>

                        <div class="p-6">
                            <FormBase :config="contactTemplate.config" @submit="handleFormSubmit" @validation-change="handleValidationChange" />
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                    <div v-for="feature in features" :key="feature.title" class="rounded-lg bg-white p-6 shadow-lg dark:bg-gray-800">
                        <div class="mb-4 flex items-center">
                            <div :class="['rounded-lg p-2', feature.iconBg]">
                                <component :is="feature.icon" class="h-6 w-6" :class="feature.iconColor" aria-hidden="true" />
                            </div>
                        ...
                        <ul class="space-y-2">
                            <li v-for="item in feature.items" :key="item" class="flex items-center text-sm text-gray-600 dark:text-gray-300">
                                <svg class="mr-2 h-4 w-4 flex-shrink-0 text-green-500" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                                    <path
                                        fill-rule="evenodd"
        ...
        <!-- Preview Modal -->
        <div v-if="previewConfig" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex min-h-screen items-end justify-center px-4 pb-20 pt-4 text-center sm:block sm:p-0">
                <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true" @click="closePreview" />
                ...
                >
                    <div class="mb-4 flex items-center justify-between">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-white">Form Preview</h3>
                        <button
                            type="button"
                            class="rounded text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-gray-500 dark:hover:text-gray-300"
                            @click="closePreview"
                        >
                    ...
                    <div class="max-h-96 overflow-y-auto">
                        <FormBase :config="previewConfig" :readonly="true" @submit="() => {}" />
                    </div>
                </div>
            </div>
        </div>
...
const features = [
    ...
    },
    {
        ...
        items: ['Client & server validation', 'Custom validation rules', 'Accessible error messages', 'Validation summary'],
    },
    {
        ...
        items: ['Responsive design', 'Touch-optimized inputs', 'Mobile-first approach', 'Gesture support'],
    },
    {
        ...
        items: ['Screen reader support', 'Keyboard navigation', 'ARIA labels', 'High contrast support'],
    },
    {
        ...
        items: ['HubSpot integration', 'Salesforce support', 'Lead scoring', 'Automated workflows'],
    },
    {
        ...
        items: ['Auto-save progress', 'Analytics tracking', 'Honeypot protection', 'reCAPTCHA support'],
    },
];
