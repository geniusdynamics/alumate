                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                        ...
                        <div class="flex items-end">
                            <button
                                ...
                                class="inline-flex w-full items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50"
                            >
            ...
            <div class="rounded-lg bg-white shadow">
                ...
                <div class="p-6">
                    ...
                    <div v-if="selectedReportType === 'institutions'" class="space-y-6">
                        <div v-for="institution in reports.institutions" :key="institution.institution" class="rounded-lg border border-gray-200 p-4">
                            <h3 class="mb-4 text-lg font-medium text-gray-900">{{ institution.institution }}</h3>
                            ...
                            <div v-if="institution.report.course_performance?.length">
                                <h4 class="mb-2 text-sm font-medium text-gray-900">Course Performance</h4>
                                <div class="space-y-2">
                                    <div
                                        v-for="course in institution.report.course_performance"
                    ...
                    <div v-if="selectedReportType === 'employment'" class="space-y-6">
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                <h3 class="mb-4 text-lg font-medium text-gray-900">Employment by Status</h3>
                                <div class="space-y-3">
                                    <div
                                        v-for="status in reports.employment?.employment_by_status"
                                        :key="status.employment_status"
                                        class="flex items-center justify-between"
                                    >
                                        <div class="flex items-center">
                                            <div class="mr-3 h-3 w-3 rounded-full" :class="getEmploymentStatusColor(status.employment_status)"></div>
                                            <span class="text-sm font-medium capitalize text-gray-900">{{
                                                status.employment_status.replace('_', ' ')
                                            }}</span>
                                        </div>
                            ...
                            <div>
                                ...
                                <div class="space-y-2">
                                    <div
                                        v-for="employer in reports.employment?.top_employers"
                                        :key="employer.current_company"
                                        class="flex items-center justify-between"
                                    >
                        ...
                        <div>
                            <h3 class="mb-4 text-lg font-medium text-gray-900">Recent Employment Changes</h3>
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="divide-y divide-gray-200 bg-white">
                                        <tr v-for="change in reports.employment?.recent_employment_changes" :key="change.name">
                                            <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">{{ change.name }}</td>
                                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{{ change.course }}</td>
                                            <td class="whitespace-nowrap px-6 py-4">
                                                <span
                                                    class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize"
                                                    :class="getEmploymentStatusBadgeClass(change.status)"
                                                >
                                                    {{ change.status.replace('_', ' ') }}
                                                </span>
                                            </td>
                                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{{ change.company || '-' }}</td>
                                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{{ formatDate(change.updated_at) }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                    ...
                    <div v-if="selectedReportType === 'jobs'" class="space-y-6">
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                <h3 class="mb-4 text-lg font-medium text-gray-900">Job Categories</h3>
                                <div class="space-y-2">
                                    <div
                                        v-for="category in reports.jobs?.top_job_categories"
                            ...
                            <div>
                                <h3 class="mb-4 text-lg font-medium text-gray-900">Application Success Rate</h3>
                                <div class="text-center">
                        ...
                        <div>
                            <h3 class="mb-4 text-lg font-medium text-gray-900">Top Performing Employers</h3>
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Company</th>
                                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Total Jobs</th>
                                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Active</th>
                                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Filled</th>
                                    ...
                                    <tbody class="divide-y divide-gray-200 bg-white">
                                        <tr v-for="employer in reports.jobs?.employer_performance" :key="employer.company_name">
                                            <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">{{ employer.company_name }}</td>
                                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{{ employer.total_jobs }}</td>
                                            <td class="whitespace-nowrap px-6 py-4 text-sm text-green-600">{{ employer.active_jobs }}</td>
                                            <td class="whitespace-nowrap px-6 py-4 text-sm text-blue-600">{{ employer.filled_jobs }}</td>
                                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{{ employer.total_applications }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
