    ...
    <div class="messaging-app">
        ...
        <div class="messaging-layout">
            ...
            <div class="messaging-sidebar">
                <ConversationList :selected-conversation-id="selectedConversation?.id" @conversation-selected="selectConversation" />
            </div>
        ...
        <div v-if="showMobileSidebar" class="mobile-sidebar-overlay" @click="showMobileSidebar = false">
            <div class="mobile-sidebar" @click.stop>
                <div class="mobile-sidebar-header">
                    ...
                    <button @click="showMobileSidebar = false" class="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                        <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
                </div>
                <ConversationList :selected-conversation-id="selectedConversation?.id" @conversation-selected="selectConversationMobile" />
            </div>
        </div>

        <!-- Mobile header -->
        <div class="mobile-header">
            <button @click="showMobileSidebar = true" class="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="unreadCount > 0" class="mobile-unread-badge" @click="showMobileSidebar = true">
...
const updateUrl = (conversation) => {
    const url = conversation ? `/messages/${conversation.id}` : '/messages';

    const state = conversation ? { conversationId: conversation.id } : null;
...
.messaging-sidebar {
    @apply hidden w-80 border-r border-gray-200 bg-white lg:block dark:border-gray-700 dark:bg-gray-800;
}
...
.mobile-sidebar {
    @apply h-full w-80 bg-white shadow-xl dark:bg-gray-800;
}
...
.mobile-header {
    @apply flex items-center justify-between border-b border-gray-200 bg-white p-4 lg:hidden dark:border-gray-700 dark:bg-gray-800;
}

.mobile-unread-badge {
    @apply fixed bottom-4 right-4 z-40 flex h-12 w-12 cursor-pointer items-center justify-center rounded-full bg-blue-500 text-sm font-bold text-white shadow-lg lg:hidden;
}
