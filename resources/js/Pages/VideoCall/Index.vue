    ...
    <div class="video-calls-index">
                ...
                <div class="py-6">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            >
                                <VideoCameraIcon class="mr-2 h-4 w-4" />
                                Schedule Call
                            </button>
                            <button
        ...
        <div class="border-b bg-white">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <nav class="flex space-x-8">
                    ...
                    >
                        {{ tab.label }}
                        <span v-if="tab.count" class="ml-2 rounded-full bg-gray-100 px-2.5 py-0.5 text-xs text-gray-900">
                            {{ tab.count }}
                        </span>
                    </button>
                </nav>
        ...
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div v-if="activeTab === 'upcoming'" class="space-y-6">
                ...
                <div v-else-if="upcomingCalls.length === 0" class="py-12 text-center">
                ...
                <div v-else class="space-y-4">
                    <CallCard v-for="call in upcomingCalls" :key="call.id" :call="call" @join="joinCall" @edit="editCall" @cancel="cancelCall" />
                </div>
            </div>

            <!-- Active Calls -->
            <div v-if="activeTab === 'active'" class="space-y-6">
                <div v-if="activeCalls.length === 0" class="py-12 text-center">
                    <VideoCameraIcon class="mx-auto h-12 w-12 text-gray-400" />
            ...
            <div v-if="activeTab === 'coffee-chats'" class="space-y-6">
                <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
                    <!-- Received Requests -->
                    <div>
                        <h3 class="mb-4 text-lg font-medium text-gray-900">Received Requests</h3>
                        <div v-if="receivedRequests.length === 0" class="py-8 text-center">
                    ...
                    <!-- Sent Requests -->
                    <div>
                        <h3 class="mb-4 text-lg font-medium text-gray-900">Sent Requests</h3>
                        <div v-if="sentRequests.length === 0" class="py-8 text-center">
                            <CoffeeIcon class="mx-auto h-8 w-8 text-gray-400" />
                            <p class="mt-2 text-sm text-gray-500">No coffee chat requests sent</p>
                        </div>
                        <div v-else class="space-y-3">
                            <CoffeeChatRequestCard v-for="request in sentRequests" :key="request.id" :request="request" type="sent" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-if="activeTab === 'history'" class="space-y-6">
                <div v-if="callHistory.length === 0" class="py-12 text-center">
        ...
        <!-- Modals -->
        <ScheduleCallModal v-if="showScheduleModal" @close="showScheduleModal = false" @scheduled="handleCallScheduled" />

        <CoffeeChatModal v-if="showCoffeeChatModal" @close="showCoffeeChatModal = false" @requested="handleCoffeeChatRequested" />
    </div>
</template>
...
const receivedRequests = computed(() => {
    return coffeeChatRequests.value.filter((req) => req.recipient_id === props.user?.id && req.status === 'pending');
});

const sentRequests = computed(() => {
    return coffeeChatRequests.value.filter((req) => req.requester_id === props.user?.id);
});
...
const loadCoffeeChatRequests = async () => {
    try {
        ...
        const [receivedData, sentData] = await Promise.all([receivedResponse.json(), sentResponse.json()]);
