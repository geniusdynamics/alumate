    ...
    <AppLayout title="Key Performance Indicators">
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Key Performance Indicators</h2>
        ...
        <div class="py-12">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                    <div v-for="kpi in kpis" :key="kpi.id" class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            <!-- KPI Header -->
                            <div class="mb-4 flex items-center justify-between">
                                ...
                                <span :class="getStatusBadgeClass(kpi.status)" class="rounded-full px-2 py-1 text-xs font-medium">
                                    {{ kpi.status }}
                                </span>
                            </div>
                            ...
                            <div class="mb-4">
                                <div class="flex items-baseline">
                                    ...
                                    <span v-if="kpi.target_value" class="ml-2 text-sm text-gray-500"> / {{ formatTargetValue(kpi) }} </span>
                                </div>
                                <div class="mt-1 text-sm text-gray-500">Category: {{ kpi.category }}</div>
                            </div>
                            ...
                            <div v-if="kpi.trend_data && kpi.trend_data.length > 0" class="mb-4">
                                <h4 class="mb-2 text-sm font-medium text-gray-700">30-Day Trend</h4>
                                <div class="h-20">
                                    <AnalyticsChart :data="getTrendChartData(kpi)" type="line" :height="80" :options="getTrendChartOptions()" />
                                </div>
                            </div>

                            <!-- Actions -->
                            <div class="flex items-center justify-between">
                                <button @click="viewKpiDetails(kpi)" class="text-sm font-medium text-indigo-600 hover:text-indigo-900">
                                    View Details
                                </button>
                                <button
                ...
                <!-- Empty State -->
                <div v-if="kpis.length === 0" class="py-12 text-center">
        ...
        <Modal :show="showDetailsModal" @close="showDetailsModal = false" max-width="4xl">
            <div class="p-6" v-if="selectedKpi">
                <h3 class="mb-4 text-lg font-medium text-gray-900">{{ selectedKpi.name }} Details</h3>

                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <!-- KPI Information -->
                    <div>
                        <h4 class="mb-2 text-sm font-medium text-gray-700">Information</h4>
                        <dl class="space-y-2">
                            <div>
                            ...
                            <div>
                                ...
                                <dd>
                                    <span :class="getStatusBadgeClass(selectedKpi.status)" class="rounded-full px-2 py-1 text-xs font-medium">
                                        {{ selectedKpi.status }}
                                    </span>
                                </dd>
                            </div>
                    ...
                    <div v-if="selectedKpi.trend_data && selectedKpi.trend_data.length > 0">
                        <h4 class="mb-2 text-sm font-medium text-gray-700">Historical Trend</h4>
                        <div class="h-64">
                            <AnalyticsChart
        ...
        datasets: [
            {
                ...
                data,
                borderColor:
                    kpi.status_color === 'green'
                        ? '#10B981'
                        : kpi.status_color === 'yellow'
                          ? '#F59E0B'
                          : kpi.status_color === 'red'
                            ? '#EF4444'
                            : '#6B7280',
                backgroundColor:
                    kpi.status_color === 'green'
