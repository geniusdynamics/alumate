...
const updateStatistics = (course) => {
    router.post(
        route('courses.statistics.update', course.id),
        {},
        {
            preserveState: true,
            onSuccess: () => {
...
<template>
    ...
    <AppLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Courses Management</h2>
        </template>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="mb-6 flex justify-between">
                    <div class="flex gap-2">
                        <Link :href="route('courses.create')" class="rounded-md bg-indigo-600 px-4 py-2 font-medium text-white hover:bg-indigo-700">
                            Add Course
                        </Link>
                        <Link
                        ...
                        <button @click="exportCourses" class="rounded-md bg-green-600 px-4 py-2 font-medium text-white hover:bg-green-700">
                            Export Courses
                        </button>
                    </div>
                </div>
                ...
                <div class="mb-6 rounded-lg bg-white p-6 shadow">
                    ...
                    <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div>
                            <label for="study_mode" class="mb-1 block text-sm font-medium text-gray-700">Study Mode</label>
                            <select
                                id="study_mode"
                                v-model="studyMode"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                    ...
                    <div v-if="showAdvancedFilters" class="mt-4 border-t pt-4">
                        ...
                        <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                            <div>
                                <label for="is_active" class="mb-1 block text-sm font-medium text-gray-700">Status</label>
                                <select
                                    id="is_active"
                                    v-model="isActive"
                                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            ...
                            <div>
                                <label for="is_featured" class="mb-1 block text-sm font-medium text-gray-700">Featured</label>
                                <select
                                    id="is_featured"
                                    v-model="isFeatured"
                                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            ...
                            <div>
                                <label for="employment_rate_min" class="mb-1 block text-sm font-medium text-gray-700">Min Employment Rate (%)</label>
                                <input
                                    id="employment_rate_min"
                                    type="number"
                                    min="0"
                                    max="100"
                                    v-model="employmentRateMin"
                                    placeholder="0"
                                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                />
                            </div>
                            ...
                            <div>
                                <label for="duration_min" class="mb-1 block text-sm font-medium text-gray-700">Min Duration (months)</label>
                                <input
                                    id="duration_min"
                                    type="number"
                                    min="1"
                            ...
                            <div>
                                <label for="duration_max" class="mb-1 block text-sm font-medium text-gray-700">Max Duration (months)</label>
                                <input
                                    id="duration_max"
                                    type="number"
                                    min="1"
                ...
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Level & Duration</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                                        <button @click="toggleSort('total_graduated')" class="flex items-center hover:text-gray-700">
                                            Statistics
                                            <svg
                                                v-if="sortBy === 'total_graduated'"
                                                class="ml-1 h-4 w-4"
                                                :class="sortOrder === 'asc' ? 'rotate-180 transform' : ''"
                                                fill="currentColor"
                                                viewBox="0 0 20 20"
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                <tr v-for="course in courses.data" :key="course.id" class="hover:bg-gray-50">
                                            ...
                                            <div>
                                                <div class="flex items-center">
                                                    ...
                                                    <span
                                                        ...
                                                        class="ml-2 inline-flex rounded bg-yellow-100 px-2 py-1 text-xs text-yellow-800"
                                                    >
                                                        Featured
                                                    </span>
                                                    <span
                                                        v-if="!course.is_active"
                                                        class="ml-2 inline-flex rounded bg-red-100 px-2 py-1 text-xs text-red-800"
                                                    >
                                                        Inactive
                                                    </span>
                                                </div>
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <span :class="['mb-1 inline-flex rounded-full px-2 py-1 text-xs font-semibold', getLevelBadge(course.level)]">
                                        ...
                                        <span
                                            :class="[
                                                ...
                                                getStudyModeBadge(course.study_mode),
                                            ]"
                                        >
                                        ...
                                        <div class="flex items-center">
                                            <div class="mr-2 h-2 w-16 rounded-full bg-gray-200">
                                                <div
                                                    ...
                                                    :style="`width: ${course.employment_rate || 0}%`"
                                                ></div>
                                            </div>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex flex-wrap gap-1">
                                            <span
                                                v-for="skill in (course.skills_gained || []).slice(0, 3)"
                                                :key="skill"
                                            ...
                                            <span
                                                v-if="(course.skills_gained || []).length > 3"
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium">
                                        <div class="flex gap-2">
                                            <Link :href="route('courses.show', course.id)" class="text-indigo-600 hover:text-indigo-900"> View </Link>
                                            <Link :href="route('courses.analytics', course.id)" class="text-blue-600 hover:text-blue-900">
                                                Analytics
                                            </Link>
                                            <Link :href="route('courses.edit', course.id)" class="text-green-600 hover:text-green-900"> Edit </Link>
                                            <button @click="updateStatistics(course)" class="text-purple-600 hover:text-purple-900">
                                                Update Stats
                                            </button>
                                        </div>
                                    </td>
                    ...
                    <!-- Pagination -->
                    <div v-if="courses.links" class="border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                                <div>
                                    <p class="text-sm text-gray-700">Showing {{ courses.from }} to {{ courses.to }} of {{ courses.total }} results</p>
                                </div>
                                <div>
                ...
                <!-- Empty State -->
                <div v-if="courses.data.length === 0" class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6 text-center">
                        <div class="mb-4 text-lg text-gray-500">No courses found</div>
