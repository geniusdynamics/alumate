...
const hasApplied = (job) => {
    return job.applications && job.applications.some((app) => app.graduate_id === props.graduate?.id);
};

const isSkillMatch = (job) => {
    if (!props.graduate?.skills || !job.required_skills) return false;
    return job.required_skills.some((skill) => props.graduate.skills.includes(skill));
};
</script>
...
<template>
    ...
    <AppLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Browse Jobs</h2>
        </template>
            ...
            <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
                ...
                <div class="rounded-lg bg-white p-6 shadow">
                    ...
                    <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div>
                            <label for="job_type" class="mb-1 block text-sm font-medium text-gray-700">Job Type</label>
                            <select
                                id="job_type"
                                v-model="jobType"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                    ...
                    <div v-if="showAdvancedFilters" class="mt-4 border-t pt-4">
                        ...
                        <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                            <div>
                                <label for="experience_level" class="mb-1 block text-sm font-medium text-gray-700">Experience Level</label>
                                <select
                                    id="experience_level"
                            ...
                            <div>
                                <label for="salary_min" class="mb-1 block text-sm font-medium text-gray-700">Minimum Salary</label>
                                <input
                                    id="salary_min"
                                    type="number"
                                    v-model="salaryMin"
                            ...
                            <div>
                                <label for="course" class="mb-1 block text-sm font-medium text-gray-700">Related Course</label>
                                <select
                                    id="course"
                                    v-model="courseId"
                                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                ...
                <div class="rounded-lg bg-white p-4 shadow">
                    <div class="flex items-center justify-between">
                        <p class="text-sm text-gray-600">Showing {{ jobs.from }} to {{ jobs.to }} of {{ jobs.total }} jobs</p>
                        <div class="flex items-center gap-2">
                            <span class="text-sm text-gray-600">Sort by:</span>
                            <select class="rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                <div class="mb-2 flex items-start justify-between">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <span v-if="isSkillMatch(job)" class="inline-flex rounded-full bg-green-100 px-2 py-1 text-xs text-green-800">
                                            Skill Match
                                        </span>
                                        <span v-if="hasApplied(job)" class="inline-flex rounded-full bg-blue-100 px-2 py-1 text-xs text-blue-800">
                                            Applied
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3 flex items-center gap-3">
                                    <span :class="['inline-flex rounded px-2 py-1 text-xs font-semibold', getJobTypeColor(job.job_type)]">
                                        {{ job.job_type?.replace('_', ' ').toUpperCase() }}
                                    </span>
                                    <span
                                        :class="[
                                            'inline-flex rounded px-2 py-1 text-xs font-semibold',
                                            getExperienceLevelColor(job.experience_level),
                                        ]"
                                    >
                                ...
                                <div v-if="job.required_skills && job.required_skills.length > 0" class="mb-4">
                                    <p class="mb-2 text-sm font-medium text-gray-700">Required Skills:</p>
                                    <div class="flex flex-wrap gap-1">
                                        <span
                                            v-for="skill in job.required_skills.slice(0, 6)"
                                            :key="skill"
                                            :class="[
                                                'inline-flex rounded px-2 py-1 text-xs',
                                                graduate?.skills?.includes(skill) ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-700',
                                            ]"
                                        >
                                            {{ skill }}
                                        </span>
                                        <span
                                            v-if="job.required_skills.length > 6"
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <button @click="saveJob(job)" class="text-gray-400 hover:text-gray-600">
                                        ...
                                        <span v-else class="rounded-md bg-green-100 px-4 py-2 font-medium text-green-800"> Applied </span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <!-- Pagination -->
                <div v-if="jobs.links" class="rounded-lg bg-white px-4 py-3 shadow">
                    <div class="flex items-center justify-between">
