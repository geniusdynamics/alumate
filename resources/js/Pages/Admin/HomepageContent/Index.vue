    ...
    <div class="homepage-content-admin">
        <div class="header">
            ...
            <div class="header-actions">
                ...
                <button @click="exportContent" class="btn btn-secondary">Export Content</button>
                <button @click="showImportModal = true" class="btn btn-secondary">Import Content</button>
            </div>
        </div>
            ...
            <div class="approval-items">
                <div v-for="approval in pendingApprovals" :key="approval.id" class="approval-item">
                    ...
                    <div class="approval-actions">
                        <button @click="approveContent(approval.homepage_content_id)" class="btn btn-success btn-sm">Approve</button>
                        <button @click="rejectContent(approval.homepage_content_id)" class="btn btn-danger btn-sm">Reject</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div v-for="item in filteredContent" :key="item.id" class="content-item">
                ...
                <div class="content-body">
                    <div v-if="editingItem === item.id" class="edit-form">
                        <textarea v-model="editForm.value" class="content-textarea" rows="4"></textarea>
                        ...
                        <div class="form-actions">
                            <textarea v-model="editForm.change_notes" placeholder="Change notes (optional)" rows="2"></textarea>
                            <div class="buttons">
                ...
                <div class="content-footer">
                    <div class="content-info">
                        <small>
                            ...
                            <span v-if="item.approved_by"> | Approved by {{ item.approver?.name }} on {{ formatDate(item.approved_at) }} </span>
                        </small>
                    </div>

                    <div class="content-actions">
                        <button v-if="editingItem !== item.id" @click="editContent(item)" class="btn btn-sm btn-secondary">Edit</button>
                        <button @click="viewHistory(item.id)" class="btn btn-sm btn-secondary">History</button>
                        <button v-if="item.status === 'draft'" @click="requestApproval(item.id)" class="btn btn-sm btn-warning">
                            Request Approval
                        </button>
                        <button v-if="item.status === 'approved'" @click="publishContent(item.id)" class="btn btn-sm btn-success">Publish</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="history-content">
                ...
                <div class="history-body">
                    <div v-for="version in contentHistory" :key="version.id" class="history-item">
                        ...
                        <div v-if="version.change_notes" class="change-notes"><strong>Notes:</strong> {{ version.change_notes }}</div>
        ...
        <div v-if="showImportModal" class="import-modal">
            <div class="import-content">
                ...
                <div class="import-body">
                    <textarea v-model="importData" placeholder="Paste JSON content data here..." rows="10" class="import-textarea"></textarea>
...
.audience-badge.individual {
    background: #e3f2fd;
    color: #1976d2;
}
.audience-badge.institutional {
    background: #f3e5f5;
    color: #7b1fa2;
}
.audience-badge.both {
