...
const verifyEmployer = (employer) => {
    router.post(
        route('employers.verify', employer.id),
        {},
        {
            preserveState: true,
            onSuccess: () => {
...
const rejectEmployer = (employer) => {
    ...
    if (reason) {
        router.post(
            route('employers.reject', employer.id),
            {
...
const suspendEmployer = (employer) => {
    ...
    if (reason) {
        router.post(
            route('employers.suspend', employer.id),
            {
...
const reactivateEmployer = (employer) => {
    if (confirm('Are you sure you want to reactivate this employer?')) {
        router.post(
            route('employers.reactivate', employer.id),
            {},
            {
                preserveState: true,
                onSuccess: () => {
...
<template>
    ...
    <AppLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Employer Management</h2>
        </template>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="mb-6 flex justify-between">
                    <div class="flex gap-2">
                        <button @click="exportEmployers" class="rounded-md bg-green-600 px-4 py-2 font-medium text-white hover:bg-green-700">
                            Export Employers
                        </button>
                    </div>
                </div>
                ...
                <div class="mb-6 rounded-lg bg-white p-6 shadow">
                    ...
                    <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div>
                            <label for="verification_status" class="mb-1 block text-sm font-medium text-gray-700">Verification Status</label>
                            <select
                        ...
                        <div>
                            <label for="company_size" class="mb-1 block text-sm font-medium text-gray-700">Company Size</label>
                            <select
                                id="company_size"
                                v-model="companySize"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                    ...
                    <div v-if="showAdvancedFilters" class="mt-4 border-t pt-4">
                        ...
                        <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                            <div>
                                <label for="subscription_plan" class="mb-1 block text-sm font-medium text-gray-700">Subscription Plan</label>
                                <select
                            ...
                            <div>
                                <label for="is_active" class="mb-1 block text-sm font-medium text-gray-700">Account Status</label>
                                <select
                                    id="is_active"
                                    v-model="isActive"
                                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            ...
                            <div>
                                <label for="can_post_jobs" class="mb-1 block text-sm font-medium text-gray-700">Job Posting Permission</label>
                                <select
                                    id="can_post_jobs"
                                    v-model="canPostJobs"
                                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                ...
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                                ...
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                                        <button @click="toggleSort('verification_status')" class="flex items-center hover:text-gray-700">
                                            Verification Status
                                            <svg
                                                v-if="sortBy === 'verification_status'"
                                                class="ml-1 h-4 w-4"
                                                :class="sortOrder === 'asc' ? 'rotate-180 transform' : ''"
                                                fill="currentColor"
                                                viewBox="0 0 20 20"
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Company Details</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                                        <button @click="toggleSort('total_jobs_posted')" class="flex items-center hover:text-gray-700">
                                            Job Statistics
                                            <svg
                                                v-if="sortBy === 'total_jobs_posted'"
                                                class="ml-1 h-4 w-4"
                                                :class="sortOrder === 'asc' ? 'rotate-180 transform' : ''"
                                                fill="currentColor"
                                                viewBox="0 0 20 20"
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                <tr v-for="employer in employers.data" :key="employer.id" class="hover:bg-gray-50">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        ...
                                        <div v-if="employer.verification_completed_at" class="mt-1 text-xs text-gray-500">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        ...
                                        <div class="mt-1 flex items-center gap-1">
                                            ...
                                            <span
                                                :class="[
                                                    'inline-flex rounded-full px-2 py-1 text-xs font-semibold',
                                                    getSubscriptionBadge(employer.subscription_plan),
                                                ]"
                                            >
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium">
                                        <div class="flex gap-2">
                                            <Link :href="route('employers.show', employer.id)" class="text-indigo-600 hover:text-indigo-900">
                                                View
                                            </Link>
                                            <Link :href="route('employers.edit', employer.id)" class="text-green-600 hover:text-green-900">
                                                Edit
                                            </Link>
                    ...
                    <!-- Pagination -->
                    <div v-if="employers.links" class="border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
                        <div class="flex items-center justify-between">
                            <div class="flex flex-1 justify-between sm:hidden">
                                <Link
                                    v-if="employers.prev_page_url"
                                    :href="employers.prev_page_url"
                                    class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
                                >
                                    Previous
                                </Link>
                                <Link
                                    v-if="employers.next_page_url"
                                    :href="employers.next_page_url"
                ...
                <!-- Empty State -->
                <div v-if="employers.data.length === 0" class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6 text-center">
                        <div class="mb-4 text-lg text-gray-500">No employers found</div>
