<template>
    <AppLayout title="Loading States Examples">
        ...
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Loading States & Skeleton Screens</h1>
            ...
            <div class="mb-8 rounded-lg bg-white p-6 shadow dark:bg-gray-800">
                <h2 class="mb-4 text-xl font-semibold text-gray-900 dark:text-gray-100">Interactive Controls</h2>
                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"> Loading Duration </label>
                        <select
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"> Loading Type </label>
                        <select
                            v-model="loadingType"
                            class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100"
                        >
            ...
            <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
                ...
                <div class="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
                    <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">Basic Loading States</h3>
                    <div class="space-y-4">
                        <div>
                            <h4 class="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Spinner Loader</h4>
                            <LoadingSpinner :loading="loadingStates.spinner" size="md" color="primary" />
                            <BaseButton @click="triggerLoading('spinner')" :disabled="loadingStates.spinner" class="mt-2" size="sm">
                                Test Spinner
                            </BaseButton>
                        </div>

                        <div>
                            <h4 class="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Smart Loader with Context</h4>
                            <SmartLoader
                                :loading="loadingStates.smart"
                                context="Fetching user data..."
                                :show-progress="true"
                                :progress="smartLoaderProgress"
                            />
                            <BaseButton @click="triggerSmartLoader" :disabled="loadingStates.smart" class="mt-2" size="sm">
                                Test Smart Loader
                            </BaseButton>
                        </div>
                    </div>
                ...
                <!-- Skeleton Loaders -->
                <div class="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
                    <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">Skeleton Loaders</h3>
                    <div class="space-y-4">
                        <div>
                            <h4 class="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Card Skeleton</h4>
                            <SkeletonCard :loading="loadingStates.card" :show-avatar="true" :show-actions="true">
                                <div v-if="!loadingStates.card" class="rounded-lg border p-4">
                                    ...
                                    <p class="text-gray-700 dark:text-gray-300">This is sample content that appears after loading.</p>
                            ...
                            <BaseButton @click="triggerLoading('card')" :disabled="loadingStates.card" class="mt-2" size="sm">
                                Test Card Skeleton
                            </BaseButton>
                        </div>
                    </div>
                ...
                <!-- List Loading -->
                <div class="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
                    <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">List Loading States</h3>
                    <SkeletonList :loading="loadingStates.list" :item-count="5" :show-avatar="true">
                        <div v-if="!loadingStates.list" class="space-y-3">
                            <div v-for="item in sampleListData" :key="item.id" class="flex items-center space-x-3 rounded-lg border p-3">
                    ...
                    <BaseButton @click="triggerLoading('list')" :disabled="loadingStates.list" class="mt-4" size="sm"> Test List Loading </BaseButton>
                </div>

                <!-- Contextual Loading -->
                <div class="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
                    <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">Contextual Loading</h3>
                    <div class="space-y-4">
                        <ContextualLoader :loading="loadingStates.contextual" context="search" :metadata="{ query: 'alumni', count: 150 }">
                            <div v-if="!loadingStates.contextual" class="rounded-lg border p-4">
                                <h5 class="mb-2 font-medium">Search Results</h5>
                                <p class="text-gray-600 dark:text-gray-400">Found 150 alumni matching your search criteria.</p>
                            </div>
                        </ContextualLoader>
                        <BaseButton @click="triggerLoading('contextual')" :disabled="loadingStates.contextual" size="sm">
            ...
            <div class="mt-8 rounded-lg bg-white p-6 shadow dark:bg-gray-800">
                <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">Performance Metrics</h3>
                <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                    <div class="rounded-lg bg-gray-50 p-4 text-center dark:bg-gray-700">
                        <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">
                            {{ performanceMetrics.totalLoads }}
                        </div>
                        <div class="text-sm text-gray-600 dark:text-gray-400">Total Loads</div>
                    </div>
                    <div class="rounded-lg bg-gray-50 p-4 text-center dark:bg-gray-700">
                        <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ performanceMetrics.avgLoadTime }}ms</div>
                        <div class="text-sm text-gray-600 dark:text-gray-400">Avg Load Time</div>
                    </div>
                    <div class="rounded-lg bg-gray-50 p-4 text-center dark:bg-gray-700">
                        <div class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ performanceMetrics.successRate }}%</div>
                    ...
                    <div class="rounded-lg bg-gray-50 p-4 text-center dark:bg-gray-700">
                        ...
                        <div class="text-sm text-gray-600 dark:text-gray-400">Active Loaders</div>
                    </div>
                </div>
            </div>
        </div>
...
const { loadingStates, setLoading, clearLoading, isAnyLoading } = useLoadingStates(['spinner', 'smart', 'card', 'list', 'contextual']);

// Performance monitoring
const { startMeasurement, endMeasurement, getMetrics } = usePerformanceMonitoring();
...
const updateActiveLoaders = () => {
    performanceMetrics.value.activeLoaders = Object.values(loadingStates.value).filter((state) => state).length;
};
