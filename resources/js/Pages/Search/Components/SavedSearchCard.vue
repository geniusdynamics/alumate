    ...
    <div class="rounded-lg border border-gray-200 bg-white p-4 transition-shadow hover:shadow-md">
        <div class="mb-2 flex items-start justify-between">
            ...
            <div class="flex space-x-1">
                <button @click="$emit('execute', search)" class="text-sm text-indigo-600 hover:text-indigo-500" title="Execute Search">
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
                </button>
                <button @click="$emit('edit', search)" class="text-sm text-gray-400 hover:text-gray-600" title="Edit Search">
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path
                ...
                <button @click="$emit('delete', search)" class="text-sm text-red-400 hover:text-red-600" title="Delete Search">
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
        ...
        <div class="mb-3 text-sm text-gray-600">
            ...
            <span v-if="search.results_count !== null" class="ml-2"> ({{ search.results_count }} results) </span>
        </div>
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-2">
                <button
                    ...
                    :class="[
                        'flex items-center space-x-1 rounded px-2 py-1 text-xs',
                        search.alert_enabled ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600 hover:bg-gray-200',
                    ]"
                >
                    <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path
                            ...
                            stroke-linejoin="round"
                            stroke-width="2"
                            d="M15 17h5l-5 5v-5zM4.828 4.828A4 4 0 015.5 4H9v1H5.5a3 3 0 00-2.121.879l-.707.707A3 3 0 002 8.5V12h1V8.5a2 2 0 01.586-1.414l.707-.707A2 2 0 015.5 6H9V5H5.5a3 3 0 00-2.121.879z"
                        />
                    </svg>
