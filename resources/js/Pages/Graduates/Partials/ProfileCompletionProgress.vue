    ...
    <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
        <div class="p-6">
            <div class="mb-4 flex items-center justify-between">
                ...
                <span :class="['text-2xl font-bold', getProgressTextColor(completionPercentage)]"> {{ completionPercentage }}% </span>
            </div>

            <div class="mb-4 h-3 w-full rounded-full bg-gray-200">
                <div
                    :class="['h-3 rounded-full transition-all duration-300', getProgressColor(completionPercentage)]"
                    :style="`width: ${completionPercentage}%`"
                ></div>
            </div>

            <p :class="['mb-4 text-sm', getProgressTextColor(completionPercentage)]">
                {{ getProgressMessage(completionPercentage) }}
            </p>

            <!-- Completed Fields -->
            <div v-if="completedFields.length > 0" class="mb-4">
                <h4 class="mb-2 text-sm font-medium text-gray-700">Completed Sections</h4>
                <div class="flex flex-wrap gap-1">
                    <span v-for="field in completedFields" :key="field" class="inline-flex rounded bg-green-100 px-2 py-1 text-xs text-green-800">
            ...
            <div v-if="highPriorityMissing.length > 0" class="mb-4">
                <h4 class="mb-2 text-sm font-medium text-red-700">Critical Missing Information</h4>
                <div class="space-y-1">
                    <div v-for="field in highPriorityMissing" :key="field.key" class="flex items-center text-sm">
                        <svg :class="['mr-2 h-4 w-4', getFieldPriorityColor(field.priority)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div v-if="missingFields.length > 0">
                <h4 class="mb-2 text-sm font-medium text-gray-700">All Missing Information</h4>
                <div class="space-y-1">
                    <div v-for="field in missingFields.slice(0, 8)" :key="field.key" class="flex items-center text-sm">
                        <svg :class="['mr-2 h-4 w-4', getFieldPriorityColor(field.priority)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="getFieldPriorityIcon(field.priority)" />
                        </svg>
                        <span :class="getFieldPriorityColor(field.priority)">{{ field.label }}</span>
                        <span class="ml-auto text-xs capitalize text-gray-400">{{ field.priority }}</span>
                    </div>
                    <div v-if="missingFields.length > 8" class="ml-6 text-xs text-gray-500">+{{ missingFields.length - 8 }} more fields</div>
                </div>
            </div>
