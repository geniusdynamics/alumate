    ...
    <AppLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Import Preview</h2>
                <Link :href="route('graduates.import.create')" class="rounded-md bg-gray-300 px-4 py-2 font-medium text-gray-700 hover:bg-gray-400">
                    Back to Import
                </Link>
            </div>
        </template>
            ...
            <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                            ...
                            >
                                <div class="flex items-center justify-between">
                                    ...
                                    <span v-if="isRequiredField(header)" class="inline-flex rounded bg-green-100 px-2 py-1 text-xs text-green-800">
                                        Required
                                    </span>
                                </div>
                                <div class="mt-1 text-sm text-gray-600">{{ header }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="mb-4 flex items-center justify-between">
                            ...
                            <button @click="showAllRows = !showAllRows" class="text-sm font-medium text-indigo-600 hover:text-indigo-900">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="divide-y divide-gray-200 bg-white">
                                    <tr v-for="(row, index) in displayedRows" :key="index" class="hover:bg-gray-50">
                                        ...
                                        <td v-for="header in headers" :key="header" class="whitespace-nowrap px-3 py-2 text-sm text-gray-900">
                ...
                <div class="flex justify-between">
                    <Link
                        ...
                        class="inline-flex items-center rounded-md border border-transparent bg-gray-300 px-4 py-2 text-xs font-semibold uppercase tracking-widest text-gray-700 transition duration-150 ease-in-out hover:bg-gray-400 focus:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 active:bg-gray-500"
                    >
                        Cancel Import
                    </Link>
                    ...
                    <form @submit.prevent="submit">
                        <button
                            ...
                            class="inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-6 py-2 text-xs font-semibold uppercase tracking-widest text-white transition duration-150 ease-in-out hover:bg-indigo-700 focus:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 active:bg-indigo-900 disabled:opacity-50"
                        >
                            <svg
