    ...
    <AppLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Job Management</h2>
                <Link :href="route('jobs.create')" class="rounded-md bg-indigo-600 px-4 py-2 font-medium text-white hover:bg-indigo-700">
                    Post New Job
                </Link>
            </div>
        </template>
        ...
        <div class="py-12">
            <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
                ...
                <div class="rounded-lg bg-white p-6 shadow">
                    <h3 class="mb-4 text-lg font-medium text-gray-900">Filter Jobs</h3>
                ...
                <!-- Bulk Actions -->
                <div v-if="selectedJobs.length > 0" class="rounded-md border border-blue-200 bg-blue-50 p-4">
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-blue-800"> {{ selectedJobs.length }} job(s) selected </span>
                        <div class="flex gap-2">
                            <button @click="bulkAction('pause')" class="rounded bg-yellow-600 px-3 py-1 text-sm text-white hover:bg-yellow-700">
                                Pause Selected
                            </button>
                            <button @click="bulkAction('resume')" class="rounded bg-green-600 px-3 py-1 text-sm text-white hover:bg-green-700">
                                Resume Selected
                            </button>
                            <button @click="bulkAction('delete')" class="rounded bg-red-600 px-3 py-1 text-sm text-white hover:bg-red-700">
                                Delete Selected
                            </button>
                        </div>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Job Details</th>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Deadline</th>
                                ...
                                <tr v-for="job in jobs.data" :key="job.id" class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-start">
                                            <div>
                                                ...
                                                <div class="mt-1 text-xs text-gray-400">Posted {{ formatDate(job.created_at) }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <span :class="['inline-flex rounded-full px-2 py-1 text-xs font-semibold', getStatusBadgeClass(job.status)]">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-900">
                                        <div v-if="job.application_deadline">
                                            ...
                                            <div
                                                v-if="getDaysUntilDeadline(job.application_deadline) !== null"
                                                :class="[
                                                    'text-xs',
                                                    getDaysUntilDeadline(job.application_deadline) <= 3 ? 'text-red-600' : 'text-gray-500',
                                                ]"
                                            >
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-sm font-medium">
                                        <div class="flex items-center space-x-2">
                                            <Link :href="route('jobs.show', job.id)" class="text-indigo-600 hover:text-indigo-900"> View </Link>
                                            <Link :href="route('jobs.edit', job.id)" class="text-green-600 hover:text-green-900"> Edit </Link>
                                            <Link :href="route('jobs.analytics', job.id)" class="text-blue-600 hover:text-blue-900"> Analytics </Link>
                                            ...
                                            <button @click="duplicateJob(job)" class="text-purple-600 hover:text-purple-900">Duplicate</button>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
                ...
                <!-- Empty State -->
                <div v-if="jobs.data.length === 0" class="overflow-hidden rounded-lg bg-white shadow">
                    <div class="p-6 text-center">
                        <div class="mb-4 text-lg text-gray-500">No jobs found</div>
