    ...
    <AppLayout title="Achievements">
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">Achievements & Recognition</h2>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Celebrate and showcase alumni accomplishments</p>
                </div>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="mb-8 grid grid-cols-1 gap-6 md:grid-cols-4">
                    <div
                        ...
                        :class="[
                            'cursor-pointer rounded-lg bg-white p-6 shadow-md transition-all duration-200 dark:bg-gray-800',
                            selectedCategory === category.id ? 'bg-blue-50 ring-2 ring-blue-500 dark:bg-blue-900/20' : 'hover:shadow-lg',
                        ]"
                    >
                        <div class="flex items-center space-x-3">
                            <div :class="['flex h-12 w-12 items-center justify-center rounded-lg', category.color]">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400">{{ category.count }} achievements</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6 rounded-lg bg-white p-6 shadow-md dark:bg-gray-800">
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                        <!-- Search -->
                        <div>
                            <label for="search" class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"> Search Achievements </label>
                            <div class="relative">
                                <input
                                    id="search"
                                    v-model="searchQuery"
                                    type="text"
                                    class="block w-full rounded-md border border-gray-300 bg-white py-2 pl-10 pr-3 text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                        ...
                        <div>
                            ...
                            <select
                                ...
                                class="block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                            >
                ...
                <!-- Load More -->
                <div v-if="hasMoreAchievements" class="mt-8 text-center">
                    <button
        ...
        <CreateAchievementModal :show="showCreateModal" @close="showCreateModal = false" @created="handleAchievementCreated" />

        <!-- Achievement Celebration -->
        <AchievementCelebration v-if="celebratingAchievement" :achievement="celebratingAchievement" @close="celebratingAchievement = null" />
    </AppLayout>
</template>
...
const recentAchievements = computed(() => {
    ...
    return achievements.value.filter((achievement) => new Date(achievement.date) > oneMonthAgo);
});
...
const filteredAchievements = computed(() => {
    ...
    if (selectedYear.value) {
        filtered = filtered.filter((achievement) => new Date(achievement.date).getFullYear() === parseInt(selectedYear.value));
    }
