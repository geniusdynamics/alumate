...
export function useSpamProtection(config: Partial<SpamProtectionConfig> = {}) {
    const defaultConfig: SpamProtectionConfig = {
        ...
        blockedUserAgents: ['bot', 'crawler', 'spider', 'scraper', 'curl', 'wget'],
    };
    ...
    const isRateLimited = computed(() => {
        ...
        return submissionAttempts.value >= spamConfig.maxSubmissionRate && minutesSinceLastSubmission < 1;
    });
    ...
    const analyzeContent = (formData: Record<string, any>): { score: number; reasons: string[] } => {
        ...
        const suspiciousKeywordCount = spamConfig.suspiciousKeywords.filter((keyword) => textContent.includes(keyword.toLowerCase())).length;
