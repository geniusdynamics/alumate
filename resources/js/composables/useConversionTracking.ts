...
export function useConversionTracking() {
     ...
     */
    const trackConversion = (eventName: string, conversionEvent: CTAConversionEvent, additionalData?: Record<string, any>) => {
     ...
     */
    const trackCTAImpression = (ctaId: string, ctaType: string, variant?: string, context?: Record<string, any>) => {
    ...
    const getConversionFunnel = (goalId: string, timeRange?: { start: Date; end: Date }) => {
        ...
        const conversions = conversionQueue.value.filter((conversion) => {
            ...
            const inTimeRange = !timeRange || (conversion.timestamp >= timeRange.start.getTime() && conversion.timestamp <= timeRange.end.getTime());
    ...
    const getAttributionData = (conversionId: string) => {
        ...
        const conversion = conversionQueue.value.find((c) => c.timestamp.toString() === conversionId);

        if (!conversion) return null;

        // Find relevant clicks within attribution window
        const attributionWindow = 30 * 24 * 60 * 60 * 1000; // 30 days
        const relevantClicks = clickAttribution.filter(
            (click) => click.click_timestamp <= conversion.timestamp && click.click_timestamp >= conversion.timestamp - attributionWindow,
        );
    ...
    const getConversionMetrics = (timeRange?: { start: Date; end: Date }) => {
        const filteredConversions = timeRange
            ? conversionQueue.value.filter((c) => c.timestamp >= timeRange.start.getTime() && c.timestamp <= timeRange.end.getTime())
        ...
        // Group by category
        const byCategory = filteredConversions.reduce(
            (acc, conversion) => {
        ...
        // Group by CTA type
        const byCTAType = filteredConversions.reduce(
            (acc, conversion) => {
