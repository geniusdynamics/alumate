...
class GraduatesExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithColumnWidths, WithTitle, ShouldAutoSize
{
    protected $filters;
            ...
            case 'certifications':
                if (is_array($graduate->certifications)) {
                    return collect($graduate->certifications)->map(function ($cert) {
                        if (is_array($cert)) {
                            $parts = [];
                            if (!empty($cert['name'])) $parts[] = $cert['name'];
                            if (!empty($cert['issuer'])) $parts[] = "({$cert['issuer']})";
                            if (!empty($cert['date_obtained'])) $parts[] = "[{$cert['date_obtained']}]";
